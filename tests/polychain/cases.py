...
def setup_env():
    ...
    path - nothing here cooks VEX. ⚠️ AND IT CLAIMS THE PER-RUN TEMP
    DIRECTORY (32.2) - `$TEMP` resolves once, at `import hou`, which every
    runner does below this line."""
    for p in (os.path.dirname(os.path.abspath(__file__)),
              "%s/polyfactory/scripts/python" % REPO.replace("\\", "/")):
