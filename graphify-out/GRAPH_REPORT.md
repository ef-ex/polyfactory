...
## Corpus Check
- 150 files · ~586,470 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3505 nodes · 5602 edges · 222 communities (205 shown, 17 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 320 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `930b642a`
...
Cohesion: 0.17
Nodes (14): _dot(), _len(), _probe_s(), polyChain 4.5 SURFACE CONFORM - input 4, the drape onto a surface.  4.5 in full, Input 4, as one question: where does a point land when it is dropped?      THE C, (position, normal, distance) of the first hit, or None., The same question as `drop`, asked for every point in ONE `ray`.          11.2 P, (position, normal, hit). A miss returns `p` unmoved (D53).          The normal i (+6 more)
...
Cohesion: 0.08
Nodes (31): axis_follows_curve(), _band_case(), band_datum(), band_hybrid(), _degenerate(), _dist_xz(), exact_fill(), _flat_in_y() (+23 more)
...
Cohesion: 0.15
Nodes (13): duplicate_curve_ids(), polyline(), TWO CURVES AUTHORED WITH ONE `pc_curve_id` - D74's control build.      ⚠️ NOT A, The same case with an UNRELATED curve merged into input 1.      3.4's id rule is, Cook the same inputs again into fresh geometry - the determinism check., 200 x 2 m panels on a 400 m straight - PC-G3's shape, small enough to     run in, The same panels on a 10 m-radius arc - 100 % DEFORMED, the branch     `tripwire_, `tripwire_packed_run`'s fence, built INTO a caller-supplied geometry -     `buil (+5 more)
...
Cohesion: 0.24
Nodes (5): ConformPath, A `place.Path` seen through the surface. Same interface, draped answers.      Th, Did any station across [sa, sb] fall off the surface?          `fracs` is THE PI, `path` draped on `surface_geo`, or `path` itself when there is none.      D34 ag, wrap()
...
- **Why does `Node` connect `Community 23` to `Community 184`, `Community 83`, `Community 148`, `Community 87`?**
...
- **Why does `Result` connect `Community 108` to `Community 32`, `Community 39`, `Community 177`, `Community 178`, `Community 179`, `Community 23`, `Community 181`, `Community 151`, `Community 156`, `Community 221`?**
