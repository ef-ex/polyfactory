...
## Corpus Check
- 145 files · ~470,051 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3187 nodes · 5070 edges · 186 communities (172 shown, 14 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 310 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8a3e2824`
...
Cohesion: 0.12
Nodes (22): _bend_deviation(), _chord_ratio(), _cross(), _deform_positions(), _frame(), _len(), _needs_deform(), _packed_transform() (+14 more)
...
Cohesion: 0.06
Nodes (36): 0.0 BUILD STATE — resume pointer (read this first, keep it current), 0. Instructions to the implementing agent, 10. Build log, 1. Scope and non-goals, 2.1 The two-face principle (load-bearing), 2.2 Node family (working names; `pf_` per suite convention), 2.3 Chain SOP seed — gate PC-G0, 2. Architecture (+28 more)
...
Cohesion: 0.14
Nodes (14): ConformPath, _dot(), _len(), polyChain 4.5 SURFACE CONFORM - input 4, the drape onto a surface.  4.5 in full, (position, normal, hit). A miss returns `p` unmoved (D53).          The normal i, A `place.Path` seen through the surface. Same interface, draped answers.      Th, Did any of `n` stations across [sa, sb] fall off the surface?, Does the drape between `sa` and `sb` leave the straight chord?          This is (+6 more)
...
Cohesion: 0.14
Nodes (16): conform_contact(), conform_drape(), conform_misses(), corner_reach(), corner_turns(), corner_welds(), module_winding(), Signed distance from `p` to the surface along the conform axis.      Negative = (+8 more)
...
Cohesion: 0.05
Nodes (29): fence_kit(), polyChain 4.3 CORNERS - the hou-free half, tested in milliseconds.  Same contrac, D39, revised. The first version gave each copy its own plane and         parted, D48. A `stepped` piece is built plumb on the horizontal projection,         so t, D38 - the odd/even rule, as an exact equality on the reserves., D40 - the three policies are three different numbers, and only in     miter mode, D40, revised. The offset used to be folded into this number, back         when t, The orchestrator, end to end - still no geometry. (+21 more)
...
Cohesion: 0.07
Nodes (44): elem_key(), A 31-bit int for grouping/sorting only - NEVER an identity., _anchor_len(), _anchor_transform(), _blank_to_none(), build(), clip_plane(), _collate_warnings() (+36 more)

### Community 78 - "Community 78"
Cohesion: 0.06
Nodes (29): AssetThumbnailWidget, HoverComboBox, HoverSlider, _load_pixmap_cached(), Asset Browser Widgets - Low-level widget classes extracted from browser_ui.py., Return a cached, pre-scaled pixmap for path, loading from disk on miss., Individual asset thumbnail with animated hover outline, Update thumbnail size dynamically without re-reading disk (+21 more)
...
Cohesion: 0.07
Nodes (27): cap_tagged(), deformed_flag_matches_geometry(), elem_ids_survive_upstream(), element_count(), element_resolution(), geometry_digest(), instancing_split(), kit_validation() (+19 more)
...
Cohesion: 0.08
Nodes (34): broken_kit(), build_all(), camber_z(), _case(), coarse_kit(), compose_kit(), corner_style(), fence_style() (+26 more)
...
Cohesion: 0.10
Nodes (20): _assembly_placements(), Bevel, _mul(), _piece_span(), plan_curve(), Metres one corner assembly takes off one leg's default fill.      NEVER NEGATIVE, WARN_CORNER_DEGENERATE onto every piece that spans a dissolved corner., One solved corner. Angles in DEGREES at the boundary, radians inside.      `e_fo (+12 more)
...
Cohesion: 0.28
Nodes (9): compose_modules(), _corner_rule(), _default_module(), The composed corner module list, in payload order (3.3's `sequence`).      A `se, The module a section's default run is made of - the one whose width the     disp, D48 - will every piece cut on this bevel be built PLUMB?      `place._frame` fla, Every corner of one curve, solved. -> {boundary index: Assembly}.      A boundar, solve_corners() (+1 more)
...
Cohesion: 0.06
Nodes (32): Asset Place HDA UI - Python Panel for pf_asset_place node.  Shows the full ass, AssetInstanceWidget, createInterface(), KitbashNodeUI, PlacedAssetsListWidget, Kitbash HDA UI - Python Panel interface for pf_kitbash node  NEW WORKFLOW: -, Create styled spinbox, Load values from node parameters (+24 more)
...
Cohesion: 0.07
Nodes (40): axis_points(), cap_dressing(), cross_section_width(), _degenerate(), _dist_xz(), exact_fill(), _fill_span(), _groups() (+32 more)
...
Cohesion: 0.11
Nodes (10): object, Marker, Params, Every HDA parm the kernel reads. Angles in DEGREES (artist boundary).      `fill, 3.1's marker point. `u` (0-1) OR `dist` (m, negative = from the end)., Resolve to metres along `curve`. Out-of-range values are clamped., Payload order preserved: the first rule that yields wins., Style (+2 more)
...
Cohesion: 0.16
Nodes (12): _apply_cuts(), build_assembly(), CornerPiece, displacement(), _half_width(), _overflow(), Hand the miter plane to every default piece whose GEOMETRY can cross it.      NO, A default piece cut by a corner it was supposed to stop short of.      With no c (+4 more)
...
Cohesion: 0.10
Nodes (18): Kit, kit_from_records(), Module, polyChain - the contracts the 1D kernel is written against. No `hou`.  Spec: `id, One kit piece (3.2). `size` is the nominal FITTED size in metres., D55 - does THIS module take the surface camber?, The blank box a missing module becomes. Never a failure (3.4)., Payload order preserved - deterministic, never set iteration. (+10 more)
...
Cohesion: 0.09
Nodes (32): _area(), _attrs(), bank_adaptive(), _by_zmode(), _clip_convex(), collect_warns(), corner_wedge(), determinism() (+24 more)
...
## Knowledge Gaps
- **627 isolated node(s):** `Project Overview`, `Package Structure`, `Environment Variables`, `Temporary / One-Off Scripts`, `Houdini Bridge - AI Agent Integration` (+622 more)
...
- **Why does `Node` connect `Community 83` to `Community 184`, `Community 148`, `Community 158`, `Community 87`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `Result` connect `Community 7` to `Community 185`, `Community 101`, `Community 38`, `Community 71`, `Community 104`, `Community 74`, `Community 43`, `Community 107`, `Community 86`, `Community 88`, `Community 89`, `Community 158`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `PyPushButton` connect `Community 78` to `Community 65`, `Community 36`, `Community 72`, `Community 137`, `Community 15`, `Community 20`, `Community 92`, `Community 126`?**
