...
## Corpus Check
- 166 files · ~685,053 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4043 nodes · 6449 edges · 248 communities (231 shown, 17 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 352 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `338a3ca5`
...
Cohesion: 0.11
Nodes (34): benches(), check(), config_payload_parity(), decompose_parity(), f32(), _fingerprint(), frame_calls(), frames_geometry() (+26 more)
...
Cohesion: 0.12
Nodes (30): colour_warnings(), config_dict(), cook(), cook_config(), cook_plan_bridge(), _input_geo(), kit_geometry(), _padded() (+22 more)
...
Cohesion: 0.25
Nodes (8): 15.0 What actually exists now, 15.1 Parity — the numbers, all of them, 15.2 §13 was wrong about the speed, and here is the measurement, 15.3 `sop_cooks_per_build` — the new tripwire, and what it pins, 15.4 Two Houdini traps found this cycle, both silent, 15.5 Decisions, 15.6 What is NOT done, 15. Native network build log — cycle N-1
...
Cohesion: 0.14
Nodes (21): Params, Every HDA parm the kernel reads. Angles in DEGREES (artist boundary).      `fill, _check_cond(), _check_slot(), _dattr(), _ensure(), _int(), _modules() (+13 more)
...
Cohesion: 0.14
Nodes (15): Binding Manager - Central coordinator for parameter-widget synchronization  Ma, Hover Outline Mixin - Animated blue outline on hover for any widget, Polyfactory Widgets Module  Qt widgets for Houdini workflows.  Submodules:, HoudiniGroupBox, HoudiniHLayout, HoudiniVLayout, Layout Helpers - Houdini-styled layouts  Provides layout classes styled to mat, Vertical layout with Houdini spacing. (+7 more)
...
Cohesion: 0.12
Nodes (15): elem_id(), kit_from_records(), Module, polyChain - the contracts the 1D kernel is written against. No `hou`.  Spec: `id, One kit piece (3.2). `size` is the nominal FITTED size in metres., D55 - does THIS module take the surface camber?, Build a Kit from the manifest dicts the adapter reads off points.      D4: `pc_r, The structural address (D1). Collision-free by construction. (+7 more)
...
Cohesion: 0.11
Nodes (13): AssetBrowserDialog, AssetDropHandler, _handle_drop(), Asset Browser UI - Grid view of assets with search and filtering, Standalone asset browser dialog, Double-click in the floating browser: create and connect the         asset plac, Show the asset browser dialog, Handles drops from the asset browser onto Houdini's viewport or network     edi (+5 more)
...
Cohesion: 0.11
Nodes (8): corner_style(), The PC-G1 fence, with 4.3's parms exposed. Same kit, same rules., One 3.3 rule point.      D119 (E1) - `yclass` scopes the rule to ONE row class o, Rule, 7.6 / D137 - enough of the clipped area to fill a facade panel., D120 - one payload, two axes, and `Style` itself does not change., TestClippedArea, TestStyleSplit
...
Cohesion: 0.20
Nodes (9): config_stub(), feed(), The NATIVE stage chain, built as real SOP nodes, for the parity harness.      13, The CONFIG stream, until 13.3.0's Python SOP exists: one detail point     carryi, One Attribute Wrangle, its VEX inlined from the .vfl of the same name.      `vex, 4.1 - unshare, arclength, the curve-id table, corners, markers.      Returns (la, A `file` SOP serving `geometry` - the rig's way of putting a     `hou.Geometry`, stage_decompose() (+1 more)
...
Cohesion: 0.29
Nodes (4): (X `Style`, Y `Style`) off one payload. `Style` itself is unchanged.      Rule, split_style(), Payload order preserved: the first rule that yields wins.          D119 - with a, Style
...
Cohesion: 0.18
Nodes (5): Curve, One polyline primitive as plain data.      `corner_flags` / `section_ids` are pe, Cumulative metres at point `i` (i == len(points) => the closing)., 4.3 item E, and D42/D43., TestFillet
...
Cohesion: 0.18
Nodes (5): Load existing categories from database, HoverOutlineMixin, PyLineEdit, Fallback if hover_outline not available, QLineEdit
...
Cohesion: 0.14
Nodes (14): 1. Every attribute that leaves a node is prefixed `pf_`, 2. Internal attributes start with `_` and must not survive the output, 3. Migration — new code now, existing tools when next touched, 4. What this does not change, 5. Groups obey both rules, exactly as attributes do, 6. `_*` and `__*` are the same rule — do not add a third spelling, 7. Enforcement — `tests/hda/run_attrib_checks.py`, 8. Migration log — what has already moved (+6 more)
...
Cohesion: 0.29
Nodes (6): export_batch_group(), Export a single asset group detected by detect_asset_groups.      Converts pri, acquire_shared_panel(), Create a shared floating panel for batch renders.      Call once before starti, Close the shared panel acquired by acquire_shared_panel().      Call once afte, release_shared_panel()

### Community 227 - "Community 227"
Cohesion: 0.14
Nodes (22): build(), cook(), feeds(), main(), merge(), polyfactory_defs(), published(), Attribute hygiene for every polyfactory HDA, in a throwaway Houdini session. (+14 more)
...
Cohesion: 0.53
Nodes (5): export, pc_bisect_left(), pc_bisect_right(), pc_unit(), vector()
...
## Knowledge Gaps
- **809 isolated node(s):** `Project Overview`, `Package Structure`, `Environment Variables`, `Temporary / One-Off Scripts`, `Houdini Bridge - AI Agent Integration` (+804 more)
...
- **Why does `Node` connect `Community 42` to `Community 72`, `Community 83`, `Community 148`, `Community 87`, `Community 184`?**
  _High betweenness centrality (0.122) - this node is a cross-community bridge._
- **Why does `Rule` connect `Community 149` to `Community 161`, `Community 135`, `Community 8`, `Community 72`, `Community 105`, `Community 76`, `Community 211`, `Community 21`, `Community 54`, `Community 119`, `Community 186`, `Community 188`?**
