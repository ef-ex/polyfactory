...
fills the gaps between them, and D15 keeps every anchor at least half a spacing clear of a capped
end. So an anchor can never land against a corner assembly or a start/end cap **by construction** —
which is the property the composed fill could not have.
...
- **Dev-loop rule 0 is NOT discharged.** Nothing here has been audited by an agent that did not
  write it. The honest words are **implemented, self-verified, unaudited**. The first things an
  auditor should attack: `single_pillar`'s aspect-ratio classifier on a kit that is not the starter
  fence; the two pinned `DOUBLE_PILLAR` entries (are they really the same defect, or is the check
  over-reaching?); and whether `evenly_spacing = 2.0` is the right shipped number for a kit whose
  panel is not 2 m.
