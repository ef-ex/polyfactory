...
| Last completed | **CYCLE V1a — THE AUDIT OF §28, WORKED THROUGH (2026-08-24). Read §28.7 first, then §28.** Fifteen findings from an agent that did not write §28; every one reproduced from a pristine export before it was touched. ⚠️ **THE STATED MECHANISM OF THE §28 FIX WAS FALSE, AND THE FALSE SENTENCE SHIPPED INSIDE THE ASSET'S OWN HELP STRING**: D15's half-module shed keys on `start_cap`/`end_cap` and **D18 makes both FALSE at a corner**, so it never ran there - what kept the shipped defaults clean was `justify = "center"`, an accident of one default. ONE Advanced parm reached the very defect the cycle exists to fix: `Evenly Justify = From the end` on a 12.161 m leg drove the evenly post **0.061 m INTO** the mitered corner post, and `Adjust to End` did it at every corner of every leg. **D269** guards a `trim` reservation exactly as it guards a cap (two lines); `Adjust to End` is left abutting ON PURPOSE - landing the last anchor on the end is what the parm means - and is pinned at 0.12 m with both help strings corrected. ⚠️ **AND `single_pillar` WAS FOUR DIFFERENT WRONG CHECKS**, every one measured: a **false NEGATIVE** (a blocky corner module was never protected - `corner_post` 0.111698 vs `corner_block` **0.0, ok=True** on the same mutation); a **false POSITIVE** on ordinary picket spacings (aspect read off the PLACED box, so `evenly_spacing = 0.35` reported **11.90 m** of double pillar and 0.50/0.60/0.80 read 0.0/**0.28**/0.0); a **degenerate metric** (a touching CLUSTER's span is the RUN LENGTH - **1999.88 m** on 2 km); and **O(n²)** cost (**33.9 s** on 16 667 uprights). **D270** protects by SLOT, classifies by the MODULE's own `pc_local` aspect, measures PAIRWISE and buckets the sweep: 0.0 across the whole spacing sweep, 0.110491 on the blocky corner, 0.12 on the 2 km run, **0.108 s** at 16 667. The three pinned `DOUBLE_PILLAR` values did not move. ⚠️ **AND THE FIXTURE GAP OF §28.1(c) WAS INVERTED, NOT CLOSED** - `grep -c evenly tests/polychain/cases.py` returned **0**, so the ~35 corner checks were again running a composition the asset does not ship. Nine cases (EA–EI) close it, including the blocky corner and the first marker-on-a-cornered-curve case in the suite. **D271: a fixture's own dimensions are part of the fixture** - every justification reads 0.0 on a 12 m leg at 2 m spacing whether the engine is fixed or broken, so `RECT_ODD` is 12.161 m. Baseline: **9 cases ADDED, 0 removed, 6 values moved (all SKIP → 0.0 - six cases that could not be measured before now are), 90 detail strings reworded.** Standing findings, named not implied: **§28.8** (the engine cannot phase a composed unit after a corner; a marker inside a corner's reserve shares ground with it; the shipped default's DEFORMED path has no cost ceiling; `evenly_spacing = 2.0` is the starter panel's length, not a general answer). ⚠️ **AND THE SWEEP ITSELF FOUND ONE MORE**: `kit_input_unplugged` SURVIVED - the asset's kit port wired to NOTHING reddened nothing at all in 37 checks, because `input2_is_the_kit` asserted a NAME and `Kit.resolve` ends `return [stand_in(name)]`, so a module the kit cannot supply becomes a blank 1 x 1 x 1 box CARRYING THE REQUESTED NAME. **D272** makes it read the module's own geometry off `pc_local` and the mutation reddens it. Mutations from a pristine export: **scene 9/9 RED, hda 5/5 RED, 0 survivors**. Suites from a pristine export: **287 unit OK, scene 0/0, 2d 0/0, native 0, hda 0 (37 rows), attrib 0/0, scale_gate 0 rows, gate_images 0**. **PC-G1 IMAGES REGENERATED AND LOOKED AT: ONE pillar at every corner in BOTH modes and on the 3 m close-up** (§28.10). Previous entry: **CYCLE V1 — THE DOUBLE PILLAR AT EVERY CORNER (2026-08-24). Read §28 first.** Hannes found it in the viewport on the shipped defaults: at every mitered corner of the PC-G1 rectangle an artist sees TWO pillars of different height (the corner assembly's 1.30 m post, then the run's own 1.20 m `post` at EXACTLY 0.0 m). The kernel was never wrong — the defect was the SHIPPED COMPOSITION (`Repeating Pieces` = `post panel`, so every section opens with a post) plus a FIXTURE SET that could not reach it (`cases.corner_style` fills with `panel` alone, so the one combination the parameter page shipped was in no scene case). Fixed to RailClone's canonical fence (D266: panels fill, posts are Evenly), new composition check `double_pillar_m` (D267) with three registered mutations (all three run through the mutation registry and reported RED), and three cost fixtures pinned to the composition their ceilings were measured on (D268). Previous entry: **CYCLE N-4F — THE §26 AUDIT, WORKED THROUGH (2026-08-24). Read §27 first, then §26, then §25.** Twenty findings, every one reproduced from a pristine `git archive HEAD` export before it was touched and every one closed as a standing check with a mutation that reddens. ⚠️ **FOUR SHIPPED DIVERGENCES ON THE DEFAULT PATH**, each with level 1 and level 2 admitting and no warning on either side: an `attr:` subject at a FLOAT ARRAY / STRING ARRAY / DICT storage (12 gate prims natively against the reference's 10 panels — the refusal tested `t == 3` and `primattribtype` returns 3 only for an INT array); a `markerData:` slot holding a vector, a list or a nested dict (read as 0.0 and published as READABLE); a marker-data KEY containing a quote, a backslash or a non-ASCII character (the probe searched for the key's SOURCE spelling, and the failure runs in BOTH directions — it fired a gate nobody asked for AND dropped one that was asked for); and `pc_row_warns`, which made `place.build` RAISE while the native chain shipped a fence. ⚠️ **AND TWO MORE THAT ONLY THE FIXED INSTRUMENT COULD SEE**: giving `_snapshot` point-attribute VALUES and a real STORAGE dimension (`numericDataType()`, which `dataType()` cannot express) turned **29 of 93 cases red on the first run** — every int of 3.4's stamp shipped at **int64 against the reference's int32** on every admitted build, and the cast has to be the LAST node of the stage because `pc_warn_collate` creates attributes at cook time. ⚠️ **AND A 118x COST REGRESSION ON A MAIN-PAGE PARM**: `pc_fill`'s twelve `export` plan arrays were copied in and out once per gap, so `Evenly Spaced Piece` at 0.5 m on a 2 km run cooked **39.1 s against the reference's 0.33 s**; fresh arrays plus a pool hoist take it to 2.99x, and 4 000 anchors from **47 168 ms to 719 ms**. ⚠️ **ONE FINDING'S PROPOSED FIX WAS MEASURED AND FOUND WRONG**: inlining MT19937 is worth 6 %, not ~100x, and `Piece Order = Random` stays at 3.06x because VEX's single-thread floor for `init_by_array` is ~1 870 sequential array steps (D260, open, with the number). **THE PYTHON SHARE, re-measured per class (§27.3): straight 2.6 % at 2 km / 0.2 % at 20 km, curved 2.5 % / 0.2 %, hilly 1.9 % / 0.2 %, 300 streets 2.1 %, 1 200 streets 0.5 % — and the only two classes still on the reference are CORNERED (95.2 % / 97.5 %, 4.3 is N8) and CONFORMED (97.1 % / 96.9 %, 4.5 is N6).** Suites from a pristine export: 328 unit tests, `run_native_checks` **144 / 0**, `run_scene_checks` **3 701 / 0, 0 moved**, `run_hda_checks` 31 / 0, `run_2d_checks` 633 / 0, `scale_gate` 0 failing rows, `gate_images` 0, `run_attrib_checks` 0 / 0; one baseline case ADDED (`DM_ripple_deformed`), none removed, none moved, and D210 re-confirmed live (a perturbed value gives 0 failing checks and exit 1). **GATES: PC-G1, PC-G2, PC-G3, PC-G4 RE-CONFIRMED AND LOOKED AT**; PC-G5 still does not pass, unchanged. **Dev-loop rule 0 is NOT discharged** — start the audit at §27.9. — the N-4E entry it replaces follows. **CYCLE N-4E — PART B, 13.9 N5: THE DEFORMED BRANCH (2026-08-24). Read §26 first, then §25, then §24.** Commits `65b9c9f`, `6f4d2f2`, `dc9a68c`, and the write-up commit that follows `dc9a68c` on `polychain` (D227 - a resume pointer may not say "this one", and it cannot carry its own hash either, so it names the three that precede it and how to find the fourth). ⚠️ **A PIECE THE CURVATURE BUDGET UNPACKS IS BUILT NATIVELY NOW**, by `pc_deformed_only` → `pc_deform_prep` → `copy_deformed` (pack 0, transform 0) → `pc_deform`, merged with the packed prims and sorted back into `place.build`'s own job order. **THE NUMBER, per class**: 300 gently HILLY streets — the citygen shape — went **1 237 ms of Python (98.7 %) → 2.3 ms (0.7 %)**, a 2 km ripple **173 ms (96.1 %) → 2.4 ms (2.0 %)**, a 2 km ramp **158 ms → 2.2 ms**, a 20 km ripple **0.2 %**; and the three rows that flipped sides in `output_guard_cost` are not merely equal to the reference but **FASTER — 0.75–0.78x**. `output_guard_parity` **18 native / 74 reference → 28 / 64, identical**. ⚠️ **THE MEASUREMENT RE-ORDERED THE BRIEF A SECOND TIME, AND THE DEPENDENCY DID IT.** Conform is the largest refused class by 4.5x (300 conformed streets **3 645 ms**, the curved variant **4 225 ms**, against cornered's **806 ms**) — but the reference's own report says a conformed street network is **80–100 % DEFORMED**, so N6 alone would have planned those builds and had level 2 refuse them anyway; the only conform row a conform-only port could ship is `conformed_2km` at 196 ms. **N5 is the keystone and it went first.** ⚠️ **THE TWO CONFORMED ROWS DOUBLED WHEN THE FIXTURE WAS FIXED** — the first pass ran 300 streets 1 600 × 1 000 m across over `conform_bench.terrain`'s 660 × 360 m default, so most of the run was over NOTHING and the row measured D53's miss path. ⚠️ **LEVEL 1's DEFORM BOUND IS DELETED, NOT LEFT EVALUATING INTO NOTHING** — it predicted a refusal that no longer exists, so `_bend_bound`, its two `pc_cfg` operands, `bend_bound_ok` and the whole-curve kink walk are gone and `pc_envelope` measures 0.0595–0.0602 µs/piece against 0.47–0.50 (ceiling 1.28 → 0.21). What replaces it is **THREE PER-PIECE REFUSALS** in `pc_deform_gate`, each explicit: a SLICED piece (N7), a module whose piece-local index would overflow the order key, and **a span D31's frame TRANSPORT could flip on** — `_transport` is a PREFIX SCAN a per-point wrangle cannot evaluate, so the gate asks whether the flip is REACHABLE and refuses the piece where it is. ⚠️ **TWO SHIPPED DIVERGENCES THE BRANCH EXPOSED, both 100 % of their build and both invisible before it**: `pc_warn_collate` counted warnings **PER PRIM** where `place._collate_warnings` counts per ELEMENT (`J_coarse_bend` read `pc_warn_bend_resolution:6` against the reference's `:1` — six prims, one element), and `pc_finalize` stamped **`pc_deformed = 0` as a CONSTANT**. ⚠️ **THE OUTPUT'S ORDER IS A NUMBER**: `place.build` interleaves one packed prim per rigid piece with the module's whole polygon soup per deformed one, and the parity check compares every prim IN ORDER plus the entire `P` array, so the two streams are sorted back together on `_pkey0 * PC_PIECE_SPAN + <index within the piece>` — the index coming free off the KIT through `copytopoints`, and the gate REFUSING the build rather than shipping a permuted fence when it would overflow. Probed: `sort` by attribute is exact at 3.07e8 over 300 000 points; a float32 read would have quantised the key at 1.7e7 silently. Probed: **`copytopoints` with `transform = 0` STILL TRANSLATES**, and **a target attribute named in TWO `targetattribs` entries is applied by the LAST one only** (no prim attribute at all, no warning, every key 0). ⚠️ **13.9 N6's DECIDING EXPERIMENT IS COMMITTED AND IT SAYS YES**: `conform_drop_is_portable_to_vex` — VEX's own `intersect()` against `conform.Surface.drop` over 200 queries on five surfaces including a REVERSED winding and a 20 km ramp, **0 hit-flag mismatches, worst 1.776e-15 m**. Its first reading said NO (3.815e-07 m at fixture scale, 9.375e-04 m at 20 km) and the whole divergence is in x/z, the components a −Y drop never moves: VEX hands back the query's DOUBLE coordinates and `hou.Geometry.intersect` hands back float32 ones, so **the REFERENCE is the quantised one** and it disappears in the float32 `P` storage the output is compared in. **ELEVEN SOURCE MUTATIONS with the .hda rebuilt: **TWO SURVIVED and both are one hole** - dropping the within-piece term from either half of the order key left the suite 136 `[PASS]` / 0, because Houdini's `sort` happens to be STABLE and equal keys kept the order `copytopoints` emitted them in. The term exists to make the key TOTAL, so the answer does not depend on how ties break; `piece_order_key_is_total` asserts that directly (0 duplicate prim keys and 0 duplicate point keys out of 1 052 / 1 112, on a build whose largest deformed piece contributes 34 prims) and **both mutations are caught by it now. No survivors remain.** ⚠️ **AND THE RESTORED .hda's HASH MOVED, which is a third finding**: two consecutive rebuilds of an UNCHANGED tree produce different bytes, while the two assets are structurally identical (61 nodes, every type, every input wire, every parameter raw value, compared through HOM). **A hash is not a valid A/B oracle for this asset** - D240.** Suites: `run_native_checks` **137 `[PASS]` / 0** (was 130), `run_scene_checks` 0 failing / **0 moved**, `run_2d_checks` 0 / **0 moved**, `run_hda_checks` 0, `scale_gate` 9 rows / 0, `gate_images` 0, `run_attrib_checks` 0 / 0, **328 unit tests OK**, and **no baselined value moved**. **Dev-loop rule 0 is NOT discharged** — start the audit at §26.12. |
| Native rebuild (§13/§15–§27) | **⚠️ §27 IS THE NEWEST ENTRY (cycle N-4F, 2026-08-24) — read §27 first, then §26, then §25, then §24.** §27 is the response to an independent audit of §26: twenty findings, every one reproduced from a pristine export and closed as a standing check. It closes **four shipped parity divergences on the DEFAULT path** (an `attr:` subject at an ARRAY or DICT storage, a `markerData:` slot that is not a scalar, a marker key JSON escapes, and `pc_row_warns` — which made the reference RAISE), **two more that only the repaired oracle could see** (every int of 3.4's stamp at int64 against the reference's int32, on 29 of 93 cases, plus the cast-order rule that makes the fix complete), **a 118x cost regression on a MAIN-page parm** (`pc_fill`'s `export` arrays copied per gap), **eight checks that could not fail**, and **four stale declarations**. It also MEASURES one finding's proposed fix and reports it as worth 6 % rather than the ~100x the finding implied (D260, open). What follows is §26's entry, unaltered. **⚠️ §26 IS THE PREVIOUS ENTRY (cycle N-4E, PART B, 2026-08-24) — read §26 first, then §25, then §24, then §23.** §26 is 13.9 **N5, the deformed branch**: a piece the curvature budget unpacks is BUILT natively now, so the elevated and tightly-curved classes left Python — 300 gently hilly streets went **1 237 ms of Python (98.7 %) to 2.3 ms (0.7 %)**, and the three `output_guard_cost` rows that flipped sides are **faster** than the reference (0.75–0.78x), not merely equal. `output_guard_parity` 18 native / 74 reference → **28 / 64, identical**. Level 1's deform bound is **deleted** (it predicted a refusal that no longer exists) and replaced by three explicit PER-PIECE refusals in `pc_deform_gate`, of which D31's frame TRANSPORT is the one an artist can reach. It exposed **two shipped divergences** that were invisible while every deformed build took the reference — a warning COUNT that was per prim where the reference counts per element, and `pc_deformed` stamped as a constant — and it commits **13.9 N6's deciding experiment**: VEX's `intersect()` reproduces `conform.Surface.drop` to 1.776e-15 m in float32 storage, so the conform port is a PORT. ⚠️ **The measurement re-ordered the brief again, and the DEPENDENCY did it**: conform is the largest refused class by 4.5x (3 645–4 225 ms) but a conformed street network is 80–100 % DEFORMED, so N6 alone could only have shipped `conformed_2km` at 196 ms — N5 was the keystone. ⚠️ **What follows is §24's paragraph, not §25's** - cycle N-4D changed no geometry and added none here, and §25 is reached through the *Last completed* row above. **⚠️ §24 IS THE NEWEST ENTRY (cycle N-4C, 2026-08-24) — read §24 first, then §23, then §22, then §21.** §24 is the response to an independent audit of §23: twenty findings, nineteen reproduced and one disproved. It closes **four shipped parity divergences** (an artist attribute at a storage the VEX chain cannot read — int `edge_id`/`pc_curve_id`/`pc_style`/`pc_yclass`, string `pc_marker_id` — each admitted by the guard and each shipping a different fence), **a 2.35x cost regression on the most ordinary input there is** (your own kit, the slot parms left alone), a **fail-open in `_bend_bound`**, a warning raised **twice**, **three checks that could not fail**, and **four shipped comments describing a build that no longer existed**. It reproduces and MEASURES `pc_plan_solve`'s per-section parallelism (30x on one long curve) without fixing it — D222 — and DISPROVES the claim that two thirds of that node is its string columns. What follows is §23's entry, unaltered. **⚠️ §23 IS THE NEWEST ENTRY (cycle N-4B, PART B, 2026-08-24) — read §23 first, then §22, then §21, then §20.** §23 widened the guard: three build classes (CURVED, MARKED, PADDED) flipped to the native chain at 0 % Python, `output_guard_parity` went 9 native / 83 reference to **18 / 74, identical**, and conformed / elevated / cornered builds still take the reference by design. It also DECLINED D154 with the measurement that declines it, and closed three mutation survivors plus one flaky check. What follows is §22's entry, unaltered. **§22 (cycle N-4A, 2026-08-23) SUPERSEDES THE “Next up” LINE IN §21.9.** §22 closes all five of §21.9's evidence defects (D206–D210); the tool's geometry, kernel and VEX are UNCHANGED by it and no baselined value moved. What follows is §21's entry, unaltered. **CYCLE N-3V2 — INDEPENDENT AUDIT OF N-3V, BY AN AGENT THAT WROTE NONE OF IT. DEV-LOOP RULE 0 IS DISCHARGED FOR N-3V. Read §21 before touching anything, then §20.** Everything was re-run from a pristine `git archive HEAD` export, so the concurrent phase-2 agent's seven uncommitted files could not colour it. **§20's 13 fixes all reproduce and its headline number reproduces on a SECOND INSTRUMENT** (`hou.perfMon` per-node self time, not a stopwatch): the shipped default measures **3.6 % Python at 2 km and 0.4 % at 20 km** against §18.2's pre-flip 88.2 % / 95.3 %, and **88-98 % on a build the guard refuses**. ⚠ **TWO CORRECTIONS TO §20's TABLE**: `config` is not the only Python SOP that cooks - `kit_starter` cooks on every COLD build at 1.6-1.9 ms, and it is geometry construction in Python, which is exactly what **D154** says must become native `box` SOPs; and **87 % of the shipped native cook at 20 km is ONE WRANGLE**, `pc_plan_solve` (679 ms of 780). ⚠ **TEN SOURCE-LEVEL MUTATIONS - each edits a source file and REBUILDS the .hda, which moves the asset and its declaration together - AND TWO SURVIVED (§21.4).** **SURVIVOR 1: `pc_finalize` DE-BATCHED** into a single-threaded detail loop ships an identical fence at **2.15 → 14.56 ms (+577 %)** with the suite 105 `[PASS]` / 0 - D193's defect one node to the left, because the per-node cost ceilings cover **4 of the 18 wrangles** and the other fourteen have no cost assertion of any kind (D206). **SURVIVOR 2: §20.5's OWN FIX IS UNDOABLE IN SILENCE** - re-wording the two "via the PYTHON BRIDGE" Stage labels back to "NATIVE" ships a menu with two entries labelled Plan NATIVE and two labelled Frames NATIVE, one of each serving the dead bridge, suite green: `stage_menu_reaches_every_stage` asserts a label is non-empty, never that it is TRUE (D207). **And §20.1's headline mutation is stopped by an assertion CRASH, not by the check credited with it** - `every_stage_entry_serves_the_node_it_names` reads the very table the mutation edits, so it passes on both declaration mutations; only `NATIVE_STAGES`' independent second declaration caught the plan row, and nothing independent covers the `reference` row (D208). Two more: `output_guard_cost` **failed once in three runs of an UNMUTATED build** (1.31x on a 5 ms node against a 1.15x ceiling - D209), and `run_scene_checks`' baseline **prints a moved value and still exits 0** (D210). Killed clean: the flip's undo, 32-bit `pc_arclength`, the corner bisector (51 red in `run_scene_checks`), the identity-permutation fast path, `_cond_columns`' revert, and the `_*` sweep. **GATES: PC-G1, PC-G2, PC-G3, PC-G4 RE-CONFIRMED AND LOOKED AT** on images whose drawn-segment counts were checked against the geometry (D194) - segment/polygon 4.0-4.4 throughout; **PC-G5 still does not pass**, unchanged, for P2-3V's two reasons. Suites from clean: **346 unit tests** (not 328 or 350), `run_native_checks` **105 / 0**, `run_scene_checks` **3 665 / 0**, `run_hda_checks` **29 / 0**, `run_2d_checks` **528 / 0**, `scale_gate` 9 rows / 0, `gate_images` 0, `run_attrib_checks` 0 / 0 unreviewed, **no baselined value moved and citygen untouched**. ⚠ `devScripts/create_pf_polychain_hda.py` defaults `POLYFACTORY` to a HARDCODED `F:/projects/polyfactory`, so running it from a copy overwrites the REAL repo's .hda (§21.7). — the N-3V entry it replaces follows. **CYCLE N-3V — THE AUDIT RESPONSE. `Stage = output` IS THE NATIVE CHAIN, IT NOW SHIPS THE SAME FENCE AS THE REFERENCE, AND THE FLIP CAN NO LONGER BE UNDONE SILENTLY. Read §20 before touching anything.** Three independent reviewers returned 13 findings; all 13 are reproduced and closed, each as a standing check with a mutation that reddens. **TWO SHIPPED PARITY DIVERGENCES (§20.2), both 100 % of the run, both on builds the guard ADMITS with no warning**: a `pc_cond` list of 2, 3 or 4 numbers comes back out of `style.read` as a `hou.Vector2/3/4`, which `_cond_columns` called unreadable — 10 `panel` prims natively against the reference's 12 `gate`, and lengths 1/5/6 agreed, so nothing in the suite (which never round-tripped a cond value through the payload) could see it; and an `attr:<name>` subject that is a VECTOR was read as COMPONENT 0 in VEX where the reference gets the whole tuple — 12 `gate` against 10 `panel` on `gt`, the other way on `lt`. Both fixed by giving `pc_cond_subject` Python's THIRD answer (`PC_S_UNREADABLE`: `ne` is True, everything else False), which also closes `ctx_base`'s dict-valued `attrs`/`marker_data`; COND_BAD now makes `_native_ok` REFUSE rather than silently answer False. **FIVE MUTATIONS OF THE SHIPPED ASSET THAT 94 PASSES COULD NOT SEE (§20.1)** — including pointing `Stage = reference` at `OUT_final`, which turned the cycle's headline parity proof into a comparison of the output WITH ITSELF over all 92 cases and stayed green. Root cause: `STAGES` claimed three consumers read it and **no check read it at all**. It is `tests/polychain/native.STAGES` now, (token, null, FEEDER, label), read by the build script and by `every_stage_entry_serves_the_node_it_names`; `asset_stages_match_the_rig` compares 27 INPUT WIRES as well as parameters; `output_guard_parity` counts `copy_packed`'s cookCount instead of believing the envelope; `NATIVE_STAGES` gained `frames_native` and `output` (17 nodes / 3 stages → 32 / 5). **`bench_guard_fallback` existed only in two source comments** — it is written (1.47x at 2 km, 1.8x ceiling) with `no_case_pays_the_guard_fallback` (0 of 92) beside it. **`pc_plan_solve` finally has the ceiling D193's mandate named** — 34 µs/piece, measuring 24.6-25.0, min over three INTERLEAVED repetitions because a single best-of-3 pass reads 26-42 on an unchanged build (D204); the identity-permutation skip took another 13-18 % off it and `bench_plan_long_curve` went **0.4369 → 0.3510 s**. The sticky note and the build script's docstring said the opposite of the build and are rewritten with a check (§20.5). **⚠ THE NUMBER, re-measured: 2.5 % Python at 2 km and 0.2 % at 20 km on an admitted build (`config` alone, a FIXED 1.7 ms), against 84-99 % on a refused one. 9 of 92 cases are inside the envelope — unchanged, and the COND_BAD refusal did not narrow it.** The asset is **51 nodes**, 18 wrangles all at `vex_precision = 64`, 4 Python SOPs, 14 nulls, 3 switches, 10 Stage entries. Suites: **`run_native_checks` 105 `[PASS]` / 0 failing** (was 94/0), `run_scene_checks` 0, `run_hda_checks` 0, `scale_gate` 9 rows / 0, `gate_images` 0, `run_2d_checks` 0, `run_attrib_checks` 0 failing / 0 unreviewed, 328 unit tests OK, **and no baselined value moved**. **Dev-loop rule 0 is NOT discharged for this cycle** — start the audit at §20.7. |
| Next up (native lane) | ⚠️ **CHECK THIS ROW AGAINST `git log --oneline -25` BEFORE STARTING.** ⚠️ **THE ORDER IS MEASURED AND §27.3 IS THE MEASUREMENT: exactly TWO classes still take the reference.** **(1) N6, CONFORM — 96.9–97.1 % Python, 2 142 ms on a 20 km conformed run and 208 ms at 2 km**, unblocked since N5 builds the 80–100 % of it that deforms, and its deciding experiment is committed and green with the RAW double difference on the record now as well as the f32 one (4.933e-08 relative, two float32 ULP — `intersect()` IS a float32 ray test, D247). **(2) N8, 4.3 CORNERS — 95.2–97.5 % Python**, 551 ms on a 20 km L, and still the hardest. **(3) D260 — `Piece Order = Random` is 3.06x the reference and CANNOT be made 1.6x in VEX**: `random.Random(s).random()` is ~1 870 strictly sequential array steps and VEX's single-thread floor is ~24 ns a step (measured: a bare 1 872-iteration scalar loop is 45 us, the whole draw 98 us, CPython 1–2 us). The fix is a TWO-PASS solve that computes the draws in a parallel wrangle; until then the row ships with a 4.0x ceiling and `output_guard_cost`'s `random_2km` holds it. **(4) `pc_order` is 25–30 % of a citygen cook** (34.3 ms of 138.2 on 300 streets, 140.6 of 463.0 on 1 200) and `pc_plan_solve` is 6 % there against 85 % on ONE long curve — D261, and read both numbers before spending a cycle on either. ⚠️ **A NEW OUTPUT ATTRIBUTE NEEDS A LINE IN `pc_out_cast`**: the chain runs at 64 and `place.build` declares int32/fpreal32, so an int or a float added to `ELEM_PRIM_ATTRS` without a cast entry ships at twice the reference's storage (D262 — that shipped on six prim ints for a whole cycle). The cast list is EXPLICIT on purpose: a `pc_*` pattern would convert `pc_u` to an integer. ⚠️ **AND THE CAST IS THE LAST NODE OF THE STAGE**, because `pc_warn_collate` creates prim attributes at cook time. ⚠️ **A CHANNEL THE NATIVE CHAIN DOES NOT READ IS A REFUSAL** (D263): `pc_row_warns` makes the reference publish one prim attribute per token, so level 1 refuses a non-empty one. ⚠️ **`export T[]` PARAMETERS ARE COPIED** — never hand an accumulating array to a function called in a loop (D243; 118x). — the previous entry follows, and its wrangle/stage/ceiling rules all still apply. ⚠️ **CHECK THIS ROW AGAINST `git log --oneline -25` BEFORE STARTING — cycle N-4D was handed a brief asking it to close defects §22 had already closed (§25), and cycle N-4E was handed one whose measured order had already been superseded (§26.1).** ⚠️ **THE ORDER IS MEASURED, NOT ASSUMED, AND §26.1's TABLE IS THE MEASUREMENT.** **(1) N6, CONFORM — 3 645 ms on 300 conformed streets and 4 225 ms on the curved variant, the largest refused class by 4.5x, and it is now UNBLOCKED because 13.9 N5 builds the 80–100 % of it that DEFORMS.** ⚠️ **Its deciding experiment is already committed and it says YES**: `conform_drop_is_portable_to_vex` reproduces `conform.Surface.drop` to 1.776e-15 m in float32 storage, on five surfaces including a reversed winding and a 20 km ramp — **read the drop off the AXIS COMPONENT and rebuild the position from the query** (`drop_many`'s own reconstruction), because compared as doubles the two disagree by 9.375e-04 m at 20 km and the REFERENCE is the quantised side. What the port still owes beyond the sampler: `ConformPath.sample`'s one-sided finite difference for the tangent (`delta = 1e-3`, and it is one-sided so a corner piece points down one leg rather than averaging two), `normal(s)` for D55's camber, `missed(sa, sb, fracs)` → `pc_warn_conform_miss` on the piece's OWN stations (D71 — `pc_kit_meta` already publishes them), `deviates(sa, sb, tol, fracs)` (which is what unpacks a piece over a hill a dead-straight spline knows nothing about), and `_stepped_base` on the conformed path. **`span_deviation`'s two unported terms — D100's camber rotation and D104's extra stations — are folded in ONLY when `normal_at` is given, i.e. only under `conform_tilt`, so refusing TILT keeps the ported two terms exact and is the cheap way in.** ⚠️ **A WRANGLE HAS FOUR INPUTS AND THE GATE ALREADY USES ALL FOUR**: put IN_SURFACE on `pc_deform_gate`/`pc_frames_native`/`pc_deform` input 3 and move the station table onto the plan point in `pc_proto` (which holds the kit), or the port stalls on wiring. **(2) N8, §4.3 CORNERS — 806 ms on 300 cornered streets, 801 ms on a 20 km L**, and still the hardest: miter/bend, compose symmetry, offset, fillet, degenerate fallbacks; `clip` and `polyfill` are native NODES now, not verbs; only 3.3 % of the class deforms, so N5 does not unblock much of it. **(3) D222 — `pc_plan_solve` parallelises per SECTION**, 80 % of a 20 km native cook, 30x more on one long curve than on the same piece count as 300 short ones; unchanged by this cycle. **(4) D231 — `pc_order` is 125–131 ms** at 17 852 pieces / ~304 000 prims, the second-largest item on a 20 km rippled cook after `pc_plan_solve`. That is the price of putting `place.build`'s job order back together; if a cheaper ordering exists it is worth a cycle, and the sort is CORRECT, which is not negotiable. **The cheapest three remaining refusals**, each measured on a 2 km run: `show_warnings` (54 ms — `colour_warnings`' `Cd`, one wrangle), `display = proxy|plan` (50 ms — D81/D82), `fill = tile` (58 ms — N7's slice caps, and `pc_deform_gate` already refuses a sliced piece explicitly). ⚠️ **THE GUARD MUST FAIL SAFE**: when the native chain cannot answer a build it takes the REFERENCE — never "evaluate False and ship", which was the root cause of both of §20.2's criticals and of §24's four storage divergences. ⚠️ **Every new wrangle needs a row in `WRANGLE_CEILING_US` OR in `WRANGLE_DEFORM_CEILING_US`** (D206 — and the two tables are split by SHAPE, not by taste: five wrangles never cook on a straight run and four more see the module's whole polygon soup on a deformed one), **a `.vfl` header naming its class, precision and inputs** (D207), and **a new stage needs a `BRIDGE_STAGES` or `NATIVE_STAGES` row** (D208) — on top of `vex_precision = 64`, the `_*` law, and `native.STAGES`' FEEDER column. ⚠️ **D216 IS STILL THE SHAPE OF EVERY WIDENING**: the exact test lives at level 2, the cheap test at level 1 may be approximate IN A NAMED DIRECTION, and the cost of being wrong is measured (`bench_guard_fallback` 1.8x; the shipped build reaches 1.50–1.62x). ⚠️ A new `pc_warn_*` must be written with `setpointattrib`, not bound with `i@`, or it is published on every element whether it fired or not (§19.2). ⚠️ **AND A COUNT IS PER ELEMENT WHILE A STAMP IS PER PRIM** — those were the same number until N5 and they are not any more (§26.4). |
...
(procedurally round the path first). **The corner assembly IS the pillar at the vertex** — the default
fill must not supply a second one (D266/D267, §28). Neighboring default pieces slice against the same plane
...
⚠️ **The default COMPOSITION is D266 (§28): `Repeating Pieces = panel`, `Evenly Spaced Piece = post`,
`Evenly Spacing = 2 m`** — RailClone's canonical fence. A composed fill (`post panel`) doubles the
pillar at every corner and every run cap, because an alternating unit of even period opens and
closes with different members; the rhythm element therefore lives in a slot whose anchors are
clear of the run's ends by construction. Do not "simplify" it back — `double_pillar_m` is the
assertion.
...
## Cycle N-1V — the independent audit of the native rebuild (2026-08-22)

**A fresh agent that wrote none of §15 ran every suite from clean, re-derived the baseline, ran
10 mutations and judged all four gates.** The full write-up is [§15.8](#158-the-audit--cycle-n-1v-by-an-agent-that-built-none-of-it);
this entry is the §10 index line so the phase-1 log does not go quiet on it.

**The verdict:** the rebuild is real, and the one claim it all hangs on survived the one mutation
that could have exposed it as cosmetic. Forcing `place._native_tables` to return `None` — a silent
fallback to the Python reference on every curve — turns `native_branch_is_load_bearing` **RED**, and
so does rewiring `kernel` back to `IN_SPLINE`. The DECOMPOSE box is on the artist's cook path and
carries the geometry.

**Two checks were pretending, and the cause was structural.** `native.py`'s `stage_decompose` and
`create_pf_polychain_hda.py`'s `DECOMPOSE` tuple are two independent declarations of one chain
sharing only their `.vfl` bodies — so `mutation_pc_unshare` bypasses the **rig's** copy, and
**bypassing `pc_unshare` in the shipped HDA left all five suites green.** Bypassing
`pc_frames_valid` was invisible the same way, and a 32-bit `vex_precision` on the asset's
`pc_arclength` passed `native_intermediates_are_64bit` *while it printed "32-bit wrangles: none"*.
Closed by **`asset_decompose_matches_the_rig`**, which reads node type, `class`, `vex_precision`,
snippet text and every bypassed node **off the built asset**; it reddens on all three.
§15.1's sentence *"the network the checks measure IS the network the asset ships"* was false and is
corrected in place.

**Said plainly, because the write-up did not:** `pc_frames` is **behind the Stage switch and
reaches no artist**. Breaking its 3×3 by 1e-7 reddens exactly one rig-side parity check and nothing
else. **One of five kernel stages (§4.1 decompose) is native and shipping; `kernel` still builds
every fence.**

**Gates:** PC-G1 ✅ (both corner modes through the parm face, 40 checks, images looked at) ·
PC-G2 ✅ (four modes, conform ON, judged on `VG2_vertical_side.png` / `VG2_adaptive_side.png`) ·
PC-G3 ✅ (9 rows both z-modes, mutation-proved at 50× the budget) · PC-G4 ✅ (40-parm sweep,
mutation-proved — reverting D91 reports `moved: padding`). All eleven gate PNGs regenerated from
this build are **byte-identical** to the committed ones. All four still owe the GUI viewport pass.

**Suites:** `run_native_checks` **32/32** · `run_scene_checks` 3 665 assertions over 89 cases,
**not one of 5 559 baselined values moved** · `run_hda_checks` 29/29 · `scale_gate` 9 rows ·
`run_2d_checks` 633 · `tests/hda/run_attrib_checks.py` no baseline change · **346 unit tests**.
citygen untouched, and it imports nothing from `polychain`.
...
`1c144b4` the network, `0a2b741` the parity rig, `22d17c9` the audit response (§15.7).
...
```
pf_polychain                      [4 inputs, contents LOCKED on every new instance]
  IN_SPLINE     [null]  <- input 0
  IN_KIT        [null]  <- input 1
  IN_STYLE      [null]  <- input 2
  IN_SURFACE    [null]  <- input 3
  +-- 0 - CONFIG (13.3.0)
      config           [python]         <- IN_KIT on 1, IN_STYLE on 2
  +-- 1 - DECOMPOSE (4.1) - VEX + native, and ON THE SHIPPED COOK PATH
      pc_unshare       [splitpoints]              <- IN_SPLINE
      pc_curveid       [attribwrangle prim   64]  <- pc_unshare
      pc_curve_index   [attribwrangle detail 64]  <- pc_curveid, config
  ...
  +-- 2 - PLAN (4.2) - still Python, and 13.9 N2 is where it goes
      pc_plan_bridge   [python]         <- OUT_sections, IN_KIT, IN_STYLE, IN_SURFACE
      OUT_plan         [null]
  +-- 4 - PLACE + DEFORM (4.4) - the frame, in VEX
      pc_frames        [attribwrangle point 64]  <- OUT_plan, config, OUT_sections
      pc_frames_valid  [blast]
      OUT_frames       [null]
  +-- R - THE REFERENCE (13.6) - the oracle, AND the shipped output
      kernel           [python]         <- OUT_sections, IN_KIT, IN_STYLE, IN_SURFACE
      OUT_reference    [null]
...
`DisplayComment` sentence saying what it computes, plus one native `splitpoints`. Five titled
network boxes. The VEX bodies are real files at `polyfactory/vex/polychain/*.vfl` (`pc_path.h`,
`pc_arclength`, `pc_curveid`, `pc_curve_index`, `pc_corners`, `pc_markers`, `pc_frames`),
**inlined** into the snippet parms at build time by `polychain.vexsrc` so the asset carries no
`HOUDINI_VEX_PATH` dependency.

**⚠️ THE ONE WIRE THAT CHANGES WHAT THIS SECTION MEANS.** `kernel`'s input 0 is `OUT_sections`, not
`IN_SPLINE` (D166). Until an audit pointed at it, it was `IN_SPLINE` — and the consequence, which
this section used to describe as a feature, was that **the entire DECOMPOSE box sat beside the tool
rather than inside it**. Measured on that build with a 4-point spline and the starter kit: all six
new nodes reported `cookCount == 0` after an Output cook; bypassing all six left the output hash
byte-identical; **destroying** all six left it byte-identical. `sop_cooks_per_build` printed
`0/20 — native nodes that cooked on the Output stage: none` and that was read as "the rebuild is
free" when what it actually said was "the rebuild has not happened". An artist toggling
`pc_corners` off saw nothing change, because the geometry never passed through it.

Behind `OUT_sections`, `place.read_curves` picks up `pc_s`, `pc_cleanidx` and the corners the
64-bit wrangles computed, and `decompose._clean` / `decompose.resolve_corners` return them instead
of re-deriving them (D166). So the 89 scene cases, the HDA gate and the scale gate now exercise the
VEX rather than walking past it. `Stage` still defaults to `output` and a node nobody has touched
still builds exactly what it built before the rebuild — but now that is a statement about the
*answer*, not about which code produced it.
...
⚠️ **THIS PARAGRAPH USED TO SAY "the HDA build script calls the same helpers, so the network the
checks measure IS the network the asset ships". IT WAS FALSE AND AN AUDIT PROVED IT** (§15.8).
`tests/polychain/native.py`'s `stage_decompose` and `create_pf_polychain_hda.py`'s `DECOMPOSE`
tuple are **two independent declarations of one chain**; they share only the `.vfl` bodies that
`polychain.vexsrc` inlines. Bypassing `pc_unshare` in the *shipped asset* left every suite green,
because `mutation_pc_unshare` bypasses the *rig's* copy. `asset_decompose_matches_the_rig` is what
makes the sentence true now: it reads node type, `class`, `vex_precision`, the snippet text and
every bypassed node back off the built asset and diffs them against the rig.

The case set is **92**: `cases.build_all()`'s 89, plus `cases.topology_cases()`' three, which exist
because **0 of the 89 has a point in more than one primitive** and citygen's own pipeline ends in
`graph_fuse` — so the target input was a shape the parity rig structurally could not contain.

| Check | Measured | Ceiling |
|---|---|---|
| `native_id_and_curve_set_parity` | the VEX id rule AND the curve SET agree with `place.read_curves` on **every prim of 94 curves + 1 declined prim** | 0 mismatches |
| `decompose_arclength_parity` | **0.000e+00 m** worst \|Δ`pc_s`\|, 94 curves, all 92 cases | exact |
| `decompose_length_parity` | **0.000e+00 m** worst \|Δ curve length\| | exact |
| `decompose_corner_parity` | **0** mismatches over **55 corners** — identical sets, identical `forced`/`degenerate` | 0 |
| `decompose_turn_parity` | **2.842e-14 deg** worst \|Δ turn angle\| | 1e-10 (it is `acos` ULP, not zero) |
| `decompose_marker_parity` | **0.000e+00 m** over 5 markers on 94 prims, 2 of them under a duplicated id | exact |
| `frames_arithmetic_linear_parity` | **2.220e-16 relative** worst \|Δ 3×3\| over **1 733 real `_packed_transform` calls**, z-modes adaptive / vertical / stepped | 4.5e-16 (2 float64 ULP — it is FMA) |
| `frames_arithmetic_position_parity` | **2 of 5 199** P components not bit-identical to `f32(reference)`, worst **1.00 float32 ULP** | 1.0 ULP |
| `plan_span_transport_at_20km` | **2.910e-11 m** worst \|transported s − reference s\| over 18 870 pieces, read back off `Stage = plan` | 1e-4 m (`checks.TOL_M`) |
| `config_reads_the_payload` | `pc_cfg` on the BUILT asset == `style.read(payload).params` (77.0° / 3.0°) | 0 disagreeing keys |
| `payload_thresholds_reach_the_corners` | **1** corner break at 77°, where the parm page's 30° gives 2 | == 1 |
...
* **`native_id_parity` compared two copies of one rule.** It asked `hda.curve_prim_index`, which
  re-implemented D29/D64 and applied *none* of `read_curves`' filters — packed prims, prims with
  fewer than two points, prims containing a marker point, duplicate ids. So the two sides could
  agree with each other while both disagreed with the curve set the builder actually plans on.
  Measured: a 3-point polyline whose middle point carries `pc_marker` gave the reference **0
  curves** and the VEX a full sampler table plus a marker at s = 0 on a curve that does not exist.
  `curve_prim_index` now *reads* `read_curves`, so there is one rule, and the check asks the
  builder's own answer prim by prim including the prims it declined (D167).
* **`frames_*_parity` rounded the span to float32 on both sides**, which excluded the one thing the
  network genuinely does differently from the reference — carrying the span through a point
  attribute. They are renamed `frames_arithmetic_*` so the scope is on their face, and the
  transport is measured, unrounded, by `plan_span_transport_at_20km`.

**Why `frames_arithmetic_*` still rounds both sides, and why its ceiling moved from 0.0.** The rig
carries the plan to the wrangle through point attributes, which are float32 storage; asking the
reference in float64 and the wrangle in float32 would measure the transport instead of the maths.
Both sides therefore go through D170's *real* transport — the float32 head plus the float32
residual — which is what the asset does. That pair carries ~48 significant bits where the single
float32 carried 24, and the extra bits un-mask the difference the position check has always named:
**VEX fuses `a + d * t` into an FMA and Python does not**, so the sampled position can round one
float64 ULP apart and the frame built on it inherits that. 2.220e-16 relative is exactly 1 ULP at
1.0. Trading 1e-16 of agreement here for 9.765e-4 m of agreement at 20 km is the point of D170.
...
the 4.5e-16 FMA floor); removing `pc_arclength`'s coincident-vertex merge moves the cleaned index
table from `[0, 1, -1, 2]` to `[0, 1, 2, 3]`; **bypassing `pc_unshare` on a fused junction loses the
corner the reference finds** (1 → 0). The `pc_arclength` mutation asserts its own target line still
exists, so a future edit cannot silently turn the mutation into a no-op.

### 15.2 §13 was wrong about the speed — and so was §15.2's first answer

**§13.2 recorded "cumulative arclength … 0.0037 s (64-bit) on a 20 001-vertex 20 km line" against
the reference's 0.030 s. That is not a comparable number.** §13.2's probe timed a wrangle that
wrote **one point attribute**; the shipped stage additionally builds the 20 000-element sampler
table that `pc_frames` reads.

**And the correction this section first published was itself measured against the wrong
comparand.** It timed the reference as `_clean` + `resolve_corners` + `resolve_markers` with
`P.read_curves` hoisted *out* of the timed loop — but `read_curves` is the geometry read the
reference cannot avoid to reach those answers, and the native chain does not need it. Measured:
`read_curves` is **48 %** of the reference's real cost on the 20 km curve and **79 %** on 300
streets. So the ratio is a **range**, both ends are printed on every run, and neither end is the
number on its own:
...
essentially unchanged (0.0442 s → 0.0428 s). So `append`'s reallocation was real but small: **the
table itself is the cost**, and it is inherent to the sampler design, not to the way it was written.
That pair of numbers is now in the .vfl header, where a stale 8× speedup claim used to be.

**R7, restated honestly.** The per-node fixed cost is *not* what hurts: 300 short curves through six
nodes cost 0.0013 s and beat the reference on both readings. What is expensive is **one very long
curve**, where the stage is sequential by nature and materialises a 20 000-element table the
reference never builds. That is the opposite of what §13.9's R7 predicted, and it is worth knowing
before N2 designs its own per-section arrays.

The bench's ceiling is deliberately "within 1.5× of the lower bound", not "faster", and both ratios
print on every run. **A check that certified a win nobody measured would be worse than no check.**
...
### 15.3 The load-bearing tripwire — inverted, and why

This section used to describe `sop_cooks_per_build`: *on the `output` stage, 0 of the native nodes
cook; set `Stage = sections` and 5 of them do; a Switch SOP cooks only its selected input, so the
rebuild is free until you look at it.* **Every clause of that was true and the conclusion was
wrong.** The nodes cost nothing on the Output stage because the Output stage did not go through
them — `kernel` took `IN_SPLINE`. The check was certifying that the port had not happened.

Since D166 the DECOMPOSE box is upstream of `kernel`, and the two checks that replaced it say the
opposite thing on purpose:

* **`native_branch_cooks_on_output`** — the seven on-path nodes (`config` and the six DECOMPOSE
  nodes) must ALL cook on a plain Output cook, measured without `force` by reading `.geometry()`
  the way an artist's cook does; and the three debug-branch nodes (`pc_plan_bridge`, `pc_frames`,
  `pc_frames_valid`) must all stay at zero, because they sit behind the Stage switch.
* **`native_branch_is_load_bearing`** — corrupt `pc_arclength` on a live instance so every metre
  moves 0.25 m, and the **shipped output** must move. This is deliberately *not* "bypassing the
  branch changes the output": D166's fallback means a curve with no native tables is answered by
  the Python, so a bypass is byte-identical **by design**. The only honest proof that the VEX
  answer reaches the artist is to corrupt it and watch the geometry follow.

The cost the rebuild now does carry, stated rather than hidden: an Output cook pays for the whole
DECOMPOSE box, including the `pc_seg_*` sampler table that only the Frames stage reads. On the
citygen shape that is a win on any reading of §15.2's range; on a single 20 km curve it is roughly a
wash. The scale gate is unmoved.

### 15.4 Three Houdini behaviours found this cycle — and one of them was a misdiagnosis

**T1 — A LOCKED HDA LOADS ITS CONTENTS LAZILY. That is all the empty `networkBoxes()` ever meant.**
This entry used to read *"`defn.save(template_node=…)` and `updateFromNode()` BOTH drop every
network box and sticky note unless the options carry `setUnlockNewInstances(True)`"*, and that
misdiagnosis is why the asset shipped unlocked. Control test, one throwaway two-node asset with one
box and one sticky, saved twice with the flag as the only difference:

| | fresh instance | after one `n.node("OUT")` | `cook(force=True)` | `matchesCurrentDefinition()` |
|---|---|---|---|---|
| locked | children 0, boxes 0, notes 0 | children 2, **boxes 1, notes 1**, comment intact | clean | **True** |
| unlocked | children 2, boxes 1, notes 1 | same | clean | **False** |

Nothing is dropped. `children()`, `networkBoxes()` and `stickyNotes()` are simply empty until
something touches the contents — probe them *after* a child access, not before. T1's second half,
*"force-cooking such an instance then fails with Network box save failed"*, did not reproduce on
either asset; that message belongs to **T2**, which carries the identical error string. So
`artist_ui.md` §6 rule 10's two halves — the graph stays readable, and the asset is editable — are
**not** one switch. Readability is free; editability is the flag.

**And the flag is not free.** Measured at citygen scale, 300 `pf_polychain` nodes in one scene:

| | unlocked | locked |
|---|---|---|
| create 300 instances | 2.056 s | **0.096 s** (21×) |
| child nodes materialised | 6 000 | **0** |
| `hipFile.save` size | 20 710 531 B | **720 008 B** (29×) |
| `matchesCurrentDefinition()` on a fresh instance | **False** | True |

The last row is worse than the other three: an unlocked instance is a private copy from the moment
it is created, so **every scene an artist saves during the rebuild forks the half-built graph and
never receives the rest of it** — with N2, N4/N5, N6, N7 and N8 still ahead. The asset ships locked
(D165's sibling decision, recorded below), `instances_do_not_fork_the_network` keeps it that way,
and an artist who genuinely wants to edit one instance still has Allow Editing of Contents.
...
**T3 — `hou.Geometry.addAttrib` has no precision argument, so a Python SOP cannot create a 64-bit
float attribute.** Probed: the signature is
`addAttrib(type, name, default_value, transform_as_normal=False, create_local_variable=False)` and a
float default gives float32 storage; `20000.0004883` written and read back is `20000.0`. That is
the whole reason D170 exists — the boundary between a Python SOP and a wrangle is 32 bits wide and
...
- **D165 — the DECOMPOSE box starts with a native `splitpoints`, and that is INPUT NORMALISATION,
  not a workaround.** `pc_arclength` keeps a per-(curve, vertex) walk — `pc_s`, `pc_cleanidx`,
  `pc_prev`, `pc_next` — in POINT storage, which is only sound while a point belongs to exactly one
  primitive. citygen's documented pipeline is `graph_resample → graph_stitch → graph_fuse →
  graph_polypath`, so a street network arrives **welded at every junction**, and the dev-loop trap
  list already records that a fused junction point cannot hold the parametrisations of every edge
  sharing it. Measured before the node existed, on two polylines sharing one vertex where A turns
  90°: the reference reports A's corner at s = 10.0, turn 90.000°; the native stage reported **zero
  corners in both primitive orders**, and the junction's `pc_s` flipped 0.000 ↔ 10.000 with the
  order alone. `splitpoints` is Houdini's own answer (procedural-modeling rule 3 — convert the
  input into the space the tool operates in), it leaves prim attributes, detail attributes and
  stand-alone marker points untouched, and it measured 0.00008 s on the 20 001-vertex fixture where
  nothing needs splitting. The alternative — moving the tables to VERTEX attributes — is more
  faithful to what the data is keyed by, and it was declined because a native node is higher on the
  ladder than a rewrite of two wrangles, a group class and the parity rig, and because the
  reference has no concept of a shared point either (`read_curves` builds one Curve per prim from
  that prim's own points). `mutation_pc_unshare` bypasses the node and asserts the corner
  disappears, so the justification is a check rather than a paragraph.
- **D166 — `kernel` is wired BEHIND `OUT_sections`, and the reference reads 4.1's answers off the
  geometry instead of recomputing them.** This is the decision that makes the rebuild real; §15.0
  carries the measurement of what it was before. `place.read_curves` harvests `pc_s`,
  `pc_cleanidx`, `pc_iscorner`, `pc_turn_deg`, `pc_corner_degen` and `pc_corner_forced` as **bulk**
  attribute arrays, slices them per curve, and hands each `Curve` a `native` payload;
  `decompose._clean` and `decompose.resolve_corners` return it. Four properties make it safe:
  1. **It is opt-in by presence.** `pc_nclean` is the sentinel. A raw spline — a check calling
     `build` directly, a node wired straight to a curve SOP, `run_scene_checks` — has none of these
     and every caller falls through to the Python. The reference never stops being runnable alone.
  2. **It falls back on a config mismatch.** `pc_curve_index` stamps `pc_cfg` onto the sections
     stream; `resolve_corners` compares `corner_angle_deg` and `min_included_angle_deg` against the
     `params` it was called with and re-derives in Python if they differ. `analyse()` with explicit
     parameters, or a check sweeping the angle, gets the thresholds it asked for.
  3. **It falls back on a derived curve.** The fillet and the slope flatten build new `Curve`
     objects, which carry no `native`, so a filleted path is decomposed by the Python — which is
     correct, because the native tables describe the *input* spline and a filleted path is a
     different polyline (D160 is the same fact stated for the frames).
  4. **The read is Python and Hannes' rule allows exactly that.** What moved to VEX is the
     per-element geometry work — the cumulative scan, the coincident merge, the turn angle, the
     corner threshold, all over N points. What is left is a handful of bulk attribute reads and one
     slice per curve: marshalling data between a node and a caller. Nothing in it touches a
     `hou.Point` for anything but its number, and the standing wrapper tripwires are unmoved
     (`wrapper_reads_streets` 330 of a 2 000 ceiling).
- **D167 — `pc_curveid` decides the curve SET, not only the curve id, and it decides it the way
  `read_curves` does.** Three kinds of primitive are declined: fewer than two points (which covers
  every packed prim, since a packed prim has exactly one), and any prim containing a point carrying
  `pc_marker`. A declined prim keeps a **blank** `pc_curve_id_r`, which no marker can match, and
  `pc_arclength` returns before writing it any table. Measured before this: a 3-point line whose
  middle point carried `pc_marker` gave `read_curves` **0 curves and 1 marker**, while the VEX
  published `pc_curve_id_r = "M"`, a full sampler table and a marker at s = 0 — a curve the
  reference refuses to build on. `hda.curve_prim_index` is now derived from `read_curves` rather
  than being a second copy of the rule, so there is one rule and the parity check compares against
  it.
- **D168 — `pc_curve_index` publishes a DICT, because it was promising a lookup and doing a scan.**
  Its comment said it existed "so a marker can find its curve with an array lookup instead of a
  scan over the whole stream", and `pc_markers` then ran `for (i…) if (ids[i] == cid)` — an
  O(nprimitives) string compare per marker, 300 × 300 = 90 000 comparisons on the citygen shape.
  It now writes `pc_curve_prim`, a VEX dict keyed by id — the same mechanism `pc_cfg` already uses.
  The stored value is **prim + 1**, because a dict read of an absent key answers 0 and "prim 0" and
  "no such curve" must not be the same number. The string array stays for readability at
  `OUT_sections`.
- **D169 — a DUPLICATED curve id warns rather than quietly answering for one of the curves.**
  `resolve_markers` filters by id, not by prim, so the reference places a marker on **every** curve
  sharing its id: two prims tagged `DUP` (10 m and 40 m) with one `pc_u = 0.5` marker give rows at
  s = 5.0 *and* s = 20.0. A point wrangle can only move the point it is on. `pc_markers` therefore
  binds to the first prim claiming the id and sets `pc_warn_marker_dup`; `decompose_marker_parity`
  asserts exactly that — the first prim keeps the reference's row, the others get none, and the
  warning must be there. D74 already warns about the duplicate id itself. `pc_curveprim` is
  initialised to **-1** on every marker for the same class of reason as D168's `+1`: the attribute
  default is 0, and 0 is a real primitive.
- **D170 — the fitted span crosses to `pc_frames` as a float32 HEAD plus a float32 RESIDUAL.**
  T3 above: `hou.Geometry` has no 64-bit float storage, so `plan_geometry`'s `pc_s0r`/`pc_s1r`
  quantised at 1.95 mm at 20 km. Measured on `cases.arc_points(20000, 1, 20000)` through the built
  asset: worst \|stored − reference\| **9.762e-04 m**, ten times `checks.TOL_M`, with every stored
  value equal to its own float32 round-trip and 18 868 of 18 870 reference values not float32-exact.
  Two neighbouring pieces snapped to the same station inside that grid. `plan_geometry` now stores
  `f32(s)` in `pc_s0r` and `f32(s − f32(s))` in `pc_s0r_lo`; `pc_frames` adds the pair in 64-bit
  VEX and gets **2.910e-11 m**. `plan_span_transport_at_20km` reads it back off `Stage = plan` and
  asserts it, and prints the head-alone error beside it so the reason the residual exists is on
  every run. **13.9 N2 deletes both attributes** the day the solve computes the span inside the
  wrangle. The irony worth recording: `native_intermediates_are_64bit` guarded the VEX→VEX boundary
  from the start while the Python→VEX boundary in the shipped wiring was 32 bits wide and unguarded.
- **D171 — `report["frames"]` is OPT-IN, amending D161.** D161 called it "additive and read-only",
  which it is, and did not count it. Measured on the packed 10k fixture: 10 000 dicts of 11 keys,
  **4.96 MB** by recursive `getsizeof` and **0.0031 s of a 0.2239 s build** — 1.4 %, allocated and
  thrown away on every cook of a node nobody has put on the Plan stage. Its only consumers are
  `cook_plan_bridge` and the parity rig, so `build(…, report_frames=True)` asks for it. The
  wall-clock half of that measurement is at the noise floor and only the megabytes and the
  milliseconds are solid — but it is instrumentation riding in the artist's cook and it grows with
  N4/N5.
- **The asset ships LOCKED** (`setUnlockNewInstances(False)`), for the four numbers and the
  non-propagation behaviour in §15.4's T1. Readability does not need the flag; a locked instance
  loads its contents the moment anything touches them.

### 15.6 What is NOT done — the complete Python inventory, not just the stages

**⚠️ THIS LIST USED TO NAME ONLY THE FIVE KERNEL STAGES, and so did the sticky note inside the
asset.** Two pieces of Python geometry construction that run in the default cook path were on
neither, so an artist reading the note would conclude they were already native and a future agent
working §13.9's list would never schedule them. Both are named here now.

**Still entirely inside `kernel`, in §13.9's order:**

| What | §13.9 item | Note |
|---|---|---|
| 4.2 the fitting solve | **N2** | and with it `pc_plan_bridge`, which exists only to feed the native frames and is deleted the day the VEX solve lands — along with D170's `pc_s0r_lo`/`pc_s1r_lo` |
| 4.4's `copytopoints` and the deform | **N4 / N5** | needs the kit-module plumbing and R8's packed-`transform` scale question |
| 4.5 conform | **N6** | `ray` as a node |
| 4.6 finalize and D153's guard switches | **N7** | |
| 4.3 corners | **N8** | boolean, or foreach + clip |

**Python geometry OUTSIDE the five stages, which the old list omitted:**

| What | Where | Status |
|---|---|---|
| `kit.box_mesh` — a `geo.createPoint()` per box corner | `kit.py:72-100` | **still Python.** §13.6 says it "leaves Python and becomes native box SOPs (§13.3.6)". It runs on every cook where input 2 is unwired, via `kit_geometry → starter_kit`. Its N is the module count (4), not the point count, which is why it has never shown up in a number — but it is unscheduled work, and it is scheduled now. |
| `hda._padded` — iterated `hou.Point` wrappers | `hda.py` | **fixed this cycle.** It read and wrote `pc_pad` one point at a time whenever the gap parm was non-zero: the exact pattern §11.9 rule 1 forbids and the standing tripwires watch for. It is two bulk calls now. |
| `place.read_curves`' native harvest | `place.py` | **Python by design (D166 point 4)** — bulk attribute reads and one slice per curve, marshalling data between a node and a caller. Named here so it is never mistaken for unfinished porting. |
| `hda.cook_config` | `hda.py` | **Python permanently.** Parameter and payload marshalling; its N is the size of the parameter page; it touches no geometry. This is the half of Hannes' rule that reads "processing data which is not possible to process with the other 3". |

The sticky note inside the asset carries the same two tables, so the graph tells the truth about
its own state to whoever opens it.

Three things a next cycle should carry forward from the numbers above:
...
2. **The long-curve row is the one to bench, not the streets row.** 300 short curves already beat
   the reference on both readings of §15.2's range; one 20 km curve is a wash. §13.9's R7 predicted
   the opposite, and benching only the citygen shape would have shown a win that is not the whole
   story.
3. **N2 should compute `s0r`/`s1r` inside the 64-bit wrangle and delete D170's residual pair.**
   The split is correct and it is a workaround for a boundary that N2 removes.

### 15.7 The audit — seventeen findings, and what each one cost

Three independent reviewers (parity, idiom/reachability, performance) read the build above. **Every
finding reproduced.** None was refuted; one — T1 — was a claim *this document* made that the
reviewers refuted, with a control test, and §15.4 now carries the correction.

| Severity | Finding | Outcome |
|---|---|---|
| critical | the native branch was disconnected from the shipped output | D166 — `kernel` behind `OUT_sections`; two inverted tripwires |
| critical | `setUnlockNewInstances(True)` cost 19× node creation, 27× .hip size, and broke fix propagation | shipped locked; §15.4 T1 corrected; `instances_do_not_fork_the_network` |
| major | a point shared by two prims had its arclength and neighbours overwritten (×2 reviewers) | D165 — `pc_unshare`, plus a fused-junction fixture and a bypass mutation |
| major | the `config` SOP read an unwired input for the style payload | one input convention; `config_reads_the_payload` on the built asset |
| major | `frames_parity` rounded away the one thing the network does differently | renamed `frames_arithmetic_*`; `plan_span_transport_at_20km` added |
| major | `plan_geometry` created float32 attributes, losing 9.765e-4 m at 20 km | D170 — head + residual, now 2.910e-11 m |
| major | the wall-clock comparand hoisted `read_curves` out of the timed loop | both bounds printed; §15.2's "not a speedup" withdrawn |
| minor | `native_id_parity` compared two copies of one rule, neither filtered | D167 — one rule; the check asks `read_curves` |
| minor | `pc_frames_valid` deleted every piece in silence | warned; the stage menu is swept under three parm states |
| minor | `pc_curve_index` promised a lookup and did a scan | D168 — a real dict |
| minor | the stale 0.0037 s claim in `pc_arclength`'s header | replaced by what the suite measures |
| minor | two dead locals in `pc_corners` one line above the vectors actually used | deleted |
| minor | §15.6 and the sticky note omitted `kit.box_mesh` and `hda._padded` | both listed; `_padded` fixed |
| minor | `report["frames"]` built 4.96 MB per cook for a key only the rig reads | D171 — opt-in |

~~**Still true after all of it (dev-loop rule 0):** no independent audit has run *on this build*.~~
**✅ DISCHARGED — the audit is §15.8, below.**


---

### 15.8 The audit — cycle N-1V, by an agent that built none of it

**Ran 2026-08-22 on commit `4be6743`.** Dev-loop rule 0 is discharged for the rebuild lane: the
words for §15 are now *implemented, measured, **audited*** — with the two named exceptions in
§15.8.7.

#### 15.8.1 Every suite, from clean

| Suite | Result |
|---|---|
| `python tests/unit/test_{citygen,plan,polychain,polychain_corner,polychain_plan,polychain_array2d}.py` | 22 / 49 / 61 / 63 / 91 / 60 = **346 OK** at HEAD (350 with the concurrent phase-2 lane's uncommitted additions). *The write-up's "328" is every unit test except `test_citygen.py`, counted against the dirty tree.* |
| `hython tests/polychain/run_native_checks.py` | **32 checks, 0 failing** (31 shipped + `asset_decompose_matches_the_rig`, added by this audit) |
| `hython tests/polychain/run_scene_checks.py` | 89 cases + `ZZ_port_tripwires`, **3 665 assertions, 0 failing**, and **not one of the 5 559 baselined values moved** |
| `hython tests/polychain/run_hda_checks.py` | **29 checks, 0 failing** |
| `hython tests/polychain/scale_gate.py` | **9 rows, 0 failing** |
| `hython tests/polychain/run_2d_checks.py` | 633 assertions, 0 failing (phase-2 lane, unaffected) |
| `hython tests/hda/run_attrib_checks.py` | 0 failing, **0 unreviewed baseline changes** — the six new wrangles publish nothing that moved the attribute baseline |

**Baseline movement, re-derived key by key.** `tests/polychain/baseline.json` does not appear in
`git diff --stat 338a3ca 4be6743`, and a clean run prints **no `moved since baseline` section at
all** — so every one of the 5 559 recorded values still *reproduces*, which is a stronger statement
than the file being unchanged. The detector was itself mutation-tested (perturb one value → it
prints `AA_reflex_miter/element_count: __MUT__ -> 17`). ⚠️ **It prints and does not fail**:
`run_scene_checks.py` exits 0 on a baseline move, so an agent reading only the exit code would miss
one. `tests/README.md`'s "a baseline move FAILS the run" is true of `tests/hda/run_attrib_checks.py`
and **not** of this runner. Unchanged by this cycle; recorded so it is not rediscovered.

**citygen: untouched.** No citygen path appears in the four commits' diff, and
`grep -rl polychain polyfactory/scripts/python/polyfactory/citygen/` returns nothing — the two
packages share no import. (`tests/hda/baseline.json` did move 293 lines, in `ff32929`, the
*conventions* cycle interleaved between these commits — not this lane.)

#### 15.8.2 The node tree, as Hannes will open it

`pf_polychain` — **21 nodes**, contents **LOCKED** on every new instance, 5 titled network boxes,
1 sticky note. Regenerated from `devScripts/create_pf_polychain_hda.py` and re-verified green, so
the shipped binary is reproducible from the script.

| Kind | Count | Nodes |
|---|---|---|
| **VEX** (`attribwrangle`, **all `vex_precision = 64`**, all commented) | **6** | `pc_curveid` (prim) · `pc_curve_index` (detail) · `pc_arclength` (prim) · `pc_corners` (point) · `pc_markers` (point) · `pc_frames` (point) |
| **native compiled SOPs** | **3** | `pc_unshare` [splitpoints] · `pc_frames_valid` [blast] · `stage_switch` [switch] |
| **nulls** (4 inputs, 4 stage outputs, 1 output) | **9** | `IN_SPLINE` `IN_KIT` `IN_STYLE` `IN_SURFACE` · `OUT_sections` `OUT_plan` `OUT_frames` `OUT_reference` · `OUT` |
| **OpenCL** | **0** | — |
| **Python SOP** | **3** | `config` · `pc_plan_bridge` · `kernel` |

**The three Python SOPs, named and judged against Hannes' rule** (*"everything geometry related
should be either native nodes, vex or opencl; Python can be used for ui or processing data which is
not possible to process with the other 3"*):

| Node | Verdict | Why |
|---|---|---|
| `config` | **✅ JUSTIFIED, permanent.** | Resolves the parameter page against the style payload on input 3 and writes one `pc_cfg` detail dict. N = the parm count; it touches no geometry. This is the "ui / marshalling" half of the rule, exactly. The wrapper tripwires are pointed at it so it cannot grow a loop. |
| `pc_plan_bridge` | **⚠️ NOT JUSTIFIED — declared scaffolding.** | Lifts what the reference already solved onto real points so the VEX `pc_frames` has something to read. Its own comment says *"13.9 N2 DELETES THIS NODE"*. It exists because §4.2's solve has not been ported. |
| `kernel` | **❌ NOT JUSTIFIED — this is the unported tool.** | ~6 000 lines, and it is `stage_switch` **input 0**, i.e. what an artist gets. §4.2 fitting solve, §4.3 corners/mitre/cap, §4.4 place + deform + stamp, §4.5 conform and §4.6 finalize all still run here, in Python, on every cook. |

**Also still Python, outside the SOPs**, each named rather than implied:
`place.read_curves`' bulk harvest (**justified** — whole-array `pointFloatAttribValues` reads plus
one slice per curve, no `hou.Point` in a loop, and the standing tripwires enforce it);
`kit.box_mesh` (**not justified but defended and unscheduled** — `geo.createPoint()` per box corner,
N = 4 modules at authoring time, kept explicit because the Box SOP's division mode emits 24
two-point polygons that can be neither sliced nor capped, measured on 22.0.398 — see §15.6).

#### 15.8.3 What the VEX actually reaches

- **DECOMPOSE is on the shipped cook path and load-bearing.** All 7 upstream nodes cook on an
  Output cook (`native_branch_cooks_on_output` 7/7), and corrupting `pc_arclength` moves the
  shipped geometry.
- **`pc_frames` is NOT.** It hangs off `OUT_plan` behind `stage_switch`, so nothing an artist
  builds passes through it. Mutation-proved: a 1e-7 relative error in its 3×3 reddens exactly one
  check (`frames_arithmetic_linear_parity`, measured on the *rig*) and nothing else in any suite.
  That is the honest scope of the port today — **one stage of five is in the artist's hands.**

#### 15.8.4 The mutation battery — 10 mutations, 2 survivors

Every source file was restored with `git checkout --` on a **named path**, and
`polyfactory/otls/pf_polychain.hda` is back at its committed sha `d1b0dec…`.

| # | Mutation | Verdict |
|---|---|---|
| MUT-4a | **`place._native_tables` returns `None`** — the whole network silently falls back to the Python reference | ✅ **RED** — `native_branch_is_load_bearing`. *The single most important result in this audit: a silent fallback fails the suite, so the rebuild is not cosmetic.* The scene and HDA suites stay green (byte-identical output **by design**, D166), which is why this one tripwire carries the whole claim. |
| MUT-4b | `kernel.setInput(0, IN_SPLINE)` — the pre-D166 defect restored | ✅ **RED** ×2 — `native_branch_cooks_on_output` 0/7 and `native_branch_is_load_bearing` |
| MUT-1 | `pc_frames.vfl`: `d * scale * 1.0000001` | ✅ **RED** — `frames_arithmetic_linear_parity` 1.000e-07 (rig only; see §15.8.3) |
| MUT-2a | `vex_precision = 32` on `pc_arclength` **in the rig** | ✅ **RED** ×6 — arclength 1.261e-05 m, curve length, the frames 3×3, frames P (1 730 ULP), the 20 km trial, and `native_intermediates_are_64bit` naming the node |
| MUT-2b | `vex_precision = 32` on `pc_arclength` **in the shipped asset only** | ✅ **RED** ×2 — `plan_span_transport_at_20km` 1.273e-02 m and `union_matches_the_python_path` 1.300 m. ⚠️ `native_intermediates_are_64bit` stayed green *and printed "32-bit wrangles: none"* while the asset shipped one — it scans the rig. Now also covered by `asset_decompose_matches_the_rig`. |
| MUT-3a | **bypass `pc_unshare` in the SHIPPED asset** | ❌ **SURVIVOR** — native, scene, HDA and scale suites all green. Root cause below. **Closed.** |
| MUT-3b | **bypass `pc_frames_valid` in the SHIPPED asset** | ❌ **SURVIVOR** — all suites green. **Closed.** |
| MUT-G4 | revert D91 in `hda.cook` (padding applied under a wired payload) | ✅ **RED** — `parms_inert_under_payload … moved: padding`. *(Reverting the same lines in `cook_plan_bridge` is invisible — the bridge is behind the Stage switch. Same root cause as MUT-3b, no shipped-output consequence.)* |
| MUT-G3 | `bend_tol` 0.01 → 0.50 (50× the curvature budget) | ✅ **RED** ×2 rows — `arc_10/kit` and `arc_80/adaptive`. PC-G3's ladder is not vacuous. |
| MUT-B | perturb one value in `tests/polychain/baseline.json` | ✅ **printed** — but exit 0; see §15.8.1 |

**Root cause of both survivors, and the fix.** `native.py`'s `stage_decompose` and
`create_pf_polychain_hda.py`'s `DECOMPOSE` tuple are **two independent declarations of the same
chain** that share only their `.vfl` bodies. Every mutation check in §4 of `run_native_checks.py`
mutates the *rig*, so an asset that drifted — a bypassed node, a changed precision, a re-ordered
input — shipped silently. **`asset_decompose_matches_the_rig`** (§5 of the runner) now reads node
type, `class`, `vex_precision`, snippet text and every bypassed node **back off the built asset**
and diffs them against the rig. Re-run under MUT-3a / MUT-3b / MUT-2b it reports
`pc_unshare(bypassed)` / `pc_frames_valid(bypassed)` / `pc_arclength.vex_precision differs`.

#### 15.8.5 Gate by gate

| Gate | Verdict | Evidence from this build |
|---|---|---|
| **PC-G1** — the fence | **✅ PASSES** (headless; GUI viewport pass still owed) | `gate_images.py` drove the closed rectangle and the L through **the parm face** in **both corner modes**: 40 checks, 0 failing. Miter — `corner_seam_m` **0.0**, `corner_abut_m` **0.0**, four 90° corners each resolved `miter`, `corner_breach_m` **0.0**. Bend — no corner post, `corner_breach_m` **0.042426** at exactly 0.00e+00 m over the accepted butt wedge. `exact_fill_m` ≤ 1.1e-07, `inward_faces` 0. **Looked at:** `VG1_closeup_miter_top.png` shows the corner post with the 45° bisector cut across it; `VG1_closeup_bend_top.png` shows the elbow turning as one continuous run with no post. |
| **PC-G2** — the hill | **✅ PASSES, judged on images** (GUI viewport pass still owed) | All four modes through the parm face, conform ON, 0 failing: `conform_contact_m` **0.0** in every mode, `plumb_deg` **0.0** over 26 vertical pieces, `flat_stepped_m` **9.7e-08**, `bank_deg` **32.79°** adaptive, `inward_faces` 0, **no warnings**. **Looked at:** in `VG2_vertical_side.png` the ribs are dead plumb while the run's foot tracks the green ground line through the full S-curve; in `VG2_adaptive_side.png` the same ribs visibly rake perpendicular to the drape. It reads as a fence on a hill. |
| **PC-G3** — scale | **✅ PASSES on its own (narrower) terms** | `scale_gate.py`: **9 rows, 0 failing, under both z-modes.** 10 000 packed / 0 deformed / 10 000 points / one `geometryid` / ~0.4 s on `two_point`, `resampled`, `arc_12000`, `arc_2000`, `arc_80` (kit) and `arc_12000`, `arc_2000` (adaptive); `arc_10` (kit) and `arc_80` (adaptive) correctly unpack. Mutation-proved (MUT-G3). |
| **PC-G4** — payload-only | **✅ PASSES, mutation-proved** | `run_hda_checks.py` §4: the payload replaces modules and styleId, `payload_matches_kernel` 64 vs 64, and **`parms_inert_under_payload` sweeps 40 parms — moved: none**. Non-vacuous: MUT-G4 reverts D91 and it reports `moved: padding`. New this cycle and independently re-run: `config_reads_the_payload` (77.0° / 3.0°, 0 disagreeing keys) and `payload_thresholds_reach_the_corners` prove the payload now reaches the **VEX** thresholds too. |

All eleven gate PNGs regenerated from this build are **byte-identical** to the committed
`tests/polychain/gate_images/` — the rebuild moved no gate geometry at all.

#### 15.8.6 What the next cycle must port — §13.9, with today's status

| §13.9 item | Stage | Status |
|---|---|---|
...
| **N8** §4.3 corners | planes / boolean | ❌ not started |
| **N10** the reference's fate | `kernel` | ❌ not decided — `kernel` is still `stage_switch` input 0 |
...
### 16.1 What exists now — 36 nodes, and one branch with no Python in its PER-PIECE path

⚠️ **CORRECTED 2026-08-23 (§17, review finding 17).** This section and commit `1768d94` both said `Stage = place_native` is "a chain with **no Python in it**". It is not, and the graph says so: `copy_packed` ← `kit_unpack` ← `pc_kit_id` ← **`kit_starter [python]`**, and `config [python]` cooks too — both were measured cooking on a `place_native` cook (`pc_plan_bridge` and `kernel` at `cookCount == 0`). The true claim is **no Python in its PER-PIECE path**: `config` marshals the parm page and the payload once per build and `kit_starter` builds §6's standalone fallback kit once per build, which is the half of Hannes' rule Python keeps. With input 2 UNWIRED — the first thing an artist sees — `kit_starter` runs `kit.box_mesh`, i.e. per-element Python geometry construction, on every cook; D178 justifies the node existing (it MOVES Python that already ran inside `kernel`) and D154's four native `box` SOPs are what discharge it. **D154 is not discharged**, and §16.8 says so now.
...
| Kind | Count (⚠️ **39 as of §17**, was 36) |
|---|---|
| **VEX** (`attribwrangle`, **every one at `vex_precision = 64`**, every one commented) | **13** — §17 adds `pc_stamp` |
| **native compiled SOPs** (`splitpoints` `attribdelete` x2 `blast` x4 `unpack` `copytopoints::2.0` `groupdelete` `switch`) | **11** — §17 adds `_scrub` and `_scrub_groups` |
...
three seconds are `copytopoints` itself.

> ⚠️⚠️ **THE SENTENCE ABOVE IS WRONG AND §17 MEASURED IT.** `copy_packed` costs **5.1 ms**, not
> two seconds — the attribution is off by ~950x and it was about to send N5 at the wrong node.
> Per-node self cook time on the shipped asset, 20 km / 20 001 vertices / 18 870 pieces,
> `Stage = place_native`: **`pc_frames_native` 4 833.5 ms**, `pc_plan_solve` 1 944.0 ms,
> `pc_plan_emit` 330.6 ms, `pc_arclength` 111.1 ms, **`copy_packed` 5.1 ms**. The cost was
> `pc_sample` reading four per-primitive ARRAY attributes — a `prim()` read of an array copies
> the WHOLE array, twice per piece. §17 fixes it (4 833 ms → 2.7 ms) and the whole 8.5x with it:
> `bench_plan_long_curve` is **2.68 s → 0.53 s**, i.e. 1.3x the reference rather than 6.8x, and
> `streets_300` is 0.3x — faster than the Python. The paragraph is left standing rather than
> deleted because a corrected claim teaches more than a vanished one. §15.2 had to withdraw a timing for having no comparand;
...
3. ~~**The 8.5x.** The native packed branch is slower than the Python it replaces on the
   10 000-piece shape, and roughly two thirds of that is inside `copytopoints`.~~
   **CLOSED AND THE DIAGNOSIS WAS WRONG (§17).** `copytopoints` measures 5.1 ms. The cost was
   `pc_sample`'s four array reads (4 833 ms), `pc_choose` rebuilding the candidate pool per piece,
   and 3.4's stamp running single-threaded — all three fixed, `bench_plan_long_curve`
   2.68 s → 0.53 s. **N5 does not need to start here any more.**

4. **`kit.box_mesh` is still Python and D154 is still unscheduled.** §16.1's "no Python in it" was
   wrong; the honest claim is "no Python in its per-piece path". With input 2 unwired,
   `kit_starter` builds §6's fallback kit in Python on every cook.

---

## 17. Native network build log — cycle N-2V (the independent review, worked through)

**Status:** written 2026-08-23. §16 was cycle N-2 and it shipped **unaudited** — dev-loop rule 0
was explicitly not discharged. Three independent reviewers (parity, determinism/threading, graph
idiom) then returned **17 findings**. This cycle reproduced every one first, fixed 15, turned one
into an asserted DECLARED LIMIT and one into a corrected claim, and left **`Stage` on `output`**
(D180 stands — §4.3, §4.5 and §4.6 are still the reference).

**Dev-loop rule 0 is now discharged for §16's build**: a fresh agent that wrote none of it
measured it and found real defects. It is **not** discharged for THIS build — the honest words
for §17 are *implemented, measured, unverified*, and the next audit should start at §17.4.

Commits on `polychain`, not pushed: `9413d81` the six parity divergences, `0386ec8` the sampler,
`a49e2eb` the candidate pool, `b5c4468` the `_*` law and the stamp.

### 17.1 The scoreboard — 17 findings, 17 verdicts

| # | Finding | Verdict | What it cost / what changed |
|---|---|---|---|
| 1 | `segIndex` defined outside `_fill`; `curve_id`/`section_index`/`slot`/`yclass` missing | **CONFIRMED, FIXED** | the two subject bags were not the same set — 0 vs 10 pieces on `segIndex`, 10 vs 11 on the rest. 16 stress rows |
| 2 / 10 | `pc_crc32` folds CODE POINTS, `zlib.crc32` folds UTF-8 BYTES | **CONFIRMED, FIXED** | a German `style_id` moved 28 of 40 `random` picks. 22 multibyte texts in the seed corpus |
| 3 | the `attr:` harvest reads this network's own `pc_*` internals | **CONFIRMED, FIXED** | `attr:pc_total` → 12 gates natively vs 10 panels in the reference. Filtered in `hda.rule_table` |
| 4 | a STRING per-point `pc_section` read as a float | **CONFIRMED, FIXED** | D7's mid-curve break vanished: 1 section / 12 pieces vs 2 / 14 |
| 5 | an EMPTY-STRING `markerData` value classified as the number 0 | **CONFIRMED, FIXED** | `json_dumps(dict, 0)` IS the dict introspection VEX was said not to have |
| 6 | `pc_frame_valid = 0` drops every piece with NO warning | **CONFIRMED, FIXED** | `pc_proto` raises a node warning; a check cooks a surface and a fillet and asserts it |
| 7 | a marker on a DUPLICATED curve id lands on one curve, not both | **CONFIRMED, ACCEPTED AS A DECLARED LIMIT — and now ASSERTED** | 22 pieces (reference) vs 21 (native), `pc_warn_marker_dup` on 1 point. `declared_limit_dup_id_marker` fails if the gap changes size in either direction |
| 8 | `stress_geometry(closed=...)` ignores its argument | **CONFIRMED, FIXED** | D19's cyclic run is a sixth shape in the matrix now |
| 9 | `pc_proto` resolves a duplicated module name FIRST-wins | **CONFIRMED, FIXED** | 8.0000 m pieces against the reference's 2.0000 m, silently, and it went away if the payload listed the kit in the other order |
| 11 | `plan_is_input_order_free` cannot reach the shared-id topology | **CONFIRMED, FIXED** | renamed `plan_distinct_ids_are_input_order_free`; `plan_shared_id_*` covers the tie-break |
| 12 | `pc_sample` copies four whole segment arrays per call | **CONFIRMED — the biggest number of the cycle** | 4 833 ms → 2.7 ms; §16.5 blamed `copytopoints`, which measures 5.1 ms |
| 13 | `pc_choose` rebuilds and re-sorts the pool per piece | **CONFIRMED, WORSE THAN REPORTED** | 66.5 s → 0.31 s on a 151-module kit (the review measured 32 s at 301) |
| 14 | conventions.md §2's `_*` rule not adopted anywhere | **CONFIRMED, ADOPTED** | 27 renames, `_scrub` + `_scrub_groups` before OUT, `run_attrib_checks` re-armed |
| 15 | "`pc_plan_only` blasts them, so nothing downstream sees an array attribute" is false | **CONFIRMED, MADE TRUE** | a blast removes points, not attribute definitions |
| 16 | 3.4's stamp runs single-threaded inside the detail wrangle | **CONFIRMED, FIXED** | `pc_plan_emit` 307 ms → 119 ms; `pc_stamp` is its own point wrangle |
| 17 | "the packed branch has no Python in it" is false | **CONFIRMED, CLAIM CORRECTED** | `copy_packed` ← `kit_unpack` ← `pc_kit_id` ← `kit_starter [python]`; `config` cooks too |

**Not one finding was wrong.** That is worth stating plainly: the author's own suite was 64 checks
and 0 failures on a build carrying five parity divergences and a 1 790x performance defect.

### 17.2 The parity half — five divergences, and why the suite could not see any of them

Every one lives in a place the 89 scene cases and the 170-build stress matrix structurally do not
go, and each is now a fixture:

1. **The conditional SUBJECT BAG was two different sets.** `plan.cond_subject` ends in
   `ctx.get(subject)`, so **every key of `plan_section`'s `ctx_base` is a readable subject** — not
   only the five `plan.COND_SUBJECTS` documents. The VEX defined `segIndex` at every pick made
   OUTSIDE the fill loop (start/end caps, the evenly probe, each evenly anchor, each marker, the
   default-rule probe) where the reference has no such key, and was missing `curve_id`,
   `section_index`, `yclass` and `slot`. Measured through the shipped HDA with a real
   `style.write` payload, one rule `{subject: segIndex, op: lt, value: 1}` on a 20 m line:
   `plan_native` 10 points, `plan` 0 points. `stress_cases` carries **16 new rows** — eight
   subjects × (the `default` slot, which runs inside `_fill` where `segIndex` IS defined, and an
   anchored slot, which does not) — because that difference IS the defect.
2. **`pc_crc32` hashed CODE POINTS.** `zlib.crc32` hashes UTF-8 BYTES, and because `pc_seed_for`
   runs through the same crc this was never only about `pc_elem_key` as the header claimed: with
   `style_id = "straße"`, 28 of 40 pieces picked a different module. **VEX already works in
   bytes** and that is the whole fix: probed on 22.0.398, `strlen("éx")` is **3** and `s[i]`
   decodes the code point that STARTS at byte i, answering `""` / `ord == -1` on a continuation
   byte — so skipping the −1 slots and re-encoding each code point reproduces
   `str.encode("utf-8")` exactly, in eight lines. **`pc_is_ascii` and `pc_warn_elem_key_utf8` are
   DELETED**: a warning that says "this may be wrong" about a value that is right trains the
   reader to ignore it. The corpus carries 22 multibyte texts and
   `seed_corpus_has_multibyte` asserts its own non-vacuity.
3. **The `attr:<name>` harvest read the network's own internals.** `primattribtype` finds
   anything, including `pc_total`, `pc_closed`, `pc_nclean`, `pc_iscurve`, `pc_curve_id_r` — every
   one of which was a live conditional subject natively and dead in the reference, whose
   `place._prim_attrs` skips `P/N/Cd/uv/v` and every `pc_` name outside `ROW_ATTRS_2D`. Fixed in
   **`hda.rule_table`**, which is the only place that decides what VEX may see, so the two filters
   cannot drift. `pc_section` and `pc_style` are no longer harvested off the prim at all — the
   section supplies them, exactly as `ctx_base` does.
4. **A STRING per-point `pc_section` was read as a float** (always 0.0), so D7's mid-curve break
   was invisible: a 4-point polyline keyed a,a,b,b built ONE section and 12 pieces against the
   reference's two sections and 14, with both interior caps gone. §3.1 types `pc_section` as an
   int; D7 promotes it to point class as the material-ID analog and `place.read_curves` reads
   whatever the artist authored, so a string is inside the contract. `pc_plan_emit` writes the key
   back in its own type as well.
5. **An EMPTY-STRING `markerData` value was classified as the number 0**, so a string condition on
   it could never match — a 0.12 m post where the reference puts a 1.6 m gate, and every
   downstream piece 0.21 m out. The old comment named the ambiguity and nothing asserted it.
   ⚠️ **`json_dumps(dict, 0)` IS the dict introspection VEX was said not to have** (probed): it
   emits `{"a": {"type": "string", "value": ""}, "b": {"type": "float", ...}}`, so one call per
   marker names the type of every slot. `dicttype` does not exist and `typeid` is ambiguous on a
   dict slot — both re-probed — but `json_dumps` was never tried.

And one more, on the PLACE side: **`pc_proto` resolved a duplicated module name FIRST-wins** while
`Kit._by_name`, `pc_kit_by_name` and `copytopoints`' own `useidattrib` are all LAST-wins. On a kit
with three prims named `post` (0.5 / 1.0 / 2.0 m) the branch measured one module and copied
another: 8.0000 m pieces against the reference's 2.0000 m, `node.warnings()` empty, and the error
vanished if the payload listed the kit in the other order. `place_duplicate_module_name` pins all
three resolvers together.

### 17.3 The performance half — three findings, and the cycle's own attribution was wrong

**(1) `pc_sample` copied the whole segment table, twice per piece.** A `prim()` read of an ARRAY
attribute copies the whole array, and the sampler read four of them per call. Per-node self cook
time on the SHIPPED asset, 20 km / 20 001 vertices / 18 870 pieces, `Stage = place_native`:

| node | before | after |
|---|---|---|
| `pc_frames_native` | **4 833.5 ms** | **2.7 ms** |
| `pc_plan_solve` | 1 944.0 ms | 1 138.5 ms |
| `pc_plan_emit` | 330.6 ms | 118.9 ms |
| `pc_arclength` | 111.1 ms | 46.8 ms |
| `copy_packed` | **5.1 ms** | 4.8 ms |
| whole cook | 7.96 s | **2.05 s** |

§16.5 wrote *"roughly two of those three seconds are `copytopoints` itself"*. It is 5.1 ms. The
attribution was off by ~950x and §16.8 handed N5 the wrong first job on the strength of it.

The table is **per-VERTEX storage** now — segment j on vertex j of the same primitive, which
always fits (`nseg <= ncum - 1 <= primvertexcount`) — and **two columns instead of four**:
`_seg_lo` IS `_seg_hi[j-1]` exactly (a dropped segment has zero length, so the kept segments stay
contiguous in arclength), and the segment's two VECTORS are not stored at all — `_seg_i` names the
vertex the segment starts at and `a`/`d` come off `P` there, the same float32 the prim wrangle
read. Two values per vertex instead of seven is what keeps the DECOMPOSE stage where it was:
seven values took `decompose_long_curve_wall_clock` from 1.33x to 1.61x, because four BULK array
writes had become a per-vertex loop. **The comparisons are untouched**, which is why D30's
extrapolation, the forward/backward tie-break and every parity number are unmoved.

**(2) `pc_choose` rebuilt the candidate pool per piece**, and each of the insertion sort's O(n²)
comparisons called `pc_m_variant`, which copies the whole `pc_k_variant` detail array. Measured on
the plan chain, 2 km fence, **1 000 pieces held constant**, one `default` rule that names no
modules so the kit resolves by ROLE (§7.2's own idiom):

| modules | 1 | 51 | 151 |
|---|---|---|---|
| `first` | 0.051 s | 0.051 s | 0.053 s |
| `random` **before** | 0.283 s | 2.632 s | **66.493 s** |
| `random` **after** | 0.267 s | 0.295 s | **0.309 s** |

The `first` row isolates the whole delta to the selection path. **215x at 151 modules**, and the
review's own reading (32 s at 301) understated it. Nothing in the pool depends on the piece, so
`pc_pool(rule, role)` resolves, sorts and weighs once and `pc_fill` hoists it out of the loop —
D175's fix one level up.

**(3) 3.4's stamp ran single-threaded.** `pc_plan_emit` must be a DETAIL wrangle (D150: `addpoint`
from a multithreaded wrangle emits in thread-completion order), so `sprintf` and `pc_elem_key`'s
per-byte crc32 — the one embarrassingly parallel part of the emit — ran on one core: 23 % of the
node at 10 000 pieces. `pc_stamp` is its own POINT wrangle. ⚠️ **The warning fan-out STAYS in the
emit**, deliberately: it writes an attribute whose NAME comes from the data, and attribute
CREATION order in a multithreaded wrangle is thread order.

**Together:** `bench_plan_long_curve` **2.68 s → 0.53 s** — from 6.8x the reference to **1.3x** —
and `bench_plan_streets_300` 0.074 s → **0.039 s**, which is **0.3x**: on citygen's own shape the
native chain is now 2.6x faster than the Python it replaces, while building 4 800 prims against
the reference's 14 700 (§4.3's corner assemblies are N8, so the two are still not comparable).

### 17.4 What the suite gained, and the checks that would have caught these

`run_native_checks` is **64 → 80 checks**. The ones worth naming:

| Check | What it pins |
|---|---|
| `plan_stress_parity` | 300 builds (was 170): **16 subject-bag rows** and a sixth shape, D19's closed ring |
| `plan_fixture_parity` | 12 SHAPE-shaped fixtures the style matrix cannot express: `markerData` by value type including `""`, per-point `pc_section` as string and int, two prims sharing a curve id, non-ASCII ids through `random`, and a **120-module kit** by role |
| `declared_limit_dup_id_marker` | D169's divergence AS A NUMBER — 22 vs 21 pieces — failing if it changes size either way |
| `plan_shared_id_matches_reference_in_both_orders` + `..._is_order_sensitive` | the prim tie-break, which `plan_is_input_order_free` structurally could not reach |
| `place_duplicate_module_name` | `Kit._by_name`, `pc_kit_by_name`, `pc_proto` and `copytopoints` all resolve LAST-wins |
| `native_place_says_why_it_is_empty` | an unported build ships 0 prims AND a warning, on a surface and on a fillet |
| `frames_cost_is_flat_in_segment_count` + `mutation_pc_sample_array_reads` | 250 ms ceiling on the 20 km fixture; the mutation measures **8 805 ms** |
| `plan_cost_is_flat_in_kit_size` + `mutation_plan_pool_per_piece` | 1 → 151 modules at a fixed piece count, ceiling 3.0x (measures 1.1x); the mutation measures **2 316 ms vs 310 ms** |
| `seed_corpus_has_multibyte` | the seed corpus's own non-vacuity — 22 of 380 texts |

⚠️ **Two of these caught a defect the moment they existed, which is the point of writing them.**
The first version of the pool hoist dropped `cfi["segIndex"]` from both reselect blocks and
`plan_stress_parity` went red on three shapes; the first version of `frames_cost_is_flat_in_...`
reported **0.0 ms for 18 870 pieces**, because `cook(force=True)` on a node whose inputs have not
changed is a no-op (D164, in a new place) — it is dirtied through a nudge wrangle now.

⚠️ **And one mutation lever was measuring the sound build.** `vexsrc.source` INLINES
`#include "pc_path.h"` — that is what it is for — so a mutation that patches the header text and
then looks for the include line in the body finds nothing. The lever is the SNIPPET.

### 17.5 conventions.md §2 and §5, adopted

Measured on the build §16 shipped: attributes beginning with `_` = **0**, on all four classes, at
every one of the seven `Stage` values; **one** `attribdelete` in the whole graph (POINT class only,
a precision fix) and **no** `groupdelete` — while the cycle had created ~50 internal attributes and
named every one of them as if it were contract. It was already leaking: the packed prims carried
`pc_deform`, `pc_index` and `pc_scale`, none of which §3.4 names and none of which `place.build`
publishes.

- **27 renames** to `_*`: the plan's twelve per-section arrays (`_p_*`), `_npieces`,
  `_sec_length`, `_is_section`, `_sec_key{,_s,_kind}`, `_style_key`, `_attr_{f,s}`,
  `_attr_names`, `_mk_{id,s,u,data}`, `_nsections`, and the sampler table `_seg_{hi,i}` /
  `_nseg` / `_first`. Every one verified by grep to be read by nothing but the VEX chain.
- `pc_corners`' working point group is **`_cornerpt`**.
- **`_scrub`** (`attribdelete _*`, **all four classes**, a WILDCARD and not a list — `pf::prepare_mesh`
  named its four internals explicitly, in the wrong class, and had never deleted one of them) and
  **`_scrub_groups`** sit immediately before `OUT`.
- `pc_deform pc_index pc_scale` are out of `COPY_ATTRIBS`.

⚠️ **`tests/hda/run_attrib_checks.py` excluded polyChain with the comment "DELETE THIS on parity —
the `_*` rule is adopted in the rebuild, so it should pass on arrival."** It would have FAILED on
arrival, and that made the one test that could see the leak both disarmed and wrong about why.
polyChain is in the run now: the law passes at every `Stage` value, `upstream/pf_polychain` records
`survives: []` (the sweep destroys upstream `_*` too — conventions.md §2's stated and deliberate
cost), and the snapshot is baselined.

**What stays `pc_*`, and why**, so the next reader does not have to re-derive it: `pc_s`,
`pc_cleanidx`, `pc_iscorner`, `pc_turn_deg`, `pc_corner_degen`, `pc_corner_forced` and `pc_nclean`
are §13.10's UNION interface, read by `place._native_tables`; `pc_curveprim`, `pc_proto_len`,
`pc_proto_ax`, `pc_yscale`, `pc_upref`, `pc_basey`, `pc_has_basey`, `pc_s0r{,_lo}` and
`pc_s1r{,_lo}` are `hda.FRAME_POINT_ATTRS`, the plan→frames bridge `pc_plan_bridge` still writes;
the rest are §3.1 input or §3.4 output contract. All of them move in the `pc_*` → `pf_*` pass that
conventions.md §3 rule 2 defers until after parity, for the stated technical reason: renaming one
side of a parity comparison destroys the comparison.

### 17.6 Decisions

- **D181 — the VEX conditional bag IS `ctx_base`, key for key, and `slot` is added inside
  `pc_choose`.** `cond_subject` ends in `ctx.get(subject)`, so `COND_SUBJECTS` is documentation and
  not a filter: `curve_id`, `section_index`, `slot` and `yclass` are readable subjects and
  `segIndex` belongs to `_fill` alone. The bag is built where the answer exists — the four
  constants at the top of `pc_plan_solve`, `slot` inside `pc_choose` — rather than at five call
  sites that would drift.
- **D182 — `pc_crc32` encodes UTF-8, and `pc_is_ascii` is deleted rather than kept as a warning.**
  VEX's `strlen` is a BYTE count and `s[i]` decodes the code point starting at byte i (−1 on a
  continuation byte), so the encode is eight lines and every string is answerable. A guard that
  warns about correct values is worse than no guard.
- **D183 — `json_dumps(dict, 0)` is VEX's dict introspection.** §16 recorded that VEX has no
  `dicttype` and that `typeid` is ambiguous on a dict slot; both are still true, and both were the
  wrong question. `json_dumps` emits the TYPE beside every value, which is what resolves the
  empty-string / numeric-zero ambiguity exactly rather than by preference.
- **D184 — what the VEX may read as `attr:<name>` is decided in `hda.rule_table`, not in the
  VEX.** One list, in the one place that already knows `place._ATTR_SKIP` and `ROW_ATTRS_2D`, so
  the two filters cannot drift. The VEX asks for names; it does not choose them.
- **D185 — the sampler table is PER-VERTEX storage, two columns, and derived where it can be.** A
  `prim()` read of an array attribute copies the whole array; per-vertex storage is read one scalar
  at a time. `_seg_lo` is `_seg_hi[j-1]` and the two vectors are `P` at `_seg_i` — two values per
  vertex instead of seven, because four bulk array writes becoming a per-vertex loop is itself a
  cost. 13.5's OpenCL transliterability is kept and arguably improved: indexed reads into flat
  tables are what OpenCL wants.
- **D186 — the candidate POOL is resolved once per RUN.** Nothing in it depends on the piece.
  This is D175 one level up, and it is the difference between 0.31 s and 66.5 s on a 151-module
  kit.
- **D187 — 3.4's stamp is its own POINT wrangle; the warning fan-out is not.** The stamp reads only
  per-point values and is embarrassingly parallel. The fan-out writes attributes whose NAMES come
  from the data, and attribute CREATION order in a multithreaded wrangle is thread order — which is
  a determinism exposure, not a speed question.
- **D188 — D169's marker-on-a-duplicated-id divergence is ACCEPTED and ASSERTED, not fixed.** The
  reference places the gate on BOTH curves (`resolve_markers` filters by id); one point wrangle can
  only bind one prim. Closing it means resolving markers per PRIM in `pc_sections`, which moves
  `Marker.distance_on`'s `dist`/`u` logic into a second place and is a stage change, not a patch.
  The limit is 22 vs 21 pieces plus `pc_warn_marker_dup`, and `declared_limit_dup_id_marker` fails
  if it ever changes size in either direction — which is the difference between a documented limit
  and a comment.
- **D189 — conventions.md §2 is adopted for the rebuild's own scaffolding NOW, and `pc_*` → `pf_*`
  still waits.** §3 rule 2 says exactly this, and the `_*` half costs nothing the parity harness can
  feel: it reaches these attributes through `native.py`, never by artist-facing name.
- **D190 — a stage that cannot answer must say so ON THE NODE.** D177's "declares what it cannot
  answer" was written to a point attribute that `pc_place_valid` then blasted. `hou.Node.warnings()`
  does not aggregate a child's warnings onto the asset (measured again here) — the badge does, and
  `run_hda_checks` already reads the inner node for `kernel`'s warnings, so `pc_proto` follows that
  convention rather than inventing one.
- **D191 — NO DEFAULT IS FLIPPED, STILL.** D180 stands unchanged. §4.3, §4.5 and §4.6 are the
  reference, so `Stage` defaults to `output`. Every number above is measured on branches an artist
  reaches only through the Stage menu.

### 17.7 What is NOT done

1. **No independent audit of THIS build.** Rule 0 is discharged for §16, not for §17.
2. **No GUI viewport pass.** Everything is headless.
3. **D169's marker fan-out** (D188) — accepted, asserted, not fixed.
4. **D154** — `kit.box_mesh` is still Python behind `kit_starter`, and §16.1's "no Python in it"
   is corrected rather than made true.
5. **N5 onwards is untouched**: the deform gate, §4.5 conform, §4.6 finalize, §4.3 corners, the
   reference's fate. What changed is that N5 no longer starts by investigating `copytopoints`.
6. **A `pc_*` conditional subject that is a VECTOR** (a prim `attr:` of size 3, or a vector-valued
   `markerData`) still reads as a scalar natively where the reference has a tuple. Nothing in the
   review raised it and no fixture reaches it; it is named here rather than left unsaid.

---

## 18. Native network build log — cycle N-2V2 (the independent verification of §17)

Written by an agent that wrote none of §17, on the committed tree at `bec344e`. Everything below
was re-derived from clean — the suites were re-run, the baseline diff re-computed key by key, the
node tree probed off the built asset, six mutations applied and reverted, and the two image gates
re-judged on pictures this cycle had to fix first.

**VERDICT: §17's 17 findings are real and its numbers reproduce. Rule 0 is DISCHARGED for §17's
build.** But the audit found **two survivors and one blind gate**, and one of them is the exact
failure mode the brief named: *a silent fallback that makes the rebuild cosmetic while everything
stays green.*

### 18.1 Everything, from clean — the actual runs

| Suite | Command | Result |
|---|---|---|
| unit | `python tests/unit/test_{citygen,plan,polychain,polychain_plan,polychain_corner,polychain_array2d}.py` | 22 + 49 + 61 + 91 + 63 + 64 = **350, OK** |
| polyChain geometry | `hython tests/polychain/run_scene_checks.py` | **0 failing** |
| polyChain native | `hython tests/polychain/run_native_checks.py` | **77 `[PASS]`, 0 failing** |
| polyChain HDA | `hython tests/polychain/run_hda_checks.py` | **0 failing** |
| scale gate | `hython tests/polychain/scale_gate.py` | **9 rows, 0 failing** |
| attribute hygiene | `hython tests/hda/run_attrib_checks.py` | **0 failing, 0 unreviewed baseline change** |
| phase 2 | `hython tests/polychain/run_2d_checks.py` | **0 failing** |
| gates | `hython tests/polychain/gate_images.py` | **0 failing** |

⚠️ **§17 reported "80 checks"; the run prints 77 `[PASS]` lines and no total.** The suite is 77
checks, not 80. Nothing is missing — the count in the write-up is wrong.

**Baseline movement, re-derived key by key.** `tests/hda/baseline.json`: 71 → 73 top-level keys,
**added `pf_polychain` and `upstream/pf_polychain`, zero removed, and `CHANGED VALUES: []`** — the
additions-only claim holds against the actual dict, not against the diff's line count.
`upstream/pf_polychain` really is `survives: []`. `tests/polychain/baseline.json` is **untouched
across the whole fix pass** (0 lines in `git diff 2324855 HEAD`), so "not one baselined value
moved" is confirmed.
...
The asset is **39 nodes** in **5 titled network boxes**: 13 attribute wrangles (**all 13 at
`vex_precision = 64`**, verified by probe), 11 nulls, 4 Python SOPs, and 11 native SOPs
(`splitpoints`, 4 `blast`, 2 `attribdelete`, `groupdelete`, `unpack`, `copytopoints::2.0`,
`switch`). The 4 Python SOPs are `config`, `kernel`, `pc_plan_bridge`, `kit_starter`.

**Per stage, cooked on a fresh node each time, so nothing is a cache artefact:**

| `Stage` | prims | Python SOPs that cook | VEX wrangles that cook | verdict |
|---|---|---|---|---|
| **`output`** (THE DEFAULT) | 317 | **`config`, `kernel`** | 5 / 13 | **PYTHON REFERENCE** — decompose is VEX, everything after it is the ~6 000-line kernel |
| `config` | 0 | `config` | 0 / 13 | Python (correctly — it is parameter resolution, §13.6) |
| `sections` | 1 | `config` | 5 / 13 | **NATIVE** |
| `plan` | 0 | `config`, `pc_plan_bridge` | 5 / 13 | **PYTHON** (this stage *is* the bridge) |
| `plan_native` | 0 | `config` | 9 / 13 | **NATIVE** |
| `frames` | 0 | `config`, `pc_plan_bridge` | 6 / 13 | **PYTHON** plan, VEX frame |
| `place_native` | 20 | `config`, `kit_starter` | 12 / 13 | **NATIVE** but for `kit_starter` (D154) |

**And the share, measured on the shipped default** (`Stage = output`, whole-node cook against the
same node at `Stage = sections`, which is the VEX decompose alone):

| input | `output` | decompose (VEX) | **Python share of the cook** |
|---|---|---|---|
| 20 m / 21 pt | 0.146 s (317 prims) | 0.017 s | **88.2 %** |
| 2 km / 2 001 pt | 0.171 s (33 007 prims) | 0.020 s | **88.2 %** |
| 20 km / 20 001 pt | 1.753 s (330 192 prims) | 0.082 s | **95.3 %** |

**So: ~88–95 % of what an artist cooks is still Python, and it gets worse with scale.** Exactly one
stage — §4.1 DECOMPOSE — has left Python on the path the artist actually runs. §4.2 and §4.4's
packed half are ported, at parity, and **behind a Stage menu nobody sets**. That is D180/D191
working as designed, and it is also the honest headline: the rebuild is real, and it is not yet
shipping.

### 18.3 MUTATION — six applied, TWO SURVIVED

Each mutation was applied, the asset rebuilt from `create_pf_polychain_hda.py` where the asset was
the target, the suite run, and the tree restored and re-verified (`git status` identical, HDA
`sha256 c9454193…828f77c`, and a re-run at **77 / 0**).

| # | Mutation | Result |
|---|---|---|
| **M1** | **the solve's accumulation broken** — `cursor += target` becomes `cursor += (idx == 2) ? target * 0.99 : target` in `pc_plan.h`'s `_fill` | **RED, 7 checks**: `plan_solve_parity`, `plan_stress_parity`, `plan_fixture_parity`, `plan_shared_id_matches_reference_in_both_orders`, both mutation baselines, `asset_stages_match_the_rig` |
| **M2** | **a wrangle de-batched** — `pc_stamp` from a POINT wrangle to a DETAIL wrangle looping over every piece on one thread (i.e. §17 finding 16 reverted) | 🔴 **SURVIVOR — 77 / 0, entirely green** |
| **M3** | **`vex_precision = 32` on a world-scale wrangle** — `pc_arclength` | **RED, 9 checks**, and honestly: `decompose_arclength_parity` **1.261e-05 m**, `trials_irrational_20km_asymmetric` 5.563e-04 m |
| **M4** | **silent fallback** — `Stage = plan_native` AND `place_native` rewired to `OUT_reference` | RED, but only **3** checks, and **not one of them is a parity check**: `native_place_says_why_it_is_empty`, `copy_useimplicitn_is_a_noop`, `mutation_pc_kit_id`. All three are place-side accidents |
| **M4b** | **the subtle half of the same** — only `plan_native` rewired, to `OUT_plan` (the Python bridge) | 🔴 **SURVIVOR — 77 / 0, entirely green** |
| **M5** | **PC-G4** — D91 reverted (`padding` applied under a wired payload) | **RED**: `parms_inert_under_payload … moved: padding` |
| **M6** | **PC-G3** — the curvature budget x 50 | **RED, 2 rows**: `arc_10/kit` and `arc_80/adaptive` — exactly D97's recorded claim |

#### SURVIVOR 1 — the batching regression the suite measures and passes anyway (M2)

`pc_stamp` exists *only* because 3.4's stamp had to stop being single-threaded (§17 finding 16,
`pc_plan_emit` 307 → 119 ms). De-batching it back is invisible: **77 / 0**. And the suite did not
merely miss it — **it measured it and printed PASS**:

| bench | sound | M2 | delta |
|---|---|---|---|
| `bench_plan_long_curve` | 0.5444 s (1.4x) | **0.6761 s (1.7x)** | +24 % |
| `bench_plan_streets_300` | 0.0395 s (0.2x) | **0.0800 s (0.6x)** | **+103 %** |

Both benches carry the line *"NO CEILING is asserted: this is a HALF-ported chain … a recorded
before, not a verdict."* That rationale is defensible for the *ratio to the reference*; it also
means the only two checks that can see a batching regression **cannot fail**. `pc_frames_native`
and `pc_plan_solve` each got a real flatness ceiling this pass
(`frames_cost_is_flat_in_segment_count`, `plan_cost_is_flat_in_kit_size`); `pc_stamp` and
`pc_plan_emit` got none.

#### SURVIVOR 2 — the native plan can be unplugged from the artist and nothing notices (M4b)

Pointing the `plan_native` entry of `STAGES` at `OUT_plan` makes the menu entry labelled
*"2 - Plan, NATIVE (4.2 - the VEX fitting solve)"* serve `pc_plan_bridge`'s **Python** plan. Probed
after the mutation, `Stage = plan_native` cooks `config, pc_plan_bridge` and **0 of the 4 solve
wrangles** — byte-for-byte the `plan` stage. The suite is **77 / 0**.

The cause is structural and worth stating plainly: **every parity check runs on `native.py`'s RIG,
not on the asset's Stage switch.** `asset_stages_match_the_rig` compares each stage node's class,
precision, snippet, type and parameters — it does not compare **wiring**, so a stage output null
routed to the wrong place is invisible to it. `stage_menu_reaches_every_stage` only asserts each
stage is non-empty and labelled. The one check pointed at this problem,
`native_plan_and_place_reach_no_artist`, asserts the native branch does **not** cook on `output` —
the opposite direction. There is nothing asserting that a native stage *is* native.

M4's louder half is worth recording too: routing `place_native` to the reference was caught **only**
by three checks that happen to poke `copytopoints` parameters or count an empty build. Had those
three not existed, half of §13.9 N4 could have been served by the Python kernel with a green suite.

### 18.4 THE BLIND GATE — PC-G1 and PC-G2 were judged on images that could not show the fence

`tests/polychain/gate_images.py` defines `unpack(geo)`, whose docstring says in as many words:
*"A PACKED PRIM HAS ONE VERTEX, so the wireframe drawn straight off a polyChain output is EMPTY."*
**It had no caller.** All five `rasterise(...)` calls were handed the raw node output.

Measured on PC-G1's rectangle:

| case | prims | packed (invisible) | segments drawn (committed) | segments with `unpack` |
|---|---|---|---|---|
| rect / miter | 80 | **40 — half the fence** | 188 | **3 388 (18x)** |
| rect / bend | 137 | 35 | 447 | **3 079 (6.9x)** |

The committed `VG1_rect_miter_top.png` was four corner posts and the input spline. Every "judged on
an image" claim for PC-G1 and PC-G2 in §0.0 rests on pictures with the packed pieces missing.

**Fixed, in one word at four call sites** (`rasterise(..., unpack(node_geo), ...)`), plus a new
`drawn_covers_packed()` assertion per case so it cannot silently regress — 10 new checks, all green,
each printing the packed count beside the polygon count. Every gate image gained content
(`VG1_rect_miter_top` 188 → 3 388 segments; the PC-G2 elevations 1 838 → 2 174).

### 18.5 The gates, re-confirmed on the corrected images

- **PC-G1 ✅ PASSES, both corner modes, through the parm face.** 6 builds x 10 checks, 0 failing —
  `exact_fill_m` ≤ 4.4e-07, `max_gap_m` ≤ 5.4e-07, `corner_seam_m` **0.0**, `corner_breach_m`
  **0.0** on every mitered corner, `inward_faces` **0**. **And now looked at**: the mitered
  rectangle is a closed ring of panels and posts with a corner post at each of the four vertices
  and the 45° bisector cut clean across it, legs terminating flush into it; the bend rectangle
  turns each elbow as one continuous arris with **no** corner post (D36's ring weld) and only the
  accepted butt notch. No holes, no overlaps.
- **PC-G2 ✅ PASSES, all three z-modes plus camber, conform ON.** 0 failing; `conform_contact_m`
  **0.0** in every mode, `plumb_deg` **0.0**, `flat_stepped_m` **9.7e-08**, `bank_deg` 32.79,
  `stepped_float_m` 0.1 against the recorded limit 0.11, `over_unpacked` **0**, no warnings.
  **And now looked at**: `vertical` draws pickets dead plumb with the run's foot on the ground
  line; `stepped` is a clean staircase of flat panels over a smooth ground line; `adaptive` drapes
  continuously with the ribs raking perpendicular to the fall; and the **front** elevation shows
  the cambered rail visibly rolled onto the cross-fall while the posts stay plumb.
- **PC-G3 ✅ PASSES, both z-modes.** `scale_gate` **9 rows, 0 failing** — `arc_12000` / `arc_2000`
  at **10 000 packed / 0 deformed / 10 000 points / 1 geometryid** under both `kit` and `adaptive`;
  `arc_10/kit` and `arc_80/adaptive` correctly deform. **Mutation-proved**: x50 the budget fails
  exactly those two rows. ⚠️ The deformed rows are much faster than §0.0 records — `arc_10/kit`
  **1.744 s** and `arc_80/adaptive` **1.774 s**, where §0.0 still says 10.8 s and 11.0 s. That is
  cycle 10c's bulk setters; the recorded numbers are stale, not wrong.
- **PC-G4 ✅ PASSES.** `payload_overrides_modules`, `payload_overrides_styleid`,
  `payload_matches_kernel` (64 vs 64), and `parms_inert_under_payload` **swept 40 parms; moved:
  none**. **Mutation-proved independently**: reverting D91 reports **`moved: padding`** and the
  check goes red, so the sweep is live on today's fixture.

### 18.6 Two smaller things, recorded so they are not rediscovered

- **The `.hda` is NOT reproducible from its builder.** Running `create_pf_polychain_hda.py` twice
  with no source change produces a different `sha256` (`c945…` → `430d…`). So the committed binary
  can only be restored with `git checkout`, never by rebuilding, and **`asset_stages_match_the_rig`
  is the only thing tying the shipped bytes to the script that claims to describe them.**
- **`run_attrib_checks.py` caps a menu sweep at `MENU_VALUES = 4`.** §17's "the law passes at all
  7 Stage values" is not what that runner proves — it sweeps the first four menu entries. The law
  does hold (0 leaks, `survives: []`), but on 4 of 7 stages, not 7.

### 18.7 Decisions

- **D192 — a native stage must be PROVED native, not labelled native.** M4b showed the whole §4.2
  port can be unplugged from its own menu entry with the suite at 77 / 0, because parity is
  measured on the rig and the asset's Stage switch is unasserted wiring. The next cycle owes one
  check: cook the SHIPPED asset at each native Stage and assert that the stage's own wrangles
  advanced their `cookCount` and that no Python SOP but `config` did. The probe in §18.2 is that
  check, minus the assertion.
- **D193 — a batching guard belongs on every wrangle, not on the two that were profiled.** M2
  survived because `pc_stamp` and `pc_plan_emit` have no cost ceiling while `pc_frames_native` and
  `pc_plan_solve` do. The two `bench_plan_*` checks explicitly assert nothing; that is defensible
  for a ratio against a half-ported chain and it means the suite cannot fail on a 2x regression it
  printed. Give the emit chain a flatness ceiling in pieces the way N4 gave one to the frames.
- **D194 — a gate image must contain the packed pieces, and now asserts that it does.** `unpack`
  existed, documented the hazard, and was never called. `drawn_covers_packed` is the assertion; the
  general rule is that a rasteriser fed polyChain output is showing the *deformed minority* unless
  something says otherwise.
- **D195 — the check count is 77.** §17 said 80. Corrected here rather than in place, because the
  number is load-bearing for "was 64".

### 18.8 What is NOT done

1. **D192 and D193 are recorded, not built.** This cycle wrote one new assertion (D194's) and left
   the two survivors' guards to the next.
2. **No GUI viewport pass**, still. Everything is headless; the bridge answered `houdini_status`
   but was not driven.
...
## 19. Native network build log — cycle N-3 (the two survivors, N5's gate, N7's stamp, and the flip)

**Status:** written 2026-08-23, on `polychain`, commits `7bed7b8`, `f5e5156`, `a13d35f`,
`58cbf82`. Everything below was measured on Houdini 22.0.398 in headless `hython`; the live bridge
was not driven. **Dev-loop rule 0 is NOT discharged for this cycle** — the honest words are
*implemented, measured, unverified*, and the next audit should start at §19.7.

**THE HEADLINE, in the metric §18.2 asked for and in a better one.** §18.2 measured the Python
share as `1 − decompose_cook / output_cook`, which was a fair proxy only while `Stage = output`
**was** the Python kernel. It is not any more, so the share is measured directly now — every
Python SOP in the asset timed on its own cook, summed, over the whole node's cook:

| build | guard | whole cook | Python SOPs that cook | **Python share** |
|---|---|---|---|---|
| 20 m straight, 20 prims | **NATIVE** | 0.0062 s | `config` only | **0.1 %** |
| 2 km straight, 1 888 prims | **NATIVE** | 0.0876 s | `config` only | **0.0 %** |
| 20 km straight, 18 870 prims | **NATIVE** | 1.6255 s | `config` only | **0.0 %** |
| 2 km arc R = 2 000 | reference | 0.1328 s | `config`, `kernel` | 97.8 % |
| 2 km over a ripple, 33 357 prims | reference | 0.2567 s | `config`, `kernel` | ~100 % |
| L with a 90° corner | reference | 0.0074 s | `config`, `kernel` | 63.0 % |

**Before this cycle every build was the bottom three rows.** The shipped default is native for a
build the guard can answer whole, and on those builds the only Python left is `config` — the parm
page and the payload, which is the half of Hannes' rule Python keeps. **9 of the 92 phase-1 cases
are inside that envelope today**, and widening it is what N5's deformed branch, N6, N7 and N8 buy.

### 19.1 D192 and D193 — the two mutation survivors, closed

Both of §18.3's survivors are guarded, and each guard is proved to redden.

- **D193 — a batching ceiling.** `emit_cost_is_flat_in_piece_count` and
  `stamp_cost_is_flat_in_piece_count` time `pc_plan_emit` and `pc_stamp` **each on its own**,
  dirtied from its own input with `cookCount` asserted, at 2 500 and 20 000 pieces. Measured:
  the emit **2.88 µs/piece** and the stamp **0.37 µs/piece**, both flat (the full ladder is
  1 000 / 2 500 / 5 000 / 10 000 / 20 000 → emit 3.25 2.80 2.78 2.75 2.91, stamp 1.34 1.32 1.28
  0.33 0.30). Ceilings 6.0 and 3.0 µs/piece plus a 1.5x growth ceiling.
  **`mutation_pc_stamp_debatched` puts M2's detail loop back — same `pc_elem_id`, same
  `pc_elem_key`, only the cost model changes — and measures 20.41 µs/piece.**
- **D192 — a native Stage proved native.** `native_stages_are_really_native` cooks the SHIPPED
  asset at `sections`, `plan_native` and `place_native` and asserts two things at once: every node
  that stage is made of advanced its `cookCount`, and no Python SOP outside a named allowance did.
  ⚠️ **The dirtying lever is the SPLINE and not a parm** — `corner_angle_deg` only dirties
  `config`, so `pc_curveid` and `pc_arclength` would have read as idle on a sound build and the
  check would have been green for the wrong reason. `mutation_plan_native_unplugged` and
  `mutation_place_native_unplugged` rewire the switch input on the built asset and both report
  **both halves** (the stage's own nodes idle AND the Python SOP cooking).

### 19.2 Nothing had ever compared the STAMP, and five of 3.4's fourteen were wrong

`place_packed_parity` measures the packed prim's `P` and its world bounds and reads **0.0 m over
471 prims**, so the native PLACE branch was believed to reproduce the reference. It reproduced the
**geometry**. `place_stamp_parity` — the first check to compare the stamp, in **both directions**
over every prim attribute either side publishes — opened at **383 complaints**:

| what | measured |
|---|---|
| `pc_generated`, `pc_deformed`, `pc_corner_cut`, `pc_replaced` | **absent** |
| `pc_style` | the CURVE's style key (empty on every case) where 3.4 wants the id of the style that built the element |
| `pc_section` | the artist's section KEY as a **float** where `_stamp_values` writes the section INDEX as an **int** — on a 30 m run sectioned 5/7/9, all 36 prims read 5.0/7.0/9.0 against 0/1/2 |
| `pc_u` | float64 `0.003703703703703704` against the reference's float32 `0.003703703638166189` |
| `pc_warn_kit_gap`, `pc_warn_stage_unported` | published on every element, where the reference declares a warning attribute only when the build raised one |

The `pc_section` one is worth stating plainly: **the reference calls one attribute name by two
meanings** — on a PLAN POINT `pc_section` is the artist's section key (`Placement.as_dict`), on the
BUILT PRIM it is the section index (`_stamp_values`) — and copying the plan point's value straight
through `copytopoints` took the wrong one.

Fixed by 13.9 **N7's first half**: `pc_finalize` (a prim wrangle carrying 4.6's stamp),
`pc_out_cast` (13.2's `attribcast`, at the one place the 64 bits are wrong), `_sec_out` carrying
the index under the `_*` law, and `pc_proto` writing its kit-gap warning with `setpointattrib` so
the attribute exists only when it fires. **471 prims, 21 cases, 0 complaints.**

Five attributes the branch still cannot publish are a **declared table** naming the 13.9 item that
owes each, and `place_stamp_owed_is_live` **fails on any entry no case exercises** — so the
exemption list can neither be padded nor rot. It has already shrunk once: `pc_warn_degenerate_frame`
and `pc_warn_corner_degenerate` came off it when the deform gate started raising them.

### 19.3 13.9 N5 — the deform gate, and a blast that was missing

`pc_gate.h` + `pc_deform_gate` is `place._needs_deform` and `place.span_deviation` in VEX. Profiled
with cProfile on the 20 km fence, this was the biggest single Python item left on the shipped path:
**`_needs_deform` + `span_deviation` is 0.133 s of `place.build`'s 0.813 s** — more than the fitting
solve (0.057 s) and the packed transform (0.048 s) put together — and it is the rule PC-G3 rides
on (10 005 packed at 0.42 s and +12 MB against 21.9 s and 360 180 points).

**`gate_parity`: 574 pieces over 28 cases, `pc_deformed` at the gate against the reference's own
`pc_deformed` on the built prim, matched on `pc_elem_id` — 574 agree, 0 disagree.**
`gate_parity_sees_both_answers` requires the comparison to contain both, and it does: **471 packed
/ 103 deformed**. `mutation_pc_deform_gate` drops the budget to −1 m and flips every piece.

⚠️ **AND 13.3.4's `blast(packed)` DID NOT EXIST.** The diagram has one; the build did not. Every
piece went to `copytopoints`, including the ones the curvature budget unpacks, so a bending panel
shipped as a rigid chord. It was invisible because `place_packed_parity` matches on `pc_elem_id`
against the reference's PACKED prims — a piece the reference deformed simply had no counterpart to
disagree with, so the check compared a SUBSET and called it parity. `pc_packed_only` is the missing
blast and **`place_packed_covers_the_reference`** is the missing assertion: the branch builds
EXACTLY the reference's packed element set, 472 elements, bar one that is 3.4's stand-in box for a
module the kit does not carry — **and the row FAILS if that count ever reads zero.**

Two of `span_deviation`'s four terms need a surface this graph has not ported (D100's camber
rotation, D104's extra stations) and D99's band is a per-point rule a packed prim cannot hold. The
gate **declares** those rather than answering: `pc_gate_valid` beside `pc_frame_valid`, the piece
dropped by `pc_place_valid`, a node warning naming the parm. **33 pieces were declared, not scored.**

The answer publishes as `pc_deformed` — 3.4's own name for it, computed one stage early — and not
as 13.3.4's `pc_needs_deform`, because `_scrub` deletes `_*` before OUT and an `_*` gate answer is
invisible at `Stage = gate`, which is the one place an artist goes to ask why a run unpacked.

### 19.4 13.9 N10 — the guard switch, and the 1.6x it nearly shipped

`Stage = output` is a guarded fork now: `guard_envelope` (level 1) → `guard_native` (level 2) →
`OUT_final`, with `OUT_reference` reachable as its own `Stage = reference`.

**Two levels, and the split is about cost.** A switch cooks only its selected input and a selector
that reads a detail attribute forces that node to cook, so level 1 is decided from `config` and the
decompose — both of which cook anyway — and a build it refuses never cooks the native chain at all.
Level 2 asks the one question only the plan can answer, **by counting** the gate's points against
the packed branch's: every reason a piece leaves is a blast, so "nothing was dropped" is
`npoints(gate) == npoints(packed)` and needs no second copy of their conditions.

⚠️ **THE FIRST VERSION OF IT WAS A 1.6x REGRESSION AND THE A/B FOUND IT.** With only the parameter
and corner tests in level 1, a fence over a ripple passed level 1, cooked the whole native chain,
was refused by level 2, and then cooked the reference as well:

| shape | reference | through the guard | |
|---|---|---|---|
| 2 km over a ripple | 0.238 s | 0.361 s | **1.52x** |
| 20 km over a ripple | 2.489 s | 3.971 s | **1.60x** |

Level 1 answers the deform question itself now, **exactly and conservatively**: a curve with no
KINK and no elevation change cannot produce a deformed piece, whatever the module or the span —
`span_deviation`'s spine term is 0 (no interior vertex, D66/D69), its off-spine term is 0 (every
tangent is the chord's, so the frame does not rotate), and `vertical`'s shear test is 0. It is a
bound, not a guess: nothing there says a curved run WOULD deform, only that a straight flat one
cannot. **`output_guard_cost` is the committed ceiling** — a REFUSED build may cost no more than
1.15x (level 1's own probe) and an ADMITTED one no more than 1.6x. Measured after the fix:

| shape | ratio | path |
|---|---|---|
| 20 m straight | 1.07x | native |
| 2 km straight | 1.39x | native |
| 20 km straight | 1.16x | native |
| 2 km arc R = 2 000 / R = 80 | 1.04x / 1.00x | reference |
| L corner | 1.07x | reference |
| 2 km / 20 km over a ripple | 1.01x / 0.99x | reference |

**`output_guard_parity` is what the cycle rests on.** Every one of the 92 cases is cooked at
`Stage = output` and at `Stage = reference` and compared on everything a consumer can see — prim
attribute NAMES, TYPES and every value in ORDER, every packed prim's world bounds, every point
position, the detail warning arrays and the prim groups. **9 native / 83 reference, IDENTICAL.**

`output_runs_the_native_chain_inside_the_envelope` replaces
`native_plan_and_place_reach_no_artist` — the ladder its own comment said to edit on this commit —
and asserts BOTH directions: inside the envelope all 8 native nodes cook and `kernel` does not;
outside, `kernel` cooks and none of them do.

**Two artist-visible behaviours had to move with the output, and the suite found both.**
`run_hda_checks`' `unread_marker_warns` went red because D88's warning is raised on the `kernel`
node — so a build with a marker in it keeps the reference until N7 moves the warning — and
`hda.cook`'s `"%s on %d elements"` is raised by `pc_warn_collate` now, in `hda.cook`'s own wording.

`mutation_guard_envelope` is aimed at **level 1, not at the fixture**, and it had to be: after each
of level 1's three tests landed, no case in the 92 reached level 2 any more, so the second switch
was a node nobody could show doing anything. Widening level 1 past what it can model hands a
rippled run to the native chain, level 2 refuses it and the output is unchanged, and widening level
2 as well changes the output.

### 19.5 §4.2's port is 20x SLOWER than the Python it replaced, and half of that is found

The A/B nobody had run: **`pc_plan_solve` costs 61 µs/piece where `plan.plan_sections` costs
2.2 µs/piece on the same input** (20 000 pieces, one section). `bench_plan_long_curve` could not
see it — it says in as many words that it asserts no ceiling — and this cycle's own D193 ceilings
landed on `pc_plan_emit` and `pc_stamp`, which do not write string arrays.

**Half of it is fixed here, and the Houdini behaviour behind it is worth not rediscovering: a VEX
string ARRAY attribute costs ~9 µs per ELEMENT to write.** Five of them at 20 000 pieces is 100 000
elements. The five values are massively repetitive — `slot` is `"default"` for a whole run and
module/variant/zmode/warns come from a unit of one or two modules — so they leave as a deduped
TABLE plus one int per piece (`_p_row`), with a fast path on the previous row and a `PC_ROW_MAX`
spill back to the old shape. Measured A/B in one process on a 20 000-piece section: **1.88 s →
1.22 s**.

⚠️ **The rest is inside `pc_fill` and this cycle did not isolate it.** Three stub experiments
(`no_localreads`, `no_u_at`, `no_join`) each moved it by ~5 µs/piece and stubbing all three still
left 40 µs/piece; two micro-benchmarks that looked decisive turned out to be dead-code-eliminated
and are **not** quoted. **This is the next cycle's first item**, and `output_guard_cost`'s 1.6x
admitted ceiling is where it gets paid off.

### 19.6 Decisions

- **D196 — a guard switch, not a big-bang flip.** `Stage = output` takes the native chain where the
  native chain can build the whole output and the reference otherwise, decided by data rather than
  by a parm. That is D153's mechanism applied to N10, and it is what lets 13.9 land one item at a
  time with the default moving each time instead of once at the end.
- **D197 — the guard's level 1 must answer the DEFORM question itself.** A capability guard that
  discovers its refusal after the expensive stage has cooked is a 1.6x regression, measured. The
  test is a conservative bound (no kink, no elevation change) and not a prediction; widening it to
  a curved run needs the turn-per-module-length bound and is not done.
- **D198 — a native Stage's output is compared on NAMES, TYPES and VALUES, in both directions.**
  `place_packed_parity`'s 0.0 m over 471 prims was a statement about geometry that read as a
  statement about the element. Four attributes were absent, two wrong and two extra with the whole
  suite green.
- **D199 — an exemption list fails when it stops being exercised.** `STAMP_OWED` names the 13.9
  item that owes each attribute and `place_stamp_owed_is_live` reddens on any entry no case hits.
  Two entries have already come off it.
- **D200 — a VEX string ARRAY attribute costs ~9 µs per element to write, so a per-piece string
  column crosses a node boundary as a table plus an index.** Recorded as a Houdini behaviour, with
  the A/B that measured it. ⚠️ And recorded with its trap: **two micro-benchmarks written to
  isolate it were dead-code-eliminated by VEX** and reported 0.05 µs/element for work that was
  never done. A VEX micro-bench whose result is not written to an attribute measures nothing.
- **D201 — `Stage = reference` is a menu entry now.** The oracle has to stay reachable from the
  parm page once `output` stops being it, and every parity check in `run_native_checks` uses it.

### 19.7 What is NOT done — where the next audit should start

1. **No independent audit.** Dev-loop rule 0 is not discharged for this cycle.
2. **`pc_plan_solve` is 61 µs/piece against the reference's 2.2**, and only the string-array half of
   that is explained. It is the first item.
3. **The envelope is 9 of 92 cases.** An ARC is refused although its pieces stay packed, because
   level 1's bound only covers straight flat runs; a MARKER is refused for a warning, not for
   geometry; a CORNER, a SURFACE, a FILLET, a BAND, a slope fix, D98's flatten and `tile`'s slice
   are refused because those stages are Python. N5's deformed branch, N6, N7 and N8 are what widen
   it.
4. **D90's slow-cook hint does not reach the artist on the native path.** `hda.cook` times its own
   build and suggests the proxy; a wrangle cannot. Declared, not fixed.
5. **`run_attrib_checks` sweeps the first FOUR menu entries** (§18.6) and the menu now starts
   `output, reference, config, sections` — so `plan` left the sweep and 11 bridge internals left
   the baseline with it. The removal is real and reviewed; the CAP is still the limitation §18.6
   named.
6. **No GUI viewport pass**, still.

---

## 20. Native network build log — cycle N-3V (the audit response: two shipped parity
divergences, and five checks that could not fail)

**Status:** written 2026-08-23, on `polychain`. Everything below was measured on Houdini 22.0.398
in headless `hython`; the live bridge answered `houdini_status` and was not driven. **Dev-loop
rule 0 is NOT discharged for this cycle** — the honest words are *implemented, measured,
unverified*; the next audit should start at §20.6.

⚠️ **TIMING CAVEAT FOR EVERY NUMBER BELOW.** Another agent was running `hython` suites in this
repo throughout. Wall-clock readings on the heavy nodes moved 26 → 42 µs/piece run to run on an
unchanged build, which is what forced §20.4's estimator change. Ratios measured inside one
interleaved call are trustworthy; single absolute readings across runs are not.

**THE HEADLINE METRIC, re-measured on this build.** Every Python SOP that cooks, timed on its own,
against the whole node's cook (best of 3, dirtied through `corner_angle_deg`):

| build | guard | whole cook | Python that cooks | **Python share** |
|---|---|---|---|---|
| 20 m straight, 20 prims | **NATIVE** | 0.0047 s | `config` 0.0016 s | **35.0 %** |
| 2 km straight, 1 888 prims | **NATIVE** | 0.0680 s | `config` 0.0017 s | **2.5 %** |
| 20 km straight, 18 870 prims | **NATIVE** | 0.8532 s | `config` 0.0018 s | **0.2 %** |
| 2 km arc R = 2 000 | reference | 0.0987 s | `config`, `kernel` | 84.5 % |
| 2 km over a ripple | reference | 0.1598 s | `config`, `kernel` | 99.5 % |
| L with a 90° corner | reference | 0.0070 s | `config`, `kernel` | 90.3 % |

**The share did not move this cycle and it was not supposed to** — §19's flip is what moved it,
from §18.2's 88.2 % / 95.3 % on every build. What moved is that the flip is now *provable*: before
this cycle a single wiring edit could undo it with the suite 94 `[PASS]` / 0 green (§20.1), and
`Stage = output` was shipping a **different fence** from the reference on two reachable
conditions (§20.2). `config`'s 1.6-1.8 ms is a FIXED cost — the parm page and the payload — so the
20 m row's 35 % is that constant against a 4.7 ms build, not a per-piece cost; it is flat from
20 m to 20 km.

**Suites: `run_native_checks` 105 `[PASS]` / 0 failing** (was 94/0), `run_hda_checks` 0,
`run_scene_checks` 0, `scale_gate` 9 rows / 0, `gate_images` 0, `run_2d_checks` 0,
`run_attrib_checks` 0 failing / 0 unreviewed, 328 unit tests OK. **No baselined value moved.**

### 20.1 Five mutations of the SHIPPED asset that a 94-PASS suite could not see

Every one was reproduced on a copy of the built `.hda`, and every one is red now.

| mutation | what it did | what saw it before | what sees it now |
|---|---|---|---|
| `stage_switch[1]` → `OUT_final` | made `Stage = reference` serve the guarded native output, so `output_guard_parity` compared the output **with itself** over all 92 cases and still printed "identical"; `output_guard_cost`'s spread collapsed to 1.02x / 1.01x / 1.02x / 0.98x and passed both ceilings | **nothing** | `every_stage_entry_serves_the_node_it_names` |
| `stage_switch[0]` → `OUT_reference` | the exact undo of §19's flip | `output_runs_the_native_chain_inside_the_envelope`, `mutation_guard_envelope` — 2 of 94 | + `every_stage_entry_serves_the_node_it_names`, + `native_stages_are_really_native` (which now has an `output` row) |
| `stage_switch[6]` → `OUT_reference` | `Stage = frames` showed the Python kernel's finished fence — 18 packed prims carrying only `P` where the sound build gives 18 points carrying 33 attributes | **nothing** | `every_stage_entry_serves_the_node_it_names` |
| `OUT_frames` ← `pc_frames` | skipped the `pc_frames_valid` blast. An UNPLUG is not a bypass, so `asset_stages_match_the_rig`'s `isBypassed()` scan — whose comment claims it closed this — structurally could not see it | **nothing** | `every_stage_entry_serves_the_node_it_names` (the FEEDER column) |
| `pc_place_valid` ← `pc_deform_gate` | an intra-stage unplug inside the PLACE box | **nothing** | `asset_stages_match_the_rig`, which compares INPUT WIRING now (27 wires) |

**The root cause was one line of prose.** `STAGES` in `create_pf_polychain_hda.py` said "the menu,
the switch's inputs and the HDA-wiring check all read it, so a stage cannot be added to one and not
the others" — and `grep -rn STAGES tests/` returned exactly one hit, a prose mention inside a
docstring. **No check read it.** It is `tests/polychain/native.STAGES` now, imported by the build
script, with a third column naming the node each stage's null must be FED BY; the build script
asserts both columns at build time and `every_stage_entry_serves_the_node_it_names` asserts them
against the built asset. `mutation_stage_wiring` runs all four repoint/unplug mutations and
requires all four to be reported.

Two more checks stopped resting on the envelope's own word. `output_guard_parity` classified a
case as native by reading `_native_ok2` off `pc_envelope2` — a detail int that cooks whether or not
anything downstream is wired to it, which is why "9 native / 83 reference … identical" survived the
native chain being disconnected from `Stage = output` entirely. It counts `copy_packed`'s
`cookCount` now and **fails the row when the observation and the envelope disagree**.
`NATIVE_STAGES` gained a `frames_native` row and an `output` row (which needs its own fixture — the
D192 L-shape has a corner, and level 1 refuses a corner outright), taking it from 17 watched nodes
over 3 stages to 32 over 5.

### 20.2 `Stage = output` was shipping a DIFFERENT FENCE, twice, on builds the guard admits

Both are 100 %-of-the-run divergences on `Stage = output` with `native_ok` at 1 and no warning.

**A `pc_cond` list of 2, 3 or 4 numbers.** The artist face writes a style into a DICT point
attribute; Houdini hands a 2-, 3- or 4-number list back out of `style.read` as a
`hou.Vector2/3/4`, which is neither `list` nor `tuple`, so `_cond_columns` fell through to
COND_BAD and `pc_evaluate_cond` answered False to `in` where the reference answers True. Measured
on a 20 m line with `{"subject": "sectionLength", "op": "in", "value": [20.0, 3.0]}`:
`Stage = output` built **10 prims, all `pc_module = panel`**; `Stage = reference` built **12, all
`gate`**. Lengths 1, 5 and 6 round-trip as tuples and agreed — which is why 92 scene cases and a
170-build stress matrix could not see it: **nothing in the suite round-tripped a condition value
through the payload at all.** `_cond_columns` accepts any non-string sequence now, and
`payload_cond_values_survive_the_round_trip` sweeps lengths 1..6 across the boundary and compares
both stages of the shipped asset on everything a consumer can see.

**An `attr:<name>` subject that is not a scalar.** `pc_sections`' bag did `float v = prim(0, name,
pr)`, which reads **component 0** of a vector and 0 of an array, while `place._prim_attrs` hands
the reference the whole tuple and `evaluate_cond`'s TypeError guard makes every ordered comparison
on it False. Measured with `vecattr = (7.5, 1.0, 2.0)` and `attr:vecattr gt 5.0`: **12 `gate`
prims natively against the reference's 10 `panel`**, and `lt 100.0` diverged the other way.
`eq`/`ne` agreed by luck, which is exactly why they are the controls in the new fixtures.

The fix is one new answer rather than two special cases. `pc_cond_subject` had two — absent (False
under every operator) and readable — where Python has three: a subject whose value is a **tuple or
a dict** is not None, so `evaluate_cond` reaches the operator, where every ordered compare raises
TypeError and is caught as False, `eq` is False and **`ne` is True**. That is `PC_S_UNREADABLE`,
published under a sentinel key prefix in the string bag so no function signature has to grow a
third dict. It closes the reviewer's third finding for free: `ctx_base`'s own dict-valued keys
`attrs` and `marker_data` are real subjects (`cond_subject` ends in `ctx.get(subject)`) that
answered False to `ne` where Python answers True — measured as a 10-vs-12-piece plan divergence,
and reaching `Stage = output` only by the accident that `style._check_cond` calls `attrs` an
unknown subject.

**And COND_BAD now fails SAFE.** It used to mean "VEX evaluates this as False", which is the
reference's answer only by luck; `_native_ok` refuses the build on it, like every other unported
feature in that function. `native_ok_refuses_an_unreadable_cond` is the check. **It cost nothing:
the envelope is still 9 of 92.**

30 new parity fixtures carry all of this — 5 attribute shapes × 6 operators, plus 2 dict-valued
subjects × 4 operators — and reverting the three VEX files reddens `plan_fixture_parity` at
`attr_vec3_gt`, `attr_vec3_ge` and `attr_vec3_lt` (13 pieces where the reference builds 167).

### 20.3 `bench_guard_fallback` was cited by two source comments and did not exist

Both `pc_envelope.vfl` and the build script said it "measures that rather than assuming it away".
`grep` found no such check anywhere: `output_guard_cost`'s four shapes are all either refused at
level 1 or admitted through level 2, so **no committed check exercised the fallback path at all**.

It exists now, and it has to force level 1 open to do its job — because level 1 answers the deform
question itself, **no legitimate input reaches level 2's refusal on this build**. Both halves are
asserted: `bench_guard_fallback` measures the double cook at **1.47x (2 km) and 1.34-1.58x
(20 km)** against a 1.8x ceiling, and `no_case_pays_the_guard_fallback` asserts **0 of 92** cases
pay it. The second row is what makes the guard free today rather than merely unmeasured, and
§19.7 item 3 — widening level 1 past straight-and-flat — is exactly the change that re-opens it.

### 20.4 `pc_plan_solve` gets the ceiling D193's mandate named, and 13-18 % off the node

D193's mandate was to give the two `bench_plan_*` rows real ceilings. It landed them on
`pc_plan_emit` and `pc_stamp` — **2.6 and 0.4 µs/piece, the two cheapest nodes in the chain** —
while `pc_plan_solve`, which is 78 % of the native chain at 20 km, kept `best < 30.0`: a
thirty-second wall around a 0.6-second node.

**The measured ladder** (min over interleaved repetitions, after the fix below):

| pieces | 1 250 | 2 500 | 5 000 | 10 000 | 20 000 | 40 000 | 80 000 |
|---|---|---|---|---|---|---|---|
| µs/piece | 21.0 | 20.7 | 21.9 | 23.7 | 32.2 | 53.3 | 53.8 |

Not quadratic — 40 000 → 80 000 is flat — but not flat either; the step is where the per-section
arrays stop fitting in cache. So the solve gets its own growth ceiling (2.0x, measures 1.20x) as
well as an absolute one (**34 µs/piece, measures 24.6-25.0**).

⚠️ **AND THE ESTIMATOR HAD TO CHANGE, WHICH IS A FINDING IN ITSELF.** A single best-of-3 pass on
this node read 26-42 µs/piece for the *same build*: three interleaved repetitions gave sound
27.40 / 42.15 / 27.10 against mutant 48.79 / 44.19 / 41.28, so one run reports a ratio of 1.05 and
another 1.78. **A loose ceiling is the "NO CEILING is asserted" problem in a new costume** —
measured, at a ceiling of 40 the mutation passed at 37.44 in a loaded run and failed at 49.45 in a
quiet one. `solve_scale_rates` takes the MIN over three interleaved repetitions of each variant:
interference can only make a pass slower, so the minimum is the closest thing to the node's own
cost, and interleaving means both variants see the same machine. Sound then reads 24.6-25.0 and
the mutant 41.6-42.5, ratio 1.67-1.72, and the check asserts **both** the absolute ceiling and
≥ 1.25x the sound node measured in the same call.

**The mutation changes no planned value.** Forcing the solve down its SPILL branch writes the five
per-piece string ARRAYS the dedup table replaced — a live, supported transport that ships the same
plan — so every parity check in the suite stays green under it. That is exactly why the original
20x regression shipped unseen.

**And 13-18 % came off the node.** The post-sort permutation copy — twelve `push()` calls per piece
into twelve parallel local arrays — ran on every fill whether `ord` permuted anything or not, and
on an ordinary run it does not: `_p_index` reads (0, 1, 2, 3, …) on a straight fence, so the merge
sort's answer is the identity and the copy reproduced the order it was already in. Guarded, the
arrays are assigned wholesale. Bit-identical on `plan_solve_parity`, `plan_stress_parity` (170
builds) and `plan_fixture_parity` (50 builds, including the anchor and marker cases where the
permutation is genuinely non-trivial). End to end: `bench_plan_long_curve` **0.4369 → 0.3510 s**
and `output_guard_cost`'s admitted shape **1.05x → 0.92x**.

### 20.5 The sticky note said the opposite of the build, and the Frames stage showed a dead branch

13.7's readability deliverable is the first thing an artist reads on opening the network, and
`every_wrangle_says_what_it_computes` only asserts that a comment is non-EMPTY. The note shipped
saying `Stage = output` "IS `kernel`", and calling 13.9 N5's deform gate and N7's finalize NOT
STARTED — while `pc_deform_gate` and `pc_finalize` were both in the network and both cooking. An
artist toggling `pc_deform_gate` off to see what it does was being told by the network that the
node does not exist yet. The build script's own docstring was stale the same way ("36 nodes"
against 51). Both are rewritten to this build, and `the_note_matches_the_build` reads the note back
off the built `.hda` and refuses a 13.9 item marked NOT STARTED whose nodes stand in the network —
mutation-proved by feeding the replaced text back in (3 complaints, needs 3).

And `Stage = frames` resolved to `OUT_frames ← pc_frames_valid ← pc_frames ← OUT_plan ←
pc_plan_bridge [python]` — the bridge branch, which is idle on every `Stage = output` cook. **Both
menu entries that named 4.4's transform showed a branch the output does not run**, and the branch
it does run had no entry at all. `frames_native` is a stage now (`OUT_frames_native ←
pc_frames_native`), and the two bridge entries say "via the PYTHON BRIDGE" in their labels.

⚠️ **THE REVIEWER'S OTHER OPTION — deleting `pc_plan_bridge`, `pc_frames`, `pc_frames_valid`,
`OUT_plan` and `OUT_frames` outright — WAS DECLINED, DELIBERATELY (D205).** It is the smaller
graph and it should happen. It is also a five-node deletion that rewrites `mutation_m4b`,
`native_branch_cooks_on_output` and two `STAGES` entries, in a cycle whose job was closing
mutation survivors and two shipped parity divergences. §0.0 already records that N5's deformed
branch is what retires the bridge; it goes with that, and the menu now labels it honestly in the
meantime.

### 20.6 Decisions

- **D202 — a `pc_cond` value's TYPE is a transport question, and the transport is the payload.**
  Any non-string sequence is a list, because `style.read` decides what the type is and Houdini's
  dict attribute returns `hou.Vector2/3/4` for three of the lengths. And a subject that is present
  but not a scalar is a THIRD answer (`PC_S_UNREADABLE`), not a missing key: Python answers True
  to `ne` on a tuple or a dict and False to everything else, and reproducing that exactly is
  cheaper than enumerating the types that reach it.
- **D203 — the stage table is DATA the checks read, not prose the build script keeps.** A comment
  claiming three consumers read a tuple that none of them read is how three menu-repoint mutations
  survived. `native.STAGES` carries (token, null, feeder, label) and both the build script and the
  suite read all four columns.
- **D204 — a cost ceiling on a heavy node needs an estimator, not a stopwatch.** Min over
  interleaved repetitions of each variant, and a mutation asserted as a RATIO against the sound
  node measured in the same call as well as against the absolute. A ceiling loose enough to
  survive machine noise is not a ceiling.
- **D205 — the bridge branch is LABELLED, not deleted, this cycle.** Deleting it is the right
  graph and it belongs with N5's deformed branch, which is what makes it dead. Until then
  `Stage = frames_native` shows the branch the output uses and the two bridge entries say what
  they are.

### 20.7 What is NOT done — where the next audit should start

1. **No independent audit.** Dev-loop rule 0 is not discharged for this cycle.
2. **The bridge branch is still in the graph** (D205) — five nodes and the last Python SOP that
   does geometry work. It goes with N5's deformed branch.
3. **The envelope is still 9 of 92**, unchanged: nothing this cycle widened it and the COND_BAD
   refusal did not narrow it. §19.7 item 3 is still the list, and level 1's straight-and-flat
   deform bound is still the cheap widening nobody has written — with `bench_guard_fallback` and
   `no_case_pays_the_guard_fallback` now standing where it will land.
4. **`pc_plan_solve` is 25 µs/piece against `plan.plan_sections`' ~3.4** — still ~7x, down from
   ~20x. What is left is inside `pc_fill`; D200's warning stands (two micro-benchmarks written to
   isolate it were dead-code-eliminated by VEX).
5. **The `N6`/`N8` half of `the_note_matches_the_build` is vacuous today** — both are genuinely
   NOT STARTED and own no nodes, so only the N5/N7 rows, the `kernel` claim and the node count are
   load-bearing. It arms itself the moment either one lands.
6. **Timing numbers were taken on a shared machine** (see the caveat at the top of §20). The ratio
...
## 22. Native network build log — cycle N-4A (PART A: the evidence machinery, repaired)

**Read this before §21.** §21's audit found ten source-level mutations, **two survivors** and
three defects in the evidence machinery itself, and recorded them as D206–D210. This cycle closes
all five. It changes **no geometry, no kernel and no VEX** — every line of it is in
`tests/polychain/`, and every baselined value in the tool is unmoved. What it changes is what the
suite can *see*.

**The rule this cycle was run by: reproduce, fix, then re-run the mutation and show it reddens.**
Four of the five defects were reproduced **at source with the `.hda` rebuilt**, which moves the
asset and its declaration together the way a careless commit would — and each rebuild's md5 is
recorded below, so a reader can tell a real reproduction from a described one.

### 22.0 The scoreboard

| # | defect | reproduced | fix | the mutation, now red |
|---|---|---|---|---|
| **D210** | `run_scene_checks`' baseline was advisory | (already, `adde049`) | its **second half**, never run: all twelve native-lane commits replayed; and `run_2d_checks`' identical rule closed | `sys.exit(1 if failures and not update...)` put back → `7/9` |
| **D206** | 14 of 18 wrangles had no cost assertion | `pc_finalize` de-batched at source, .hda `6f2fec2e`, **106 `[PASS]` / 0** | a measured ceiling for **all 18**, one generic loop | four mutations, each reddening only what it should |
| **D207** | a Stage label can claim NATIVE for the dead bridge | M8 at source, .hda `34fbd13a`, **111 `[PASS]` / 0** | labels and the 18 wrangle headers become **checked claims** | the source M8 → `stage_labels_are_true` red |
| **D208** | the one declaration was its own oracle | M10 at source, .hda `92b0d456`, **94 `[PASS]` / 0 `[FAIL]` + a traceback** | a behavioural second voice for **all ten** stages, and the crash made a complaint | same build → 114 `[PASS]` / **3 `[FAIL]`**, no crash |
| **D209** | `output_guard_cost` failed once in three unmutated runs | six runs read **0.85–1.05x** against a 1.15x ceiling | D204's estimator generalised + a 2 km fixture + a floor | shrinking the fixture back → red, naming the row |

**Suites after:** `run_native_checks` **117 `[PASS]` / 0** (was 105 at §21, 106 after `adde049`),
`run_scene_checks` 0 failing / **0 moved**, `run_2d_checks` 0 failing / **0 moved**,
`run_hda_checks` 0, `scale_gate` 9 rows / 0, `gate_images` 0, `run_attrib_checks` 0 failing /
0 unreviewed, **328 unit tests OK**, `pf_polychain.hda` back at md5 `37f1e344`, citygen untouched.

### 22.1 D210's second half — twelve claims re-checked, and none of them was wrong

`adde049` fixed the exit rule and *asserted in its own message* that the claims made under the
broken rule were true. That assertion was itself unverified, so it was re-run from scratch: all
twelve native-lane commits `b5c4468..fbf0961` exported with `git archive` into separate trees, and
**both** baseline-carrying runners replayed at each.

| commit | `run_scene_checks` | `run_2d_checks` |
|---|---|---|
| b5c4468 bec344e cd3b03a 0f7310e 7bed7b8 f5e5156 | 0 failing / 0 moved | 0 failing / 0 moved |
| a13d35f 58cbf82 85fff64 909e9cb 8ba9451 fbf0961 | 0 failing / 0 moved | 0 failing / 0 moved |

**Not one "no baselined value moved" claim in this build log was wrong.** They were *unsupported* —
the exit code structurally could not carry them — and they were also *true*. `baseline.json` has
not changed since `ac66815`, which is before the native rebuild started.

**The replay is not vacuous.** Perturbing `A_straight/element_count` 20 → 19 in b5c4468's *own*
exported `baseline.json` makes that export print

```
--- moved since baseline (check each is an improvement) ---
  A_straight/element_count: 19 -> 20

0 failing checks
```

and **exit 0** — the D210 defect reproduced at the commit that made the claim.

**And the other runner.** `adde049` named `run_2d_checks.py:437` and left it, because the phase-2
agent owned the file. That file is committed now (`54f4bbd`), so the identical rule is closed: the
diff and the exit rule are `R.baseline_movement` / `R.exit_code`, **imported** from
`run_scene_checks` rather than copied, since a copied exit rule is exactly how the two drifted.
Perturbing `FA_L_facade/element_count` 176 → 999 now exits **1**.

⚠️ **`scene_baseline_movement_fails_the_run` grew 6 rows → 9 and reads `run_2d_checks.main`'s own
source through `inspect.getsource`. Its comment-stripping is not tidiness**: the first version
matched **this fix's own comment**, which quotes the rule it removed, and reported 8/9 on a correct
build. A source assertion a comment can satisfy is D207's defect wearing a different hat.

Still not fixed, still named: `tests/citygen/run_scene_checks.py`, out of scope.

### 22.2 D206 — the batching law was decoration on fourteen of eighteen nodes

**Reproduced at source.** `pc_finalize.vfl` rewritten as a single-threaded detail loop over the
prims, `native.py`'s class moved `primitive → detail` with it, `.hda` rebuilt (`37f1e344 →
6f2fec2e`). The fence is value-for-value identical and **`run_native_checks` 106 `[PASS]` / 0,
`run_scene_checks` 0 / 0 moved, `run_hda_checks` 0** — while the node goes **2.13 → 13.15 ms at
17 804 pieces** (0.114 → 0.738 µs/piece).

The fix is **one generic loop over a measured table**, not four more bespoke checks. The instrument
is `hou.perfMon` per-node **`Cook - ms`** on the *shipped asset* — inclusive of the wrangle's
`attribvop` child, where the VEX time actually lands (a probe read a wrangle at 0.166 ms self
against 234.7 ms in its child, so self time would measure nothing). Three dirtying levers reach all
eighteen — spline nudge, kit nudge, `Stage = frames` — and **the per-node lever is derived** from
where its own minimum came from, so there is no hand-maintained lever table to drift.

Recorded from this build, min over five runs × three interleaved repetitions × three levers, at
4 454 and 17 804 pieces:

| node | small µs/p | big µs/p | growth | big ms |
|---|---|---|---|---|
...
Ceiling = big-size rate × 2.5 + **one millisecond expressed per piece**, judged at the **big size
only** (five of these are fixed-cost at 4 454 — `pc_corners` is 0.39 ms at *both* sizes, so one
per-piece ceiling cannot be right at both). The small size buys the **growth** ratio, which is
scale-free and is what catches a quadratic regression.

Five checks, each closing a different way for the table to rot:

- `every_wrangle_has_a_cost_ceiling` — **18 wrangles / 18 ceilings**, read off the built network.
  A nineteenth wrangle without a ceiling fails *here* rather than shipping unwatched.
- `wrangle_cost_is_flat_in_piece_count` — 18 rows, worst 42 % of its ceiling.
- `wrangle_ceilings_are_tight` — **worst 64 % of its allowance**; no ceiling above 4× the
  measurement (+ twice the noise floor). ⚠️ A real 40 % speedup — which is what §21.10 asks for on
  `pc_plan_solve` — **fails this check until the table is re-recorded**. That is the intent.
- `mutation_every_wrangle_ceiling_bites` — 15/18 at 5×, **3 under the floor**.
- `mutation_pc_finalize_debatched` — **1.0043 µs/piece against 0.35**.

Mutations, each reddening only what it should: `pc_finalize`'s ceiling ×10 → *tight* red at
**537 % of its allowance** and the M3 mutation red; `pc_envelope2` dropped from the table →
*coverage* red at 18 wrangles / 17 ceilings; `pc_stamp`'s ceiling set below the measurement →
*flat* red at 161 %; and **the verdict silently skipping one row → only `mutation_every_wrangle
_ceiling_bites` sees it (14/18)**, which is the hole that check exists for.

#### ⚠️ A physical burn was built first and was thrown away, and the reason is the finding

The obvious way to prove all eighteen redden is to append a workload to each wrangle in turn. It
was written and measured, and **on one machine, in one process, it read 18/18 on the first pass and
11/18 on the second and third** — identical build, identical ceilings, `pc_deform_gate` swinging
**6.94 → 0.43 µs/piece**. A loop whose body does not depend on the element is loop-invariant, so
VEX hoists it and the "total" workload collapses by the thread count; making it element-dependent
needs a per-node iteration budget, because a DETAIL wrangle and a 17 804-element POINT wrangle are
sixteen times apart on the same count.

**A mutation check that reddens on a sound build is D209's defect wearing D206's hat**, and
shipping one while closing D209 in the same cycle would be absurd. The per-node proof is
deterministic instead — `wrangle_verdict` is a pure function of the measured rates — and the one
*physical* mutation kept is M3, which measured **0.80–1.42 µs/piece across six runs** against a
0.35 ceiling.

#### Two things worth not rediscovering

- **`vex_threadjobsize = 1e9` is not a de-batching lever.** One job, one thread — it looks perfect,
  and it moves these nodes by **10–25 %** with **nothing** crossing its ceiling. They are not
  thread-bound: M3's 6.2× is the cost of `prim()` / `setprimattrib` random access in a loop, not of
  losing fifteen cores.
- **Locked and unlocked instances measure the same node** (13.459 / 13.490 ms `pc_arclength`,
  2.159 / 2.132 `pc_finalize`, 497.2 / 484.0 `pc_plan_solve`), which is what lets the table be
  recorded from the unlocked instance the mutation levers need. What unlocking costs is scene size,
  and that is `instances_do_not_fork_the_network`'s business.

⚠️ **The mutation check's exemption is derived, not listed.** A node whose whole cook is 0.16 ms
has a floor-dominated ceiling, so a 5× regression on it (0.65 ms) is genuinely below what this
instrument can see. It is computed from the floor the ceiling was built with, so a node that grows
out of the floor stops being exempt by itself — and every exempt row is still required to be
*reached* at its own ceiling.

### 22.3 D207 — the menu could claim NATIVE for the dead bridge, and eighteen headers could say anything

**Reproduced at source, twice.** `native.STAGES`' two "via the PYTHON BRIDGE" labels re-worded back
to "NATIVE" and the `.hda` rebuilt (`34fbd13a`, and `6e25f967` on the second run): the asset ships
a menu offering **two entries reading `2 - Plan, NATIVE` and two reading `4 - Frames, NATIVE`**, one
of each serving the dead Python bridge, with **`run_native_checks` 111 `[PASS]` / 0** and
`run_hda_checks` 0.

**The rule adopted: a claim token in artist-facing text is verified against the GRAPH, and the
verification is independent of the declaration the text comes from.** Reading `native.STAGES` to
check a label built from `native.STAGES` would prove nothing — that is D208, applied here in
advance.

- `stage_labels_are_true` — 10 labels. Each label's NATIVE / PYTHON claim against the Python SOPs
  actually upstream of the node it serves, walked through `inputs()` *inside* the asset; plus no
  two entries reading the same stem.
- `mutation_stage_labels_claim_native` — M8 replayed on the labels alone, 4 complaints, so the fix
  cannot be undone without a rebuild proving it.
- `every_wrangle_comment_is_checkable` — 18. Every `.vfl` header names its **class** and it must be
  the node's class; any **precision** it states must be the node's; every **`input N`** it declares
  must be wired.

⚠️ **Those header claims were already written in all eighteen `.vfl` files and nothing had ever
read one.** `pc_finalize.vfl` has said *"PRIMITIVE wrangle, precision 64 / input 0 the packed
pieces out of `copy_packed` / input 1 CONFIG"* since it was written.

Mutations: the source M8 → `plan claims NATIVE and its branch cooks pc_plan_bridge; frames claims
NATIVE and its branch cooks pc_plan_bridge; 2 entries read '2 - Plan, NATIVE'; 2 entries read
'4 - Frames, NATIVE'`. Header says DETAIL where the node is PRIMITIVE → reported. `input 1`
unplugged while the header declares it → reported. Header says precision 32 where the node is 64 →
reported. Header deleted outright → reported.

⚠️ **Three false alarms the first version raised on a correct build**, all fixed and all recorded
in the source, because a check that cries wolf is the same disease as one that cannot fail:

1. `4 - Place, NATIVE (4.4 - packed pieces, no Python)` was read as **claiming** Python. A negated
   claim read as its opposite. `NO PYTHON` is a NATIVE claim now.
2. `pc_envelope.vfl` line 44 **quotes the reference's warning text** — *"no spline on input 1"* —
   and a loose `input\s+(\d)` read it as a declaration. The pattern is exact now.
3. The upstream walk climbed **out of the asset** through the indirect inputs, so `readability`'s
   own `readable_spline` Python SOP made four NATIVE labels read as lies. It stops at the boundary.

⚠️ `config` and `kit_starter` are exempt from "is there Python upstream", by name and with reasons
(§13.6 — `config` is UI marshalling and stays; D154 — `kit_starter` should not be Python and is
**named rather than skipped**). The exemption is **live-checked**: when D154 lands and
`kit_starter` stops being a Python SOP, `stage_labels_are_true` fails until the exemption is
deleted.

### 22.4 D208 — the single declaration was its own oracle, and the headline mutation ended in a crash that printed no FAIL

**Reproduced at source.** `native.STAGES`' `reference` row re-pointed at `OUT_final` /
`guard_envelope`, `.hda` rebuilt (`92b0d456`) — §20.1's own headline mutation, the one that turns
`output_guard_parity`'s oracle into a comparison of the guarded native output *with itself* over
all 92 cases. The run reported:

```
[PASS] every_stage_entry_serves_the_node_it_names  10 entries ... Wrong: none
94 [PASS], 0 [FAIL]
AssertionError: no switch input serves OUT_final
```

**Two fixes.**

1. **The crash.** `assert len(moved) == 1` inside `stage_is_really_native` is a **complaint** now.
   The exit code was 1, so a caller checking it was safe — but every summary in this build log
   counts `[FAIL]` lines, and *94/0-with-a-traceback reads as green*. On the same mutated build the
   run now reports **114 `[PASS]` / 3 `[FAIL]`** and finishes.
2. **The second voice, for all ten stages** and not only the `reference` row §21.5 asked for.
   `NATIVE_STAGES` and `OUTPUT_STAGE` were already the independent oracle for five, and they work
   because they are **behavioural** — nodes that must cook, Python that must not — rather than a
   copy of the table. `BRIDGE_STAGES` is the same shape for the other five (`reference`, `config`,
   `plan`, `frames`, `gate`), with a **fifth column: the nodes that must NOT cook**. A stage on the
   Python side cannot be asserted by "no Python cooked", so it is asserted by what the *other*
   branch would do if the entry were re-pointed at it.

`every_stage_has_a_second_source` **10/10 stages**; `mutation_reference_unplugged` reports
*"reference: idle kernel; reference: copy_packed,pc_finalize,pc_out_cast cooked and must not"*;
`mutation_gate_unplugged` 3 complaints. Under the source mutation, `every_stage_has_a_second_source`
is the check that reddens **while `every_stage_entry_serves_the_node_it_names` still prints PASS** —
which is D208 stated as a test.

⚠️ **The fixture is the straight run and it has to be.** On the L-shape the guard *refuses*, so
`Stage = output` runs the reference and `copy_packed` never cooks whatever the entry is wired to —
the forbidden column would be vacuous on exactly the mutation it exists to catch.

⚠️ **`config` has no must-cook list, and that is a property of the node, not leniency:** it is wired
to IN_KIT / IN_STYLE / IN_SURFACE and *not* to the spline, so the dirtying lever cannot reach it.
Its row is the forbidden column alone, which still catches a re-point at either neighbour.

The fifth column is optional, so the existing 4-tuples did not have to be rewritten to add it.

### 22.5 D209 — the flaky check had two causes, and fixing one would have left it flaky

**Reproduced as a spread rather than as a single failure.** Six consecutive runs of the *unmutated*
build read `corner` at **1.00x, 0.97x, 0.96x, 1.05x, 0.95x, 0.85x** — a **24 % swing against a
15 % ceiling**. It did not tip over on those six; it tipped over on the audit's three. That is the
defect.

1. **The estimator.** Both sides were timed in **separate blocks** — all three reference passes,
   then all three output passes — so a busy moment during one block lands entirely on one side of
   the ratio. D204 diagnosed exactly this on `pc_plan_solve` and applied the cure to that one node.
   It is a function now, **`interleaved_best`**, used by `output_guard_cost` and *both*
   `decompose_*_wall_clock` rows: min over interleaved repetitions, setup outside the timer.
2. **The fixture.** The `corner` row was an 18 m × 14 m L — **three points, a 5 ms cook**. No
   estimator rescues a 1.15x ceiling on 5 ms. It is a **2 km L** now (2 001 points, one 90° corner,
   still refused by level 1), the same size as the other three rows.

Measured after, six consecutive runs: `corner` **1.03x/81ms, 1.04x/80ms, 1.02x/84ms, 1.00x/84ms,
1.03x/82ms, 1.01x/83ms`. **0.85–1.05x on 5 ms became 1.00–1.04x on 82 ms**, and the whole check's
spread across those six is 0.92–1.06x against ceilings of 1.15x and 1.6x.

The two rows §21.6 named beside it, five consecutive runs each:
`decompose_long_curve_wall_clock` **1.05 1.05 1.07 1.06 1.08x** (35 ms) and
`decompose_streets_300_wall_clock` **1.16 1.10 1.09 1.24 1.11x**, against a ceiling that needs
> 0.667x. The `0.63-1.20x` that went red once is gone; **interleaving alone was enough for these
two and neither fixture changed.**

**And the floor is an assertion now, not a habit.** Every `output_guard_cost` row must cook at
least 20 ms on the reference side or the check **fails, naming the row** — because a fixture that
shrinks turns a tight ceiling back into a coin toss *silently*. Mutation: putting the 3-point L
back reports `corner 1.02x/5ms ... the reference cooks in 0.0050 s, under the 20 ms floor - a
1.15x ceiling on it is noise, not a measurement`. The absolute time is printed on every row now
(`corner 0.99x/81ms`), so the next reader can see the size the ratio rests on.

### 22.6 The phase-2 lane's uncommitted tree, verified and committed

Seven files from the concurrent phase-2 workstream were sitting **unversioned** after a usage-limit
kill: P2-3V's six unassertable fixes (D147) — cases `FT_row_overflow`, `FU_row_kit_gap`,
`FV_area_short` with their `EXPECTED_WARNS` / `CELLS` / `UNBUILT` rows; `rows_clipped_out`; a
`clip_stamp` re-written to assert the row-curve-to-element **transfer** rather than
`area or n == 0`; `stage` added to `run_hda_checks`' `PARM_LANE_EXEMPT`; nine phase-2 unit tests;
and `conventions.md` cross-linked from `citygen.md`.

**Nothing in it was written or changed by this lane.** It was run — `run_2d_checks` 0 failing / no
movement, `run_scene_checks` 0 / 0 moved, `run_hda_checks` 0, 328 unit tests OK — and, being green,
committed as `54f4bbd` so it stopped being at risk.

### 22.7 Decisions

- **D211 — a timing check declares the size it rests on, and fails below it.** D209's cure is two
  parts and only one of them is an estimator: `interleaved_best` fixes *which* machine each side
  saw, and `GUARD_COST_FLOOR_S` fixes *whether the fixture is big enough to carry the ratio at
  all*. Any new wall-clock ratio takes both, and prints its absolute time in the value.
- **D212 — a mutation proof must be deterministic, or it is not a proof.** The physical burn read
  18/18 then 11/18 on the same build. Where a physical mutation cannot be made stable, mutate the
  **pure verdict** instead and keep one real physical mutation beside it as the anchor.
- **D213 — a claim about the build, wherever it is written, is a check.** D205 established this for
  the sticky note; it now covers the ten `Stage` labels and the eighteen wrangle headers. The
  corollary is the one that bites: **a claim that cannot be checked against the graph may not be
  made** — which is why `NO PYTHON` had to become a *native* claim rather than being ignored.
- **D214 — every single-source declaration owes a behavioural second voice, not a second copy.**
  `NATIVE_STAGES` works because it names nodes that must cook. A second table of the same three
  columns would have been moved by the same careless edit.
- **D215 — an `assert` inside a check is a crash that prints no `[FAIL]`.** Every summary in this
  build log counts `[FAIL]` lines. An impossible-state guard inside a check body returns a
  complaint; it does not raise.

### 22.8 What is NOT done — where the next audit should start

**PART B was not started.** The guard still refuses curved, cornered and conformed runs, and those
still run 88–98 % Python on the reference path; `kit_starter` still builds geometry in Python on
every cold build (D154). §21.10's list stands unchanged, and so does its headline: **`pc_plan_solve`
is 87 % of the shipped native cook at 20 km** — 469.572 ms of the 18 871-piece table above, and
everything else on the native path together is the remaining 13 %.

**Dev-loop rule 0 is NOT discharged for this cycle.** Nothing here has been audited by an agent
that did not write it. The honest words for §22 are **implemented, self-verified, unaudited** — and
the specific things an auditor should attack first are:

1. **The D206 table is a set of numbers from one machine.** `wrangle_ceilings_are_tight` refuses a
   40 % speedup by design, which means the first person to optimise `pc_plan_solve` meets a red
   check. Confirm that is a feature and not a trap.
2. **`mutation_every_wrangle_ceiling_bites` exempts rows under the noise floor.** The exemption is
   derived, but derived from a constant (`WRANGLE_FLOOR_MS = 1.0`) that nobody has challenged.
3. **`BRIDGE_STAGES`' forbidden columns are hand-chosen.** `copy_packed` and `kernel` are the right
   pair on a straight run; nothing proves they are the right pair on every re-point.
4. **`every_wrangle_comment_is_checkable` reads only the `.vfl` header**, not the node's
   DisplayComment, which is a *separate* string authored in the build script and still asserted
   only to be non-empty.
5. **§22 ran ten source-level rebuilds of the `.hda`** and restored it each time by copying a
   backup. The md5 is `37f1e344` and `git status` is clean — verify that independently.

---

## 23. Native network build log — cycle N-4B (PART B: widening the guard, in measured order)

**Read this before §22.** §22 repaired the evidence machinery and explicitly did not start PART B.
This cycle is PART B: measure which refused build classes cost the most Python, port them in that
order, keep the guard failing safe, and say plainly which classes still take the reference.

**The rule this cycle was run by, and it is the reason the order below is not the order the brief
proposed: measure first, then port.** The brief's order was curved → cornered → conformed →
`kit_starter`. The measurement re-ordered three of the four and *declined* the fourth.

### 23.0 The scoreboard

| # | class | Python before | after | what it took |
|---|---|---|---|---|
| 1 | **gently CURVED runs** (arcs, curved streets) | 490 ms / 54 ms | **0** | level 1's deform bound, widened |
| 2 | **MARKED runs** | 212 ms / 52 ms | **0** | D88's warning moved to `pc_sections` |
| 3 | **PADDED runs** (the Gap parm) | 211 ms / 53 ms | **0** | one branch in `config_resolved` |
| — | **conformed** (a surface) | 954 ms | 954 ms | N6, **not attempted** — takes the reference |
| — | **elevation** (ripples, hills) | 1 349 ms | 1 349 ms | N5, **not attempted** — takes the reference |
| — | **cornered** | 336 ms | 336 ms | N8, **not attempted** — takes the reference |
| — | `kit_starter`, cold | 1.5–1.8 ms | 1.5–1.8 ms | **D154 DECLINED, with the measurement** |

**Suites after:** `run_native_checks` **122 `[PASS]` / 0** (was 117), `run_scene_checks` 0 failing /
**0 moved**, `run_2d_checks` 0 / **0 moved**, `run_hda_checks` 0, `scale_gate` 9 rows / 0,
`gate_images` 0, `run_attrib_checks` 0 / 0 unreviewed, **350 unit tests OK**. `output_guard_parity`
went **9 native / 83 reference → 18 / 74, identical**. Three commits: `7ece224`, `12d89c0`,
`1c167b4`.

### 23.1 The measurement that set the order — and it disagreed with the brief

`hou.perfMon` per-node `Cook - ms` on the **shipped asset** at `Stage` = its default, minimum over
three dirtied cooks. Every row is a build the guard refused, and the number is the Python that
cooks on it.

| refused class | fixture | Python | level 2's verdict if level 1 were forced open |
|---|---|---|---|
| **conformed streets** | 300 straight streets + a terrain | **772 ms** | n/a — there is no native conform at all |
| **hilly streets** | 300 streets, gentle elevation | **1 217 ms** | REFUSE — 17 400 planned / 9 000 built |
| **curved streets** | 300 short streets, R ≈ 280–520 m | **490 ms** | **ADMIT — 17 400 / 17 400** |
| **cornered streets** | 300 streets with a 90° bend | **283 ms** | REFUSE — 9 600 / 9 300 |
| **marked streets** | 300 straight streets + markers | **212 ms** | (level 1's own refusal) |
| **padded streets** | 300 straight streets, Gap 0.05 | **211 ms** | (level 1's own refusal) |
| conformed 2 km | a straight run + a terrain | 182 ms | n/a |
| ripple 2 km | | 132 ms | REFUSE — 1 892 / 947 |
| corner L 2 km | | 53 ms | REFUSE — 1 890 / 1 889 |
| **arc 2 km** | R = 2 000 m | **54 ms** | **ADMIT — 1 888 / 1 888** |
| `kit_starter` | cold build, no kit wired | 1.5–1.8 ms | n/a |

**Three things in that table changed the plan.**

1. **The citygen shape is not the cornered one, it is the CURVED one.** `streets_300` in the bench
   suite bends 90° at its middle vertex, so it is refused for a corner whatever the deform bound
   says. A real street network mostly *curves*. That row is 490 ms — the second-largest refused
   class — and **level 2 already admits every piece of it**. It needed no geometry code at all.
2. **The two cheapest refusals were not about geometry.** A marker cost 212 ms to keep one warning
   string; the Gap cost 211 ms because of a sentence about the kit that a `grep` contradicts.
   Neither appears in §21.10's list at all.
3. **`kit_starter` is the smallest item in the table by two orders of magnitude**, and the case for
   porting it does not survive measurement (23.5).

### 23.2 Curved runs — level 1's bound changed CATEGORY, and that is the finding

Level 1's deform test was *"a curve with no KINK and no elevation change cannot produce a deformed
piece"* — an **upper** bound on `span_deviation`: exact, free, and it refuses every arc in
existence.

**But level 1's bend question was never load-bearing for correctness.** Level 2 asks
`pc_deform_gate` the exact question, per piece, and counts `npoints(gate) == npoints(packed)`.
Level 1's only job is not cooking the native chain for a build that is going to be refused anyway —
it is a **cost heuristic**. So what belongs there is a **lower** bound: *will a piece CERTAINLY
unpack.* Both ways of being wrong are safe:

* refuse where level 2 would have admitted → the reference cooks, the output is right, one native
  opportunity is missed;
* admit where level 2 refuses → the double cook, measured **1.21x and 1.27x** on the two ladder
  rows that reach it, against `bench_guard_fallback`'s 1.8x ceiling.

The bound is `place.span_deviation`'s own two terms on the **sharpest kink anywhere in the build**
against the **widest module in the kit**:

```
dev = 0.25 * span * sin(min(theta, pi/2))   +   2 * radius * sin(theta / 2)
       the SPINE term                            the OFF-SPINE term (D87)
```

A vertex sits `(p*q/W)·sin θ` off the chord of a span `W = p + q`, worst at `p = q`, hence
`0.25·W·sin θ`; a point `radius` metres off the spine swings by the chord of the frame's own
rotation. `min(θ, π/2)` inside the spine sine because `sin` turns over there and a bound may not
shrink as its input gets worse.

#### ⚠️ THE OPERANDS ARE THE FINDING, NOT THE FORMULA

The first version took the kit's **raw maxima** and refused the 300 curved streets at a bound of
**0.0258 m against a 0.01 m tolerance** — while level 2 admitted all 17 400 pieces. Two of the
reference's own rules were being ignored, and each one narrows the set:

* **D27 — a RIGID module never deforms.** `_needs_deform` returns False at `module.deform <= 0`
  before it measures anything. The starter kit's widest module by far is `corner_post` at radius
  **1.3025 m**, and it is rigid; it was setting the bound for every build in the tool.
* **D87 — a yaw-only mode reaches off the spine by its Z HALF ONLY.** The gate spends the budget
  with `radius = (zmode == "adaptive") ? hypot(ry, rz) : rz`. Every deformable module in the
  starter kit is `vertical`, so the honest reach is **0.03–0.04 m**, not 1.00–1.10.

With both applied the bound tracks the exact answer instead of shadowing it:

| build | turn (rad) | bound | level 1 | level 2 |
|---|---|---|---|---|
| straight 2 km | 0.0 | 0.00000 | admit | admit |
| arc R = 2 000, 2 km | 0.00108 | 0.00058 | admit | admit |
| arc R = 400, 2 km | 0.00505 | 0.00273 | admit | admit |
| arc R = 200, 2 km | 0.01002 | 0.00541 | admit | **admit — 1 888 / 1 888** |
| arc R = 100, 2 km | 0.02001 | 0.01080 | **refuse** | — |
| arc R = 50, 2 km | 0.04001 | 0.02160 | **refuse** | — |
| 300 curved streets | 0.01431 | 0.00773 | admit | **admit — 17 400 / 17 400** |
| ripple 2 km | 0.0 | 0.00000 | **refuse (elevated)** | — |
| corner L 2 km | 1.57080 | 0.55657 | **refuse (corner + bound)** | — |

**Elevation stays an outright refusal**, and that is measured too: a `vertical` piece deforms on any
sheared span at 1e-6 m, and the rippled and hilly shapes are the ones that pay the double cook
worst (300 hilly streets plan 17 400 and build 9 000). Those need N5, not a wider bound, so they are
refused where the refusal is free.

**And a bound that could not be DERIVED is a refusal, not a zero.** An empty or unreadable kit
yields span 0 / radius 0, and a bound of zero is under every tolerance there is — the fail-OPEN
answer that shipped 100 % wrong modules twice (§20.1's `hou.Vector2` cond value, §20.2's `attr:` on
a multi-component prim attribute). `_bend_bound` returns a third value saying whether the pair was
derived, and `pc_envelope` refuses outright without it.

#### The fail-safe path is exercised deliberately, because it is real

The bound reads **one** kink, so a finely resampled arc puts several inside one span and it
**under-reads**. `GUARD_BEND_LADDER` drives that band on purpose: R = 20 m at 0.05 m spacing bounds
the deviation at 0.00142 m and level 2 then finds **284 planned / 143 built**. `guard_bend_bound`
compares `Stage = output` with `Stage = reference` element for element on every row, including
those two — **identical**, at 1.21x and 1.27x.

⚠️ `no_case_pays_the_guard_fallback` says no case in the 92 reaches the double cook. **That is a
statement about the cases, not about the guard**, and it would stay green if the fallback path were
broken. The ladder is what stops it being the only evidence.

### 23.3 A marker cost 212 ms of Python to keep one sentence

4.2's marker anchors have been at parity since N2 — `plan_solve_parity` runs the marker cases. The
envelope refused every marked build anyway, because **D88's unread-marker warning is raised by
`hda.cook` on the `kernel` node**, which a native output never cooks: `unread_marker_warns` went red
the moment the guard took a marked straight run, so the refusal was put in to keep it.

The sentence moved instead, into **`pc_sections`** — and the placement is the whole of the design:

* it is the **first node of the native plan chain**, so it only cooks when level 1 has admitted the
  build, which is exactly when `kernel` is silent. In the decompose it would fire beside `kernel`'s
  own on every refused build and the artist would read it twice;
* it is the **last node that can still see a marker point** — `pc_sec_only` blasts them one node
  down, so §19.7's nomination of `pc_plan_emit` cannot work;
* it is **DETAIL**, so it can aggregate the ids into one sentence, which a point wrangle cannot;
* a new node would have cost a `WRANGLE_CEILING_US` row, a `.vfl` header and a `NATIVE_STAGES` row
  to say the same thing in the same place.

The message is `_warn_unread_markers`' own, word for word, including which of the two faces authored
the style — a warning phrased differently depending on which branch cooked is a warning an artist
cannot search for.

⚠️ **`hou.Node.warnings()` ON THE HDA INSTANCE IS EMPTY IN EVERY CASE.** Measured: a native build, a
reference build, and even the `no spline on input 1` build that every reader would expect it to
carry. Houdini propagates the warning **state** up the hierarchy for the badge in the network
editor, not the text through HOM. Every check in this suite has always read an **inner** node; what
PART B changed is which one, and `unread_marker_warns` now asserts **both** paths and that they are
identical (`pc_sections` on the native build, `kernel` on a cornered one, with `copy_packed` /
`kernel` `cookCount` proving which cooked).

**One new refusal, because dropping the blanket one exposed a real divergence.**
`place.read_curves` **SKIPS any primitive containing a marker point** —
`if any(p.number() in marker_pts for p in pts): continue` — so a marker authored ON a curve vertex
makes the reference build no curve there while the decompose builds one, sampler table and all.
That is `T2_marker_in_prim`; level 2 caught it, which is the guard working, but only after the whole
native chain had cooked, and `no_case_pays_the_guard_fallback` went 0 → 1 of 92. `pointprims` at
level 1 is the same question from the other side, and the row is back to **0 of 92**.

### 23.4 The Gap refusal was one `grep` wrong

`_native_ok` refused any non-zero Gap because *"D91's kit padding rewrites the KIT, and `cook_kit`
does not do it"* — 53 ms on a 2 km run, 211 ms on 300 streets. But **nothing in the native chain
reads the kit geometry's `pc_pad`**: grepping it over `polyfactory/vex/polychain/` returns exactly
one hit, `pc_plan.h` reading CONFIG's own flattened `pc_k_pad0` / `pc_k_pad1` columns. Padding the
kit in `config_resolved` *before it is flattened* is the whole port — the solve reads the padded
numbers and `copy_packed` still gets the unpadded geometry, which is what it wants.

It is done in **`cook`'s own order**, not a tidier one: the style is read against the UNPADDED kit
and the padding applied afterwards, so a payload's validation warnings are still about the kit the
artist wired. And D91's own rule holds — a wired payload is never padded.

`guard_padding_parity` sweeps four values **including negative overlaps** (a different branch of the
solve, D17) against the reference element for element: **182 / 138 / 200 / 266 prims, identical**.
No scene case sets this parm, so nothing else in the suite could see it — and *"parity cases that
cannot reach a code path prove nothing"* has cost this project three criticals already.

### 23.5 D154 — DECLINED, with the measurement that declines it

§13.3.6 and §21.10 both list `kit_starter` as Python that must go native. Both halves of the case
turn out to be wrong.

1. **It does not re-cook.** `cookCount` stays at **1** through two spline nudges, `bend_tol`,
   `padding`, `seed` and both `Stage` changes — while `config` beside it goes 1 → 4 on the same
   sequence. **1.50–1.83 ms, 1.4 % of ONE cold build and 0 % of every cook after it.**
   `houdini-procedural-modeling` rule 1 forbids per-element geometry in a node that **re-cooks** and
   explicitly permits a Python SOP for a trivial one-off constructor. This is the second thing.
2. **The prescribed mechanism does not preserve the geometry.** §13.3.6 says "four `box` SOPs +
   `pack`". Probed on 22.0.398: a Box SOP at `type = polymesh, divrate1 = 9` gives **34 four-sided
   prims and 36 points** — exactly `box_mesh(divx=8)`'s counts — and D33's centroid-dot-normal test
   scores **0 inward faces on both**. But the point **set**, the point **order** and the vertex
   **order** all differ: the Box SOP lays points out per FACE, `box_mesh` in 4-point RINGS along x.
   Every module is packed and copied by `copytopoints`, so swapping the builder re-orders the points
   of **every element the tool ships** and moves `geometry_digest` on every case in the suite.

⚠️ **`box_mesh`'s own docstring was STALE and pointed the other way**, which is why this had to be
probed rather than read. It said the Box SOP's division mode *"emits 24 two-point polygons alongside
the faces (measured on 22.0.398), which is not a solid"*. It does not — not on this build, at these
parms. A reader checking that sentence finds it false and concludes the hand-build is obsolete,
which is the opposite of the truth. The real reason is **order**, and it says so now.

`kit_starter_cooks_once` pins the **premise**, not the conclusion: a decision resting on "it only
cooks once" is worth exactly what that sentence is worth, and nothing was watching it. `config` is
the control and must advance, or "stayed at 1" is also what a node that never cooked would report.

### 23.6 The Python share, before and after

`hou.perfMon`, shipped asset, `Stage` at its default, dirtied through a parm **every** stage reads
so `config` is in the denominator — the same basis as §21.2's table.

| build | §21.2 (before) | **after PART B** | what cooks |
|---|---|---|---|
| straight 2 km | 3.6 % | **2.5 %** | `config` 1.60 ms |
| straight 20 km | 0.4 % | **0.2 %** | `config` 1.61 ms |
| **arc R = 2 000, 2 km** | **96.0 %** | **2.3 %** | `config` 1.61 ms |
| **arc R = 2 000, 20 km** | (refused) | **0.2 %** | `config` 1.64 ms |
| **arc R = 400, 2 km** | (refused) | **2.3 %** | `config` 1.63 ms |
| corner L, 2 km | 88.2 % | 91.8 % | `kernel` 54.91 + `config` 1.67 |
| corner L, 20 km | — | 94.9 % | `kernel` 596.03 + `config` 1.85 |
| conformed straight, 2 km | — | 97.2 % | `kernel` 162.33 + `config` 1.78 |
| conformed straight, 20 km | — | 98.4 % | `kernel` 1915.81 + `config` 1.64 |

**On a warm re-cook driven by the SPLINE alone the native rows are 0.0 %** — `config` is wired to
IN_KIT / IN_STYLE / IN_SURFACE and not to the spline, so it does not re-cook. Both numbers are true
about different things and both are printed rather than the flattering one being chosen.

End-to-end, `output_guard_cost` on the same node:

```
straight_2km 0.91x/79ms native; arc_2km 0.93x/85ms native;
curved_streets_300 0.23x/512ms native; arc_R50_2km 1.02x/164ms;
bumpy_2km 1.02x/166ms; ramp_2km 1.00x/161ms; corner 1.05x/81ms
```

**The citygen shape reads 0.22–0.23x — a 4.4x speedup** — because the reference pays a per-curve
fixed cost 300 times and the native chain pays a per-node one once (§11.9 rule 2, from the other
side).

### 23.7 It was looked at

`gate_images.py` renders the curved run PART B flipped, from **both** chains, cropped to 12 m of a
90 m R = 60 m arc — because a 90 m arc drawn whole is a one-pixel line, which is §18.4's defect in a
new place. `PARTB_arc_native_side.png` shows alternating 1.20 m posts and 0.90 m panels on the
0.25 m station ladder, evenly spaced, nothing collapsed; `PARTB_arc_native_top.png` shows the fence
following the arc as a chain of rigid chords, which is what "every piece stays packed" must look
like (the chord sagitta at R = 60 over 2 m is 0.0083 m, under the 0.01 m tolerance — which is
exactly why the guard admits it). `partb_curved_is_native` asserts `copy_packed` advanced, because
otherwise the pair is satisfied by the reference rendered twice.

### 23.8 Mutation — eleven at source with the `.hda` rebuilt, THREE SURVIVED, all three closed

| # | mutation | verdict | what saw it |
|---|---|---|---|
| M-B1 | the widening reverted to the old boolean | KILLED | `output_guard_cost`, `guard_bend_bound`, `guard_bend_bound_needs_its_operands` |
| M-B2 | the bound loosened 10x | KILLED | `no_case_pays_the_guard_fallback`, `output_guard_cost` (`arc_R50_2km` **1.37x** over 1.15), `guard_bend_bound` |
| M-B3 | the ELEVATION refusal dropped | KILLED | `no_case_pays_the_guard_fallback` (4 of 92), `output_guard_cost` (`ramp_2km` **1.51x**) |
| M-B4 | D27 ignored — a RIGID module sets the bound | ⚠️ **SURVIVOR** | nothing → `guard_bend_bound_skips_rigid_modules` |
| M-B5 | D87 ignored — the full radius for every mode | KILLED | `output_guard_cost`, `guard_bend_bound` |
| M-B6 | an unreadable kit claims its bound was derived | ⚠️ **SURVIVOR** | nothing → the pure verdict, asserted directly |
| M-B7 | the native marker warning deleted | KILLED | `unread_marker_warns` — `SILENT / kernel` |
| M-B8 | the blanket marker refusal put back | KILLED | `unread_marker_warns` — `kernel / kernel` |
| M-B9 | the marker-in-prim refusal dropped | KILLED | `no_case_pays_the_guard_fallback` — 1 of 92 |
| M-B10 | the Gap port removed | KILLED | `guard_padding_parity` — red on all four values |
| M-B11 | **a WIRED PAYLOAD padded too** (D91 reverted) | ⚠️ **SURVIVOR** | nothing → `parms_inert_under_payload_native` |

**M-B4 — two correct rules and only one of them load-bearing.** Deleting D27's skip left the suite
at 0 `[FAIL]`, because the starter kit's rigid modules are all `stepped` and D87 had already cut
them to their z half; they were not the widest thing in the kit any more. The kit that separates
them is `corner_post` re-tagged `adaptive` — rigid, and 1.3 m wide — on an R = 200 m arc: **0.0054 m
with D27, 0.018 m without**, against 0.01.

**M-B6 — a branch no fixture can reach.** No kit broken enough to make `_bend_bound` raise gets past
`kit.validate` and `_native_ok`'s warning refusal first. D212's remedy applies: the **pure verdict**
is called directly with an unreadable source and asserted to be `(0.0, 0.0, 0.0)`, with the starter
kit beside it so the row is not satisfied by a function that refuses everything.

**M-B11 is the one worth keeping.** `parms_inert_under_payload` exists to catch exactly this — D107's
comment says so — and could not, because **its fixture asks for `corner_mode='miter'` on a cornered
spline, which level 1 refuses**. Every parm it sweeps is therefore read by `hda.cook`, and D77's
guarantee **on the native chain** was never tested. PART B is what made that matter: the parm lane
has a second implementation behind it now. `parms_inert_under_payload_native` sweeps all 40 parms on
a straight run the guard ADMITS, asserts `copy_packed` cooked as part of the verdict, and reports
`moved: padding` under the mutation.

### 23.9 A flaky check found on an UNMUTATED build, and fixed

`decompose_streets_300_wall_clock` read **0.53x, 0.92x, 0.94x, 1.12x** across four consecutive runs
of a correct build, against a ceiling that needs > 0.667x — and the 0.53x run went **red**. D209
fixed the *estimator* on both `decompose_*_wall_clock` rows and gave `output_guard_cost` a 20 ms
floor, but left this row resting on a **1.3–2.6 ms** fixture. That is D209's defect with only half
the cure applied, and D211 is the rule it broke.

The **curve count** is raised, not the points per curve — the whole point of the fixture is §11.9
rule 2's per-curve fixed cost, and lengthening the streets would measure something else. 6 000
3-point streets, with the floor asserted. It reads **1.55x, 1.61x, 1.55x** across three runs now,
reference side 25–27 ms.

⚠️ **And the bigger fixture says something the small one could not.** At 300 curves the native
decompose read 0.92–1.12x — par. At 6 000 it reads **1.55–1.61x faster** than the reference's best
side. §11.9 rule 2 is confirmed from the other direction: the reference's per-curve cost is what
scales, and a fixture too small to see it was also too small to carry a ratio.

### 23.10 Decisions

* **D216 — level 1's bend test is a COST heuristic and holds a LOWER bound; level 2 holds the
  answer.** Level 2 asks `pc_deform_gate` the exact question per piece. Level 1 exists only to avoid
  cooking a chain that is going to be discarded, so it may be wrong in either direction as long as
  neither direction can ship a wrong fence — and both are asserted (`guard_bend_bound`), not argued.
  This is the general shape for every future widening: *the exact test lives downstream; the cheap
  test upstream is allowed to be approximate, in a named direction, with the cost measured.*
* **D217 — a guard bound must be built from the reference's OWN exclusions, not from the kit's raw
  maxima.** D27 (rigid never deforms) and D87 (a yaw-only mode reaches by its z half) are the two
  that mattered here, and ignoring them made the bound 3.3x too pessimistic — it refused the class it
  was written to admit. A bound that does not know what the thing it bounds knows is not
  conservative, it is useless.
* **D218 — a refusal that exists to preserve a WARNING is a refusal in the wrong place.** The marker
  cost 212 ms of Python for one sentence. Move the sentence to a node the native chain cooks; put it
  in the branch that only cooks when the guard admits, so it cannot double up with the reference's.
  Every remaining `kernel`-raised warning is a candidate for the same treatment.
* **D219 — D154 is DECLINED.** `kit_starter` cooks once per instance, ever (1.4 % of one cold build,
  0 % thereafter), and the prescribed `box` SOP does not reproduce `box_mesh`'s point order, so the
  port would move `geometry_digest` on every case for 1.5 ms. Rule 1 permits a Python SOP for a
  one-off constructor. `kit_starter_cooks_once` pins the premise; the day it re-cooks, D154 reopens.
* **D220 — a claim in a docstring about how Houdini behaves is a claim about the BUILD, and it goes
  stale.** `box_mesh`'s recorded reason ("24 two-point polygons") is false on 22.0.398 and would have
  led the next reader to the opposite conclusion. D213 said a claim about the build is a check;
  where a check is not practical, the claim must at least be **re-probed** before it is relied on,
  and corrected in place when it fails.
* **D221 — a parity sweep must assert WHICH implementation it swept.** `parms_inert_under_payload`
  was green for two cycles while testing only the reference, because its fixture is a shape the
  guard refuses. Any check that exists to police a behaviour now has two implementations behind it;
  it must either run on both or say which one it ran on. `took_native` is part of the verdict, not
  a print.

### 23.11 What is NOT done — where the next audit should start

**Three classes still take the reference, and that is correct rather than a gap** — the guard fails
safe on all three and the output is the reference's, bit for bit:

1. **Conform (N6) — 954 ms, the largest remaining by 3.4x.** §13.3.5's design stands (`ray` as a
   node + a `pc_drop` wrangle), but it is bigger than that design admits: `ConformPath` wraps
   `place.Path`, so the drape reaches the **frames, the plan positions, the deform gate and
   `span_deviation`'s two unported terms** (D100's camber rotation and D104's extra stations) at
   once. `pc_gate.h` says so in its own header. It is a cycle, not a stage.
2. **The deformed branch (N5) — 1 349 ms.** Ripples and hills. The hottest node in the tool would
   live here.
3. **Corners (N8) — 336 ms.** Unchanged, still the least certain thing in the document.

**Also still refused, and now the cheapest three left:** `show_warnings` (54 ms — `colour_warnings`'
`Cd`, a wrangle), `display = proxy|plan` (50 ms — D81/D82, and the proxy needs a box kit built in
the graph, which is D154's problem wearing a different hat), `fill = tile` (58 ms — N7's slice caps).

**`pc_plan_solve` is 85 % of a 20 km SINGLE-CURVE cook and 6 % of a 300-street one.**
⚠️ **CORRECTED IN §27 (D261). The sentence that stood here — "still 87 % of the shipped native
cook at 20 km … still the whole performance story" — is true of exactly one shape, and it is not
the shape citygen builds.** Re-measured this cycle with `hou.perfMon` per-node `Cook - ms`, min over
3 dirtied cooks on the shipped asset (`scratchpad/pyshare.py`, §27.3):

| shape | `pc_plan_solve` | `pc_order` | `pc_plan_emit` | total |
|---|---|---|---|---|
| 20 km straight | 865.0 ms (**84.8 %**) | — | 51.8 ms (5.1 %) | 1 019.7 ms |
| 20 km hilly (deformed) | 801.2 ms (63.5 %) | 135.2 ms (10.7 %) | 54.6 ms (4.3 %) | 1 261.3 ms |
| 300 hilly streets | 8.6 ms (**6.2 %**) | **34.3 ms (24.8 %)** | 16.5 ms (11.9 %) | 138.2 ms |
| 1 200 hilly streets | 30.5 ms (6.6 %) | **140.6 ms (30.4 %)** | 59.4 ms (12.8 %) | 463.0 ms |

The solve parallelises across sections (D222), so its share collapses on the shape a city is made
of. An implementer who reads the old sentence and spends a cycle on `pc_plan_solve` optimises 6 %
of the citygen cook. **`pc_order` (the sort that puts `place.build`'s job order back) and
`pc_pieces` (the merge) carried no ceiling of any kind until `bench_deform_20km` (D252).**
⚠️ `wrangle_ceilings_are_tight` will go RED on the first real speedup in any of them until the
D206 table is re-recorded — that is deliberate (§22.2).

**Dev-loop rule 0 is NOT discharged for this cycle.** Nothing here has been audited by an agent that
did not write it. The honest words for §23 are **implemented, self-verified, unaudited**, and the
specific things an auditor should attack first are:

1. **The bend bound's SPAN operand is the module's nominal length**, not the span the fill actually
   produces — `adaptive` can stretch a module to ~2x. Under-reading the span makes level 1 ADMIT
   more, which level 2 answers exactly, so it is the safe direction — but nobody has measured how
   often it lands there, and `no_case_pays_the_guard_fallback` being 0 of 92 is evidence about the
   92, not about the bound.
2. **`GUARD_BEND_LADDER` is six arcs on one machine.** The two fallback rows are the only proof the
   double-cook path is safe, and both are the same shape (a finely resampled arc). A different way
   for the bound to under-read would not be caught by either.
3. **The marker warning is asserted on two fixtures.** `unread_marker_warns` proves `pc_sections`
   and `kernel` agree on one straight and one cornered run; it does not prove they agree on a
   marker whose id is 0 (the attribute's own default), or on a payload-authored `marker:<id>`.
4. **`_bend_bound` resolves the zmode itself** (`params.zmode or module.zmode`, falling back to
   `adaptive`). That is `plan._zmode` transcribed, and a transcription is exactly what D214 says
   needs a second voice — there is none.
5. **§23 ran eleven source-level rebuilds of the `.hda`** and restored each by copying a backup.
   Verify `git status` is clean and the committed asset is the one the checks were run against.

## 24. Native network build log — cycle N-4C (the §23 audit, worked through)

**Read this before §23.** §23 shipped PART B — three build classes flipped to the native chain —
and said in as many words that dev-loop rule 0 was **not** discharged for it. An independent audit
of that build returned **twenty findings**. This section is all twenty: each one reproduced or
disproved with the measurement, each fix a standing check, each check mutated.

**The shape of the cycle, in one line: the guard was admitting builds it could not answer, because
3.1 names the spline's attributes and never named their STORAGE.**

### 24.0 The scoreboard

| # | finding | verdict | closed by |
|---|---|---|---|
| 1 | `wrangle_cost_is_flat_in_piece_count` is flaky on a sound build | **reproduced** | the two SIZES are interleaved now (§24.2) |
| 2 | `BRIDGE_STAGES`' forbidden column is blind to a rename | **reproduced** | a missing name is a complaint (§24.3) |
| 3 | `run_2d_checks`' D210 rule is proved by `grep`, not by running it | **reproduced** | both runners' `main` are CALLED (§24.4) |
| 4 | the wrangle mutation scales only the big size | **reproduced** | both sizes, plus a physical burn (§24.2) |
| 5 | `_bend_bound` fails OPEN on an empty kit | **reproduced** | `(0, 0, 0)` unless a module was inspected |
| 6 | D88's warning is raised TWICE on the L1-admit/L2-refuse class | **reproduced** | it moved to `pc_warn_collate` (§24.5) |
| 7 | `pc_envelope.vfl` cites a check that does not exist | **reproduced** | `every_cited_check_exists` (§24.6) |
| 8 | an int `edge_id` / `pc_curve_id` ships a different fence | **reproduced, CRITICAL** | level 1 refuses the STORAGE (§24.1) |
| 9 | an int `pc_style` ships the wrong module, 100 % of the run | **reproduced, CRITICAL** | ditto |
| 10 | an int `pc_yclass` builds 1D where the reference builds 2D | **reproduced** | ditto |
| 11 | a string `pc_marker_id` never fires the `marker:` rule | **reproduced** | ditto |
| 12 | `marker:03` warns natively and is SILENT in the reference | **reproduced** | `_check_slot` asks for the CANONICAL form |
| 13 | the guard's refusal list still claims Kit Padding | **reproduced** | the list is TAGGED and checked (§24.6) |
| 14 | `pc_plan_solve` parallelises per SECTION — 30x on one long curve | **reproduced** | measured, `plan_solve_section_shape`, **not fixed** (D222) |
| 15 | ~2/3 of `pc_plan_solve` is the five string columns | **DISPROVED** | §24.7 — the method measures dead-code elimination |
| 16 | a kit whose names no rule matches costs 2.35x | **reproduced** | level 1 refuses it (§24.1) |
| 17 | `bench_guard_fallback` says the fallback path is unreachable | **reproduced** | both comments rewritten |
| 18 | `kit_starter` promises a port the doc DECLINED | **reproduced** | rewritten + `every_python_sop_comment_is_checkable` |
| 19 | `config`'s comment names the wrong N | **reproduced** | rewritten + `config_cost_per_module` |
| 20 | `pc_plan_bridge` promises its own deletion | **reproduced** | rewritten + the same check |

**Suites, all from a pristine `git archive HEAD` export:** `run_native_checks` **130 `[PASS]` / 0**
(was 122), `run_hda_checks` **31 / 0**, `run_scene_checks` **3 665 / 0 failing, 0 moved**,
`run_2d_checks` **633 / 0 failing, 0 moved**, `scale_gate` 9 rows / 0, `gate_images` 0,
`facade_images` 0, `run_attrib_checks` 0 / 0 unreviewed, **350 unit tests OK**.

### 24.1 THE STORAGE WAS THE FINDING — four shipped divergences in one shape

3.1 names the spline attributes. It does not name their **storage**, and Python and VEX disagree
about what an unexpected storage MEANS. `place.read_curves` reads whatever the artist authored and
converts it (`str(cid)`, `int(... or 0)`, `float(u)`); VEX asks for a typed value and gets **the
type's ZERO** when the storage does not match — `""` for a string read of an int, `0` for an int
read of a string. Zero is a legal value in every one of them, so nothing errors and nothing warns.

Reproduced on the shipped `pf_polychain`, each with **level 1 admitting and `copy_packed` cooking**:

| authored | native | reference |
|---|---|---|
| prim `edge_id` **int** 7 / 3 on two lines | `pc_curve_id` `'0'`, `'0'`; `pc_elem_id` `0\|0\|start\|…` | `'3'`, `'3'`; `3\|0\|start\|…` — **and the curves emitted in a different ORDER**, because `decompose_all` sorts on `str(curve_id)` |
| prim `pc_style` **int** 5, rule `attr:pc_style eq "5"` | `{'panel': 10}` | `{'gate': 12}` — **100 % of the run is a different module** |
| prim `pc_yclass` **int** 2 | a 1D run; `pc_array`/`pc_cell`/`pc_clipped`/`pc_row`/`pc_yclass` **absent from the output schema** | a 2D run, `pc_cell='start_2'`, `pc_row=-1` |
| point `pc_marker_id` **string** `"3"`, rule `marker:3` | `{'panel': 10}` — **no gate at all** | `{'panel': 9, 'gate': 1}` |

**Nothing in the suite could see any of it, and the reason is uniform.** Every case in `cases.py`
authors these at the storage VEX happens to want, and `edge_id` — D29's SECOND rung, added for the
streets stream — had **zero occurrences anywhere under `tests/`**. A parity case that cannot reach
a code path proves nothing (the third time that sentence has been written in this document).

**THE ANSWER IS A REFUSAL AND NOT A COERCION.** Reproducing Python's `str()` in VEX is a trap —
`str(2.0)` is `"2.0"` where `sprintf("%g")` is `"2"` — so a ported coercion would be a NEW
divergence wearing the fix's clothes. `pc_envelope.vfl` carries a table of eleven (name, class,
wanted storage) rows; a name present at the wrong storage refuses the build at level 1 and it takes
the reference, which is the guard's own law. The test is O(1) — a Houdini attribute is
geometry-wide, so one `attribtype` call decides for the whole build.

`guard_spline_attr_types` asserts BOTH halves on every row (refused **and** identical to the
reference) plus a CONTROL — a **string** `edge_id`, which must still take the native chain and must
still agree with the reference. `mutation_spline_attr_types` blanks each row of the table in turn:
**5 of 5 bite.**

**AND THE ORDINARY ARTIST KIT COST 2.35x (finding 16).** The parameter page's slot defaults name the
STARTER kit's modules (`slot_default = "post panel"`, `slot_corner = "corner_post"`). Wire your own
kit into input 2 and leave the slots alone — which is how a kit arrives — and every row of
`_native_ok` passed: no surface, no fillet, and **no warning**, because the parm face never
validates the style against the kit (only `style.read` does, and only for a wired payload). So
level 1 admitted, the native chain planned **18 870** pieces and built **ZERO** (`pc_place_valid`
drops a piece whose module `pc_proto` could not resolve), level 2 refused on `planned != built`,
and the reference cooked on top of the discarded work: **`Stage = output` 1.507 s against
`Stage = reference` 0.642 s = 2.35x**, over `GUARD_FALLBACK_CEILING`'s 1.8x, with `node.warnings()`
empty. `_native_ok` takes `style.read`'s own test now (`kit.by_name(name) is None and not
kit.by_role(name)`) so the parm face refuses exactly what the payload face already warns about, and
`guard_kit_mismatch` measures **0.98x at 18.9 km** with D211's floor on the reference side.

**And `_bend_bound` failed OPEN on an EMPTY kit (finding 5).** `H._bend_bound(Kit('empty', 1, []),
{}, DEFAULTS)` returned `(0.0, 0.0, 1.0)` — a bound of zero, flagged **DERIVED**, which
`pc_envelope.vfl` reads as *admit any curvature*. Both that file's warning block and
`guard_bend_bound_needs_its_operands`' own docstring say in as many words that this is the case the
flag exists to prevent; only the UNREADABLE branch was implemented and only it was asserted. It
counts what it INSPECTED now, and the check has three rows: unreadable → `(0,0,0)`, **empty →
`(0,0,0)`**, and **all-rigid → `(0,0,1)`**, which is the deliberate derived zero the docstring
argues for and which stops the fix being satisfied by a function that refuses everything.

### 24.2 The check that closed D209 had D209 inside it

`wrangle_cost_check` built the small rig, measured it over nine cooks, **destroyed it**, and only
then built and measured the big one. That is verbatim D209's *"a stopwatch in two separate blocks"*
— inside the check written to close D206, in the same cycle that closed D209. It **failed on an
unmutated build** (`pc_plan_solve growth 1.85x` against a 1.6 ceiling, run 0 of three in one
process), and six measurements of that sound build read growth **0.97 / 1.14 / 1.15 / 1.40 / 1.69 /
1.85** while the recorded table claimed *"the widest growth this build shows is `pc_plan_solve`'s
1.05x"*. Its absolute big-size cook swung **482 → 818 ms**.

`wrangle_cost_tables` applies `interleaved_best`'s two properties to the **SIZE** axis: both rigs
exist before either is measured, and within one repetition every rig sees the same machine. Six
independent runs of the new estimator read `pc_plan_solve` growth **1.10 / 1.10 / 1.15 / 1.17 /
1.18 / 1.09** and its big-size cook **470–507 ms**. The recorded table is a **spread over those six
runs**, not one run's numbers.

**AND THE MUTATION HAD THE WRONG SHAPE (finding 4).** `_named` scaled `big[name]` alone, which
multiplies that row's GROWTH by 5 and trips `WRANGLE_GROWTH_CEILING` almost regardless of the
per-piece ceiling — so eleven of the eighteen rows were being caught by a term the check does not
claim to be proving. A real de-batching slows BOTH sizes. With both scaled, `pc_curve_index`,
`pc_kit_id` and `pc_markers` survive entirely (their sound growth is ~0.3, so even 5x reads 1.5 <
1.6) and `pc_curveid` and `pc_corners` clear their ceilings by 1.3 % and 3.6 %. And `blind` — the
rows the noise floor exempts — **never entered the verdict at all**, so the row would have printed
PASS at *"0/18 at 5x, 18 under the floor"* if the instrument ever read low across the board.

Both sizes move now, and every blind row is reddened **physically** instead. The auditor was right
that the abandoned physical burn failed for a fixable reason: it was **loop-invariant**, so VEX
hoisted it. An **element-dependent** burn whose accumulator is also loop-carried cannot be hoisted,
and it is deterministic — all five blind rows redden by 10–25x: `pc_corners` 0.40 → 33.4 ms,
`pc_curve_index` 0.15 → 23.9, `pc_curveid` 0.34 → 16.4, `pc_kit_id` 0.17 → 16.0, `pc_markers` 0.21
→ 17.6. **13 at 5x + 5 by burn = eighteen of eighteen.**

### 24.3 A declaration that names a node that does not exist asserts nothing

D208's forbidden column is the whole of D208's fix — it is the only behavioural assertion the
`config` row has, whose `must_cook` list is empty by design. It read
`node.node(n) is not None and node.node(n).cookCount() > before[n]`, so **a RENAMED node silently
turned the second voice off**, where `must_cook` has complained about a missing name since D208.

Demonstrated: replace the forbidden entries of `config`, `plan` and `frames` with `kernelXX` /
`copy_packedXX` / `pc_plan_solveXX` — simulating a rename of `copy_packed` — and the suite prints
`every_stage_has_a_second_source` **PASS 10/10**, `native_stages_are_really_native` PASS, all five
`mutation_*_unplugged` PASS, **0 failing**. With the sound table, re-pointing the `config` Stage
entry at `OUT_final` yields `config: copy_packed,pc_plan_solve cooked and must not`; with the
column blinded it yields nothing. A missing name in **either** column (`forbidden` or `allowed`) is
a complaint now, and the same mutation reddens the check on all five rows.

### 24.4 A rule about a RUNNER is proved by calling the runner

D210's rule was asserted on `run_scene_checks.exit_code` as a pure function and on `run_2d_checks`
by grepping three literal strings out of `inspect.getsource(main)`. **Neither is reachability.**
Inserting, before the baseline block of `run_2d_checks.main`,
...
leaves every asserted string present and unreachable: the row printed **PASS 9/9** while the runner
itself, with a `baseline_2d.json` value perturbed to 999999 against a real 176, printed **no
movement line and exited 0** — D210's exact finding, restored past a green check.

Both runners' `main` are **called** now, in-process, with three things replaced and all three
restored: the case builder returns `{}`, the tripwire block returns one stub value (so the run
costs milliseconds rather than minutes and the only thing exercised is the baseline-and-exit tail),
and `BASELINE` points at a throwaway file. Three passes: write the baseline, perturb it and demand
exit 1 **with the movement line printed**, restore it and demand exit 0. **17 rows**, and the
mutation above takes it to **15/17**, naming `run_2d_checks REACHES the baseline block`.

**D210's own second half, re-checked.** §22.1 replayed twelve claims made while the runner could
exit 0 on movement and found none of them wrong. That still holds: `run_scene_checks` and
`run_2d_checks` are **0 failing / 0 moved** from the pristine export at this commit, so no baseline
claim in this document rests on the advisory rule.

### 24.5 A warning belongs where the cookCount is the complement

PART B moved D88's unread-marker warning into `pc_sections` — the first node of the native PLAN
chain — on the premise that *"this chain only cooks when level 1 has admitted the build, which is
exactly when `kernel` is silent"*. **That premise stopped being true in the same cycle that wrote
it**: PART B also widened level 1, so there is now a class of build that level 1 ADMITS and level 2
REFUSES, and on those the plan chain cooks (`pc_envelope2` needs it) **and** `kernel` cooks. The
artist read the identical sentence twice. Reproduced on the shipped asset with a loose marker of id
7:

| fixture | `copy_packed` | `kernel` | who warned |
|---|---|---|---|
| straight | 1 | 0 | `pc_sections` — correct |
| `arc R=20 step 0.05` | 0 | 1 | **`pc_sections` AND `kernel`** |
| `arc R=50 step 0.1` | 0 | 1 | **`pc_sections` AND `kernel`** |

Those two arcs are `guard_bend_bound`'s own L2-refusal fixtures, so the class was already in the
suite — but `unread_marker_warns` asserted on a straight run (pure native) and a cornered one (pure
reference), and **between them they never cook both chains**.

The cook map is what picked the new node, not taste:

| node | native | L1-admit / L2-refuse | `Stage = reference` |
|---|---|---|---|
| `pc_sections` | 1 | 1 | 0 |
| `copy_packed` … `pc_warn_collate` | 1 | 0 | 0 |
| `kernel` | 0 | 1 | 1 |

`pc_warn_collate` and `kernel` are **exact complements**, so the warning is raised exactly once on
every build, by whichever branch actually shipped it. It is `pc_warn_collate`'s job already (D61's
summary, and `hda.cook`'s own `addWarning` wording), and it gained two inputs — `config` for the
flattened rule table and `OUT_sections` for the marker points. `unread_marker_warns` has a third
fixture now, the arc, and asserts L1 = 1 ∧ L2 = 0 explicitly so the row cannot silently stop
covering that class.

**And the two paths disagreed about what a marker id IS (finding 12).** The VEX uses the canonical
test (`tail != itoa(atoi(tail))`); `hda._warn_unread_markers` uses Python's permissive `int()`. So
for `marker:03`, `marker:+3` and `marker: 3` — none of which can ever match the kernel's own
`"marker:%d"` — the reference counted id 3 as READ and **said nothing**, while the native path
warned. The silent one was the wrong one: it suppressed the warning for a rule that can never fire,
which is exactly what D93 says must not be silent. `style._check_slot` asks for the canonical form
now, which warns on the RULE (the honest sentence) and makes `_native_ok` refuse the build, so the
two faces agree by construction.

### 24.6 Prose that makes a claim gets a TAG, not a promise

Three separate stale-text findings, and all three are D207's rule (*text that makes a claim about
the build is a CHECK, not a comment*) in places D207 did not look.

**The guard's own refusal list.** `pc_envelope.vfl`'s `WHAT LEVEL 1 REFUSES` block still named
*"a non-zero Kit Padding"* a whole cycle after PART B ported D91's Gap into `config_resolved` and
deleted that row from `_native_ok`. It is the **only prose statement of the guard's envelope** there
is, so an auditor reading the contract against the code was reading a contract the code had left
(measured: `padding = 0.5` admits and `copy_packed` cooks). Every row carries a TAG now —
`[cfg:<name>]` for something `_native_ok` tests, `[vex:<name>]` for a term of this file's
`i@_native_ok` expression — and `guard_refusal_list_is_true` checks **both directions**: every tag
exists in the code, and every parameter the code refuses on (read off the ASSET's own parm list,
not a second hand-kept table) plus every term of the verdict expression has a row. Three mutations,
three complaints: the padding row restored → `[cfg:padding] is not tested by _native_ok`; the
storage row's tag removed → `the verdict reads 'type_bad' and no row says so`; the storage term
deleted from the verdict → `[vex:type_bad] is not read by the verdict expression`.

**A check named in a comment must exist.** §20.3 found `bench_guard_fallback` cited by two source
comments and existing nowhere; the cycle that wrote it immediately grew a citation of
`guard_fallback_classes`, which `grep -rn` finds **once in the whole tree, on the comment line
itself**. `every_cited_check_exists` reads every wrangle header and every Python SOP comment and
refuses a backticked snake_case name that exists nowhere in the codebase. The vocabulary is
deliberately *every name the codebase has* rather than a list of checks — a header legitimately
cites `plan_sections` (a function), `native_ok` (a `pc_cfg` key) and `pc_deform_gate` (a node), and
a narrower rule fires on correct text.

**The four Python SOPs' DisplayComments were the one shipped string nothing read** (§22.8 item 4),
and two of the four had gone stale in the worst way — by promising a port:

* `pc_plan_bridge` read `SCAFFOLDING - 13.9 N2 DELETES THIS NODE` on a build where N2 landed
  cycles ago, the node is still wired, still on the Stage menu and still the only feeder of
  `OUT_frames`. It is kept **deliberately** — it is the per-stage oracle `frames_parity` and
  `plan_parity` compare against — and the comment says that now.
* `kit_starter` promised *"D154's native box SOPs replace this body"* on a build whose own cycle
  formally **DECLINED** D154 with the measurement that declines it (D219). So did `hda.cook_kit`'s
  docstring.

`every_python_sop_comment_is_checkable` asserts the comment exists, is displayed, and **may not
promise the removal of a node that is still in the asset**. (It deliberately does NOT check
`input N` on these: they are prose, not headers — `kit_starter` says *"ran inside `kernel` on every
cook with input 2 unwired"*, which a loose wiring rule reads as a declaration. A rule that has to be
written loosely to fire at all is a rule that fires on correct text.)

**And `config`'s comment named the wrong N (finding 19).** It said *"No geometry; N = the parm
count. It must never grow a geometry loop — the wrapper tripwires are pointed at it"*, and the
tripwires it invokes are pointed at a LOOP SHAPE. N is the **kit**: `config_resolved` runs
`kit.read()` over the kit geometry's packed prims in Python on every cook. Measured, `hou.perfMon`,
min of 3 on a 2 km straight:

| modules | 5 | 50 | 150 | 301 |
|---|---|---|---|---|
| `config` | 1.61 ms | 5.45 ms | 14.48 ms | 28.52 ms |

~0.09 ms per module per cook, and at 301 modules it is the **second most expensive node in the
graph**. §13.6's own exemption text said the module count all along; the artist-facing comment did
not. `config_cost_per_module` holds the SLOPE at 0.25 ms/module (measured 0.089–0.091), on builds
the guard ADMITS so the number sits beside the VEX and not beside `kernel`.

`pc_deform_gate`'s comment had the mirror problem — it described in the present tense the PYTHON it
replaced (*"the biggest single Python item left on the shipped path: 0.133 s of `place.build`'s
0.813 s at 20 km"*) while the node measures **4.2–4.5 ms of VEX**. Past tense now, with both
numbers.

`bench_guard_fallback`'s docstring and `no_case_pays_the_guard_fallback`'s comment both said *"on
the shipped build there is no legitimate input that passes level 1 and fails level 2 … and it is
the reason the guard is free today"*. That was true of the UPPER-bound level 1 they were written
against and false the moment PART B made the bound a LOWER one: `GUARD_BEND_LADDER`'s
`arc_R20_step0.05` and `arc_R50_step0.1` rows are exactly such inputs, and reproduce (L1 = 1,
L2 = 0). Both are rewritten to say the path IS reachable, to name the shapes, and to say that
"0 of 92" is a statement about the 92 fixtures' kits and not about the tool.

### 24.7 The two performance findings — one reproduced, one disproved

**REPRODUCED, MEASURED, NOT FIXED: `pc_plan_solve` parallelises per SECTION.** It is a POINT
wrangle over the section stream — one section per thread — which is right for the data dependency
the fitting solve has and wrong for a build made of one long curve: the whole solve, 81–87 % of the
native cook, runs on a single core. `hou.perfMon`, min of 4 dirtied cooks on the shipped asset:

| shape | pieces | `pc_plan_solve` | µs/piece | whole build |
|---|---|---|---|---|
| one 18.9 km straight | 33 586 | 1 139.7 ms | 33.93 | 1 353.8 ms |
| 300 curved streets | 34 200 | 33.7 ms | 0.99 | 234.6 ms |

Same node, same work per piece, **34x apart purely because the parallel unit is the section**.
The committed guard could not see it: `WRANGLE_SMALL_PTS` and `WRANGLE_BIG_PTS` are 2 361 and 9 436
points of a **single straight**, so both sizes sit on the one-thread shape and the growth ratio
compares slow against slow.

`plan_solve_section_shape` is the row that would have caught it — the same piece count over 1
section and over 300, ceiling set from a six-run spread (24.2–35.0x observed, **55.0x** ceiling,
D211's floors on both piece count and cook time). **It is a tripwire, not an endorsement.** The fix
the auditor proposes — emit per-section SCALARS and let a parallel POINT wrangle after
`pc_plan_emit` compute `s0`/`s1`/`u` per piece — is a rewrite of the hottest and most
parity-sensitive node in the tool: the cursor is only affine in the piece index for the ordinary
`first`/`adaptive` run, and `reselects`, the slice tail, the marker anchors and D150's determinism
all live in the loop it would replace. That is a cycle, not a patch. **D222.**

**DISPROVED: "~2/3 of `pc_plan_solve` is the five per-piece STRING columns."** The finding's method
is to leave the columns *"with no consumer, so VEX eliminates their construction"*. It eliminates
far more than that. Measured on the shipped asset at 33 594 pieces, `hou.perfMon`, min over **five
INTERLEAVED** repetitions of the three variants:

| variant | `pc_plan_solve` | |
|---|---|---|
| sound | 1 615.3 ms | |
| the five `s[]@_p_*` writes DELETED, everything else live | 1 871.3 ms | **no speedup at all** |
| truncated before the sort | 7.9 ms | **204x — VEX eliminated the entire dead fill** |

The 204x row is what the finding's numbers are made of: the truncation measures **dead-code
elimination**, not the fill. And the direct test — deleting only the attribute writes and leaving
the arrays live — shows the write is inside the sound build's own run-to-run spread (886–1 774 ms
on this node at this size, which is itself worth knowing). The reason is in the node's own header:
the **deduped row table** (this node's recorded 7x fix) means only a handful of string ELEMENTS are
written per section, not one per piece. There is no two-thirds to recover.

### 24.8 THE PYTHON SHARE — measured here, per build class and per size

`hou.perfMon` `Cook - ms` per child of the shipped instance; a child whose type is `python` is
Python, everything else is native. Min over 3 dirtied cooks, from the pristine export.

| class | km | Python ms | native ms | total | **Python %** | chain |
|---|---|---|---|---|---|---|
| straight | 2 | 2.3 | 63.1 | 65.4 | **3.5 %** | NATIVE |
| straight | 20 | 2.3 | 733.9 | 736.2 | **0.3 %** | NATIVE |
| curved (R = 2 000 arc) | 2 | 2.4 | 67.4 | 69.8 | **3.4 %** | NATIVE |
| curved (R = 2 000 arc) | 20 | 2.4 | 708.1 | 710.5 | **0.3 %** | NATIVE |
| cornered (dog-leg / 200 m) | 2 | 59.2 | 5.2 | 64.4 | **91.9 %** | reference |
| cornered (dog-leg / 200 m) | 20 | 593.7 | 31.9 | 625.6 | **94.9 %** | reference |
| conformed (surface on input 4) | 2 | 180.0 | 5.3 | 185.3 | **97.2 %** | reference |
| conformed (surface on input 4) | 20 | 1 945.1 | 33.6 | 1 978.7 | **98.3 %** | reference |

**The whole of the Python on an admitted build is `config`, and it is a FIXED 2.3–2.4 ms** — see
§24.6 for what makes it fixed and what makes it grow. Per stage at 20 km, straight (every child over
0.5 ms):

| node | ms | |
|---|---|---|
| `pc_plan_solve` | 585.19 | **80 % of the build** |
| `pc_plan_emit` | 49.97 | |
| `pc_arclength` | 14.85 | |
| `pc_envelope` | 9.03 | |
| `pc_stamp` | 7.65 | |
...
| `copy_packed` | 4.57 | |
| `pc_deform_gate` | 4.55 | |
| `pc_warn_collate` | 4.49 | |
| `pc_frames_native` | 2.83 | |
| **`config`** | **2.38** | **PYTHON — all of it** |
| `pc_finalize` | 2.15 | |
| `pc_proto` | 2.10 | |
| `_scrub_groups` / `_scrub` | 1.92 / 1.91 | |
| `pc_out_cast` | 1.80 | |
| `pc_sec_only` | 1.52 | |
| `pc_envelope2` | 1.46 | |
| `pc_plan_only` | 1.07 | |

**WHICH CLASSES STILL TAKE THE REFERENCE, and it is correct rather than a gap** — the guard fails
safe and the output is the reference's, bit for bit, on every one:

1. **conformed** (a surface on input 4) — N6, the largest remaining;
2. **elevation** (ripples, hills, any climb) — N5;
3. **cornered** — N8;
4. `show_warnings`, `display = proxy|plan`, `fill = tile` — the three cheap ones;
5. **and, new this cycle and by design:** a spline attribute at a storage the VEX chain cannot read,
   and a kit whose module names no resolved rule matches. Both were previously ADMITTED — the first
   shipped a different fence, the second cost 2.35x.

### 24.9 Mutation — twelve at source with the `.hda` rebuilt

Every row edits a SOURCE file in a throwaway copy of the pristine export, **rebuilds the `.hda` from
that copy** (so the asset and its declaration move together) with `POLYFACTORY` pointed at the copy
— §21.7's hazard, which would otherwise overwrite the real repo's asset — and runs the suite there.

| # | mutation | verdict |
|---|---|---|
| 1 | `pc_stamp` de-batched (per-element full scan) | **RED** — `stamp_cost_is_flat_in_piece_count`, `wrangle_cost_is_flat_in_piece_count` |
| 2 | `pc_deform_gate` de-batched | **RED** — `wrangle_cost_is_flat_in_piece_count`, `output_guard_cost` |
| 3 | `pc_warn_collate` de-batched ×8 | **RED** — `wrangle_cost_is_flat_in_piece_count` |
| 4 | `OUT_place_native` re-fed from `pc_out_cast` | **RED at BUILD** — the build script asserts the wiring against `native.STAGES` |
| 5 | `OUT_frames_native` re-fed from `pc_proto` | **RED at BUILD** — same |
| 6 | `guard_native` input 1 moved onto `OUT_reference` | **RED** — 13 failing, incl. `output_runs_the_native_chain_inside_the_envelope`, `native_stages_are_really_native`, `mutation_reference_unplugged` |
| 7 | the `plan` Stage label re-worded to NATIVE | **RED** — `stage_labels_are_true` |
| 8 | `pc_arclength` at `vex_precision = 32` | **RED** — 8 failing, incl. `plan_span_transport_at_20km`, `place_packed_parity`, `output_guard_parity`, `every_wrangle_comment_is_checkable` |
| 9 | the corner bisector broken (`_unit(tin, tin)`) | **RED** — **51 failing** in `run_scene_checks` (`corner_face_mate_m`, `corner_outside_m`, `corner_breach_m`), the same 51 §21.4 recorded |
| 10 | level 1 admits everything | **RED** — 9 failing, incl. `output_guard_parity`, `no_case_pays_the_guard_fallback`, `guard_bend_bound` |
| 11 | the storage table dropped from the verdict | **RED** — `guard_spline_attr_types`, `guard_refusal_list_is_true` |
| 12 | the kit-mismatch row removed from `_native_ok` | **RED** — `guard_kit_mismatch` |
| 13 | `_bend_bound` fails open on an empty kit | **RED** — `guard_bend_bound_needs_its_operands` |
| 14 | `kit_starter`'s promise restored | **RED** — `every_python_sop_comment_is_checkable` |
| 15 | D88's warning taken out of `pc_warn_collate` | **RED** — `unread_marker_warns` |

**FIFTEEN MUTATIONS, NO SURVIVORS** — the first cycle in this document's history where that is
true. Two of the fifteen never reach the suite because the BUILD SCRIPT refuses them, which is a
stronger answer than a red check and is D203's `native.STAGES` earning its keep.

**Two mutation-shape lessons recorded rather than rediscovered**, both found while writing this
table and both the reason a mutation can look like a survivor when it is not:

* **A burn whose value is OVERWRITTEN each iteration is eliminated.** `_junk = point(...)` in a loop
  keeps only the last read, so a "per-element full scan" costs one read. It has to ACCUMULATE.
* **A ceiling built as 2.5x the measurement tolerates a 2x regression by design.** One extra full
  scan of `pc_warn_collate` (0.24 → ~0.45 µs/piece against a 0.56 ceiling) passes, correctly: the
  claim the pair of checks makes is FIVE-fold, so the mutation has to be five-fold to test it.

**And one real defect the mutations found**, in the check machinery rather than in the tool:
unplugging `guard_native` takes `pc_finalize` and `pc_warn_collate` off the cooked path entirely,
and `wrangle_cost_check` then did `lever["pc_finalize"]` — a **KeyError**, so the run ended in a
traceback after two `[FAIL]` lines with every check below it never run and the summary line never
printed. That is D208's crash-instead-of-complaint, one cycle later and in a new place. It is a
complaint now, and the same mutation reports **13 failing** where it reported 2 and a traceback.

### 24.10 GATES PC-G1..PC-G5 — re-confirmed on images whose segment counts were checked

Every image regenerated from this build; the rasteriser's `image_shows_packed_*` rows unpack first
and report `[packed prims, polygons]` beside the drawn-segment count (D194's rule, after a committed
gate image once held 188 segments where the fence is 3 388).

- **PC-G1 — PASSES, and was LOOKED AT.** `VG1_rect_miter_top.png` **40 packed / 840 polys / 3 388
  segments**: a closed rectangle, four continuous sides, posts at even intervals, and a corner post
  at every one of the four corners **with the 45° bisector drawn across it** — confirmed by eye on
  this build. `VG1_rect_bend_top.png` 35 / 760 / 3 079, `VG1_L_bend` 15 / 292 / 1 185, `VG1_L_miter`
  18 / 342 / 1 384, `VG1_closeup_bend` 537, `VG1_closeup_miter` 412. Segment-per-polygon 4.0–4.4
  throughout.
- **PC-G2 — PASSES, and was LOOKED AT.** All four z-modes on the 24 m S-curve over the 2D hill,
  14 packed / 492 polys / 2 078–2 174 segments each. `VG2_camber_front.png` confirmed by eye: the
  rail is visibly rolled onto the cross-fall around the whole S and the feet track the green ground
  line.
- **PART B's pair, still identical.** `PARTB_arc_{native,reference}_{top,side}.png`, 1 132 segments
  each, `(86, 283)` prims whole and cropped on both — `partb_curved_is_native` and
  `partb_curved_matches_the_reference` both PASS.
- **PC-G3 — PASSES on its own terms.** `scale_gate` **9 rows, 0 failing**, unchanged: five rows at
  10 000 packed / 0 deformed / 10 000 points / one geometryid / 0.22–0.40 s, and `arc_10` (kit) /
  `arc_80` (adaptive) correctly deforming at ~360 000 points / 1.25 s.
- **PC-G4 — PASSES.** `run_hda_checks` **31 `[PASS]` / 0** (was 29; `unread_marker_warns` gained its
  third fixture).
- **PC-G5 — STILL DOES NOT PASS, and this cycle did not touch it.** The images regenerate unchanged
  (`PCG5_FA_L_facade_iso.png` 10 368 segments, `_reflex` 1 184) and `run_2d_checks` is **633 `[PASS]`
  / 0 failing / 0 moved**. What is missing is what P2-3V and §21.8 already recorded: **condition 4 is
  true and unasserted** (0 of 176 placements carry a `slice_t` and no check says so) and **condition
  3 needs a fixture as well as a mode**.

### 24.11 Decisions

- **D222 — `pc_plan_solve`'s parallel unit is the SECTION, and the ROW is not the fix.** One long
  curve pays 30x what the same piece count pays as 300 short ones. `plan_solve_section_shape` pins
  the ratio so it cannot get worse in silence; bringing it DOWN means emitting per-section scalars
  and expanding per piece in a parallel wrangle after `pc_plan_emit`, and that is a cycle with the
  `reselects` path, the slice tail, the marker anchors and D150's determinism all in it. Until then
  the honest sentence is: **the tool is fast on many short curves and slow on one long one, and the
  suite says so.**
- **D223 — AN ATTRIBUTE'S STORAGE IS PART OF THE CONTRACT, AND THE ANSWER IS A REFUSAL.** Python
  converts what the artist authored; VEX reads the type's zero. Where the two disagree the guard
  refuses, because porting the coercion is a trap (`str(2.0)` is `"2.0"`, `sprintf("%g")` is `"2"`).
  Every future `point()`/`prim()` read of an artist-authored name owes a row in
  `pc_envelope.vfl`'s storage table.
- **D224 — the PARM face never validates the style against the kit, and level 1 must.** `style.read`
  warns when a rule names a module the kit lacks and `_native_ok` refuses on any warning; the parm
  face has no such path, so the ordinary "bring your own kit" input planned every piece, built none,
  and paid 2.35x. The test is `style.read`'s own, transcribed.
- **D225 — a warning goes on the node whose cookCount is the COMPLEMENT of the other branch's.**
  Not on the first node that can see the data. `pc_sections` cooks on the L1-admit/L2-refuse class
  and so does `kernel`; `pc_warn_collate` and `kernel` are exact complements. Any future warning
  that both faces must raise needs that cook map drawn before it is placed.
- **D226 — a prose contract carries TAGS.** The guard's refusal list is the only statement of its
  envelope, and it drifted for a cycle. Tags make both directions checkable: every claim exists in
  the code, and every refusal in the code has a claim.
- **D227 — a comment may not promise the removal of a node that exists, and a check it names must
  exist.** Two of the four Python SOP comments promised ports (one already declined by the document
  itself), and two source comments have now cited non-existent checks in two consecutive cycles.
- **D228 — `interleaved_best`'s rule applies to the SIZE axis, not only the repetition axis.** Two
  rigs measured in two blocks is D209 whatever the estimator inside each block is. Build every rig a
  ratio compares before measuring any of them.
- **D229 — a mutation must have the SHAPE of the regression it stands for, and the exempt set must
  enter the verdict.** Scaling one side of a growth ratio tests the growth term, not the ceiling;
  and rows the instrument cannot see must be proved some other way (an element-dependent,
  loop-carried burn) rather than counted and ignored.
- **D230 — a declaration that names a node complains when the node is missing, in EVERY column.**
  `must_cook` did; `forbidden` and `allowed` did not, so a rename turned D208's second voice off.

### 24.12 What is NOT done — where the next audit should start

**Dev-loop rule 0 is NOT discharged for this cycle.** Nothing here has been audited by an agent that
did not write it. The honest words for §24 are **implemented, self-verified, unaudited**.

1. **D222 is open and it is the whole performance story.** `pc_plan_solve` is 80 % of a 20 km native
   cook and 30x worse on one curve than on three hundred. `wrangle_ceilings_are_tight` will go RED
   on the first real speedup until the §24.2 table is re-recorded — deliberate.
2. **The storage table is eleven names on one reading of the VEX.** It was built by grepping every
   `point(0, "…")` / `prim(0, "…")` in `polyfactory/vex/polychain/`. A name the chain reads through
   a variable, or one added later, is not in it — and `guard_spline_attr_types` proves the ELEVEN
   bite, not that eleven is the whole list.
3. **`_native_ok`'s kit-mismatch row uses `style.read`'s test, and that is a transcription.** D214's
   rule says a transcription needs a second voice; there is none. If `style.read`'s rule changes,
   the two faces diverge silently again.
4. **`config_cost_per_module` measures two points and holds a slope.** It cannot see a quadratic
   between them, and 301 modules is the largest kit anyone has tried.
5. **The three classes that still take the reference are unchanged** — conform (N6), the deformed
   branch (N5), corners (N8) — and §23.11's notes on each still stand.
6. **PC-G5's two gaps are untouched** and are the cheapest untested truths in the tool.
7. **Verify `git status` is clean and the committed `.hda` is the one the checks ran against.**
   Fifteen source-level rebuilds ran this cycle, all of them in throwaway copies with `POLYFACTORY`
   redirected; confirm that held.

---

## 25. Native network build log — cycle N-4D (D206–D210 re-verified, and the brief was stale)

**This cycle changed no geometry, no kernel, no VEX and no check.** It was commissioned to
*"repair the evidence machinery (D210, D206, D207, D208, D209)"* — and every one of those five was
already closed, four cycles of work ago. The finding is that **the brief was stale, not the
suite**, so the deliverable is the independent re-verification the dev loop's rule 0 asks for:
each defect's mutation re-applied at `7af1498` and shown to redden, with the output recorded.

**Read this before assuming a brief's premise.** Two of the three things the brief asserted were
false, and both were checkable in one `git log`:

| the brief said | the truth | where |
|---|---|---|
| "repair D206–D210" | closed in **§22** (`58dba2d` `3b63340` `1776977` `7cb291b` `b280efc`), hardened again in **§24** (`4f04aa5`) | `git log --oneline` |
| "§0.0's *Last completed* row is STALE — it predates the flip cycle" | **current**: it names cycle N-4C's four commits and matches the log exactly. It was corrected by `203da1e` and rewritten by `7af1498` | §0.0 |
| "the last audit ran fifteen source mutations with no survivors" | **true**, reproduced: `run_native_checks` **130 `[PASS]` / 0** | below |

### 25.0 The scoreboard — five mutations re-run, five reddened

`.hda` at md5 `592ab55c` before and after; the three rebuilds below each restored it byte for byte.

| # | the mutation re-applied | what went red | reading |
|---|---|---|---|
| **D210-a** | `exit_code` reverted to `1 if failures and not update` | `scene_baseline_movement_fails_the_run` | **14/17**, and it catches BOTH runners |
| **D210-b** | an early `sys.exit` above `run_2d_checks.main`'s baseline block | same check | **15/17** — the two BEHAVIOURAL rows only |
| **D206** | `pc_finalize.vfl` as a detail loop, `native.py` `primitive → detail`, `.hda` rebuilt (`a447c8ac`) | `wrangle_cost_is_flat_in_piece_count` | `pc_finalize 0.7314 > 0.3500 us/piece` — **209 %** of ceiling |
| **D207** | both "via the PYTHON BRIDGE" labels re-worded to "NATIVE", `.hda` rebuilt (`5b53f748`) | `stage_labels_are_true` **and** `mutation_stage_labels_claim_native` | 4 complaints: 2 false NATIVE claims + 2 duplicate menu entries |
| **D208** | `native.STAGES`' `reference` row re-pointed at `OUT_final` / `guard_envelope`, `.hda` rebuilt (`1a63f3cb`) | `every_stage_has_a_second_source` | **127 `[PASS]` / 3 `[FAIL]`, NO traceback**, summary printed, exit 1 |
| **D209** | `straight_2km` shrunk 2 000 m → 40 m | `output_guard_cost` | *"the reference cooks in 0.0059 s, under the 20 ms floor — a 1.60x ceiling on it is noise, not a measurement"* |

### 25.1 D210 — and the twelve claims, re-checked from scratch a second time

§22.1 replayed the twelve native-lane commits made while a moved value could exit 0 and reported
none of them wrong. **That replay was itself a claim, so it was re-run here rather than cited.**
All twelve exported with `git archive` into separate trees; **both** baseline-carrying runners run
at each.

| commit | `run_scene_checks` | `run_2d_checks` |
|---|---|---|
| bec344e cd3b03a 0f7310e 7bed7b8 f5e5156 a13d35f | 0 failing / 0 moved | 0 failing |
| 58cbf82 85fff64 909e9cb 8ba9451 fbf0961 adde049 | 0 failing / 0 moved | 0 failing |

**Plainly: not one of the twelve "no baselined value moved" claims was wrong.** They were
*unsupported* — the exit code structurally could not carry them — and they were also *true*.
`baseline.json` has not changed since `ac66815`, which predates the native rebuild entirely.

**And the replay is not vacuous**, which is the half a reader should demand. Perturbing
`A_straight/element_count` 20 → 19 in **`bec344e`'s own exported baseline** reproduces D210 at a
commit that made the claim:
...
The identical perturbation at `HEAD`:

```
--- MOVED SINCE BASELINE: 1 value(s) ---
  A_straight/element_count: 19 -> 20
  ^ each must be an improvement; confirm, then re-run with --update-baseline

0 failing checks, 1 moved baseline values
ACTUAL EXIT CODE = 1
```

⚠️ **D210-b is the row worth keeping.** The three `inspect.getsource` rows stay GREEN under it —
every grepped string is still present, just unreachable — and only the two rows that CALL `main`
go red. A source grep proves a line exists; it never proves it runs.

### 25.2 D209's other half — the flakiness, not just the floor

D209 was a check that *failed once in three runs of an unmutated build*, so a mutation proof alone
does not close it. Three consecutive unmutated runs here:

```
[PASS] output_guard_cost  straight_2km 0.95x/74ms native; arc_2km 0.97x/82ms native; ... arc_R50_2km 1.10x
[PASS] output_guard_cost  straight_2km 0.95x/75ms native; arc_2km 0.93x/80ms native; ... arc_R50_2km 1.08x
[PASS] output_guard_cost  straight_2km 0.94x/76ms native; arc_2km 0.93x/80ms native; ... arc_R50_2km 1.05x
```

0.93–1.10x against ceilings of 1.15x and 1.6x. The original defect read **1.31x on a 5 ms node**;
both causes — the stopwatch in two blocks and the tiny fixture — are gone.

### 25.3 One thing the D206 re-run exposes, and it is minor

On the de-batched build `mutation_pc_finalize_debatched` **still passes** — it compares the mutated
node against a "batched" figure it measures on that same mutated build, so both sides read 0.73
µs/piece. It is not wrong (it asserts the de-batched rate crosses the ceiling, which it does), but
it is **not the row that catches the regression**; `wrangle_cost_is_flat_in_piece_count` is, at
209 %. Worth knowing before anyone credits the wrong check. **D226.**

### 25.4 What this cycle asks of the next agent

- **D226 — a mutation check that measures its own control on the mutated build has no control.**
  `mutation_pc_finalize_debatched` should read its batched comparand from `WRANGLE_CEILING_US`'s
  recorded table, not from a live measurement of the node it just mutated.
- **D227 — a resume pointer may not name its own commit as "this one".** §0.0 did, and a fresh
  agent cannot resolve it. It is a hash now.
- **⚠️ CHECK THE BRIEF AGAINST `git log` BEFORE WORKING.** This cycle's brief asked for five
  defects that were closed and for a §0.0 correction that had already been made. One `git log
  --oneline -25` would have caught it, and re-doing closed work is the most expensive thing an
  overnight lane can spend a cycle on.
- **The three big ports are untouched and still in §24's measured order** — conform (954 ms),
  the deformed branch (1 349 ms), corners (336 ms). Nothing here moved them.

---

## 26. Native network build log — cycle N-4E (PART B: 13.9 N5, the deformed branch)

**Read this before §25.** §25 re-verified §22's evidence machinery and explicitly did not start
a port. This cycle is PART B proper: measure which refused build classes cost the most Python
across a realistic mix, port them in that order, keep the guard failing safe, flip
`Stage = output` only at parity, and say plainly which classes still take the reference.

### 26.0 The scoreboard

| # | class | Python before | after | what it took |
|---|---|---|---|---|
| 1 | **hilly / elevated streets** (300, the citygen shape) | 1 237 ms — **98.7 %** | **2.3 ms — 0.7 %** | 13.9 N5, the deformed branch |
| 2 | **ripple 2 km** | 173 ms — 96.1 % | **2.4 ms — 2.0 %** | the same |
| 2 | **ramp 2 km** (a uniform slope, zero turn) | 158 ms — 96.0 % | **2.2 ms — 1.9 %** | the same |
| 2 | **ripple 20 km** | ~96 % | **2.3 ms — 0.2 %** | the same |
| 2 | **arc R = 50, 2 km** (a piece really unpacks) | 96 % | **native** | the same |
| — | **conformed streets** (300 + a terrain) | 3 645 ms — 99.6 % | unchanged | **N6, not attempted — takes the reference** |
| — | **conformed CURVED streets** (300 + a terrain) | 4 225 ms — 99.6 % | unchanged | **N6, not attempted** |
| — | **cornered streets** (300, one 90° bend each) | 806 ms — 98.1 % | unchanged | **N8, not attempted** |
| — | **cornered L, 20 km** | 801 ms — 93.2 % | unchanged | **N8, not attempted** |
| — | conformed 2 km (0 % deformed) | 196 ms — 96.5 % | unchanged | N6 |

**Suites after:** `run_native_checks` **137 `[PASS]` / 0** (was 130), `run_scene_checks` 0 failing /
**0 moved**, `run_2d_checks` 0 / **0 moved**, `run_hda_checks` 0, `scale_gate` 9 rows / 0,
`gate_images` 0, `run_attrib_checks` 0 / 0, **328 unit tests OK**. `output_guard_parity` went
**18 native / 74 reference → 28 / 64, identical**. Two commits: `65b9c9f`, `6f4d2f2`.

### 26.1 The measurement that set the order — and the DEPENDENCY re-ordered it again

`hou.perfMon` per-node `Cook - ms` on the **shipped asset** at `Stage` = its default, minimum
over three dirtied cooks. Every row is a build the guard refused, and the number is the Python
that cooks on it. The mix is §23's plus the four 20 km rows and a conformed variant of the
citygen curved shape.

| refused class | fixture | Python | prims |
|---|---|---|---|
| **conformed curved streets** | 300 curved streets + a terrain | **1 953 → 4 225 ms** | 294 600 |
| **conformed streets** | 300 straight streets + a terrain | **1 742 → 3 645 ms** | 239 160 |
| **hilly streets** | 300 curved streets, gentle elevation | **1 237 ms** | 294 600 |
| **cornered streets** | 300 streets with a 90° bend | **806 ms** | 27 300 |
| **cornered L, 20 km** | | **801 ms** | 18 903 |
| conformed 2 km | a straight run + a terrain | 196 ms | 1 888 |
| ripple 2 km | | 173 ms | 33 357 |
| ramp 2 km | | 158 ms | 33 042 |
| corner L 2 km | | 80 ms | 1 921 |

⚠️ **THE TWO CONFORMED ROWS DOUBLED WHEN THE FIXTURE WAS FIXED, and that is worth writing down
rather than quietly correcting.** The first pass wired `conform_bench.terrain`'s default extent
(660 × 360 m) under a street grid 1 600 × 1 000 m across, so most of the run was over NOTHING and
the row was measuring a miss. A terrain that actually covers the streets is 3 645 ms, not 1 742.
A conform benchmark whose runs are off the surface is a benchmark of `D53`'s miss path.

**AND THE ORDER THE RAW NUMBERS GIVE IS NOT THE ORDER THE PORT CAN BE DONE IN.** Conform is the
largest class by 4.5x — and the reference's own report says how much of each class is DEFORMED:

| class | packed | deformed | % deformed |
|---|---|---|---|
| conformed streets 300 | 1 800 | 7 200 | **80.0 %** |
| conformed CURVED streets 300 | 0 | 9 000 | **100 %** |
| hilly streets 300 | 1 | 8 999 | **100 %** |
| ripple 2 km / ramp 2 km / arc R = 50 | 0 | 1 011 / 1 001 / 1 000 | **100 %** |
| cornered streets 300 | 8 700 | 300 | 3.3 % |
| conformed 2 km | 1 000 | 0 | 0 % |

A conformed street network is **80–100 % deformed**, so N6 on its own would have planned those
builds and had level 2 refuse them for the pieces it still could not make. The only conform row a
conform-only port could ship is `conformed_2km` at **196 ms** — 8x smaller than hilly streets. So
**N5 is the keystone and it goes first**, which is what this cycle did; it is also the unblocking
work for 3 645–4 225 ms of conform and for the 3.3 % of the corner class that miters.

### 26.2 The deformed branch, as it is built

Six new nodes on the place side plus four on the kit side, and one `sort`:

```
pc_place_valid ─┬─ pc_packed_only ─── copy_packed (pack=1) ─────────┐
                │                                                   ├─ pc_pieces (merge)
                └─ pc_deformed_only ─ pc_deform_prep ─ copy_deformed ┘        │
                                       (pack=0, transform=0) ─ pc_deform      │
kit_unpack ─┬─ pc_kit_rank  ─────────────────────────────────────────────────┘
            └─ (pc_kit_meta reads it beside pc_kit_id)
                                pc_built (switch) ─ pc_finalize ─ pc_piece_key ─ pc_order (sort)
```

`pc_deform_prep` zeroes the target's `P` and its 3×3 so the module arrives in **its own local
space**; `pc_deform` is `place._deform_positions` in VEX — every point of the module re-read at
its own arc position, `s0 + (x - ax) * scale`, on the frame at that station.

⚠️ **`scale` IS THE SPAN OVER THE MODULE'S NOMINAL LENGTH, NOT `pc_frames`' CHORD FIT SCALE.** A
deformed piece follows the ARC where a packed one spans the CHORD. Two different numbers, one
node apart, and the mutation that swaps them is in §26.6.

⚠️ **`copytopoints` WITH `transform = 0` STILL TRANSLATES.** Probed on 22.0.398: a module point at
local x = 10 copied onto a target at x = 100 lands at 110. So the reset in `pc_deform_prep` is
not belt-and-braces with the toggle, it is the thing that works; the toggle is the braces.

⚠️ **A TARGET ATTRIBUTE NAMED IN TWO `targetattribs` ENTRIES IS APPLIED BY THE LAST ONE ONLY.**
Measured: `_pkey0` listed in both the prims entry and the points entry produced the point
attribute, **no prim attribute at all**, no warning, and every prim key reading 0. The piece
index crosses on the POINT and `pc_finalize` lifts it off the prim's own first point.

### 26.3 The output's ORDER is a number, and it needed two of them

`place.build` materialises pass B in JOB ORDER, appending **one packed prim and one point** per
rigid piece and **the module's whole polygon soup** per deformed one. `output_guard_parity`
compares every prim IN ORDER and the entire `P` array element by element, so two streams merged in
stream order is a failed parity, not a cosmetic difference.

The key is `_pkey0 * PC_PIECE_SPAN + <index within the piece>`, on both classes:

* `_pkey0` is the plan point's own number, stamped in `pc_proto` — the last node upstream of the
  packed/deformed split that still sees every piece in plan order;
* the index within the piece is `_srcrank` (prim) and `_srcpt` (point), written **on the kit** by
  `pc_kit_rank` and carried onto every copy by `copytopoints` for free;
* `PC_PIECE_SPAN` is 65 536, declared once in `pc_path.h`, and `pc_deform_gate` **REFUSES the
  build** when `_kit_rank_max` reaches it rather than shipping a permuted fence.

⚠️ **`pc_kit_rank` IS A SEPARATE BRANCH OFF `kit_unpack`, NOT AN EDIT OF IT**, because
`copy_packed` copies its source with `pack = 1`: a `_srcpt` written on the shared stream would be
baked INSIDE every packed prim's embedded geometry, where `_scrub` cannot reach it.

⚠️ **AND THE SORT'S PRECISION WAS PROBED, NOT ASSUMED.** `sort` by attribute is exact at
3.07e8 over 300 000 points (strictly increasing, 15 ms) — a float32 read would have quantised the
key at 1.7e7 and produced a permutation of the right answer, silently.

⚠️ **AN ALL-PACKED BUILD MUST NOT SEE THE DEFORMED BRANCH AT ALL**, which is what `pc_built` is
for. An EMPTY `copy_deformed` still carries its source's attribute DEFINITIONS, so merging it into
a run with nothing to deform publishes `pc_local` — and any `uv`/`N`/`Cd` an artist's kit carries —
as point attributes on a fence where the reference has none. That is a differing point attribute
LIST on all 92 cases.

### 26.4 Two divergences the branch found on the way, both 100 % of their build

**`pc_warn_collate` counted warnings PER PRIM where `place._collate_warnings` counts per ELEMENT.**
Identical on every build the branch could previously make — a packed piece is one prim — and on
`J_coarse_bend` the native output read `pc_warn_bend_resolution:6` against the reference's `:1`:
six prims, one element. The fan-out stays per prim (`_stamp_bulk` writes the warning onto every
prim of the piece); the COUNT takes `_srcrank == 0`, which is "this prim is the first of its
element" and reads 0 on a packed prim, which is the same sentence.

**`pc_finalize` stamped `pc_deformed = 0` as a CONSTANT**, with a comment explaining that
`copy_packed` is the packed branch. Both branches feed that node now. The flag rides across
`copytopoints` from the plan point the gate wrote it on.

Neither was reachable before this cycle, and both are the shape §20.2 and D223 shipped: a value
that is right by construction until the construction changes.

### 26.5 The guard fails safe, and what it refuses is a per-piece question now

⚠️ **LEVEL 1's DEFORM BOUND IS DELETED, NOT LEFT EVALUATING INTO NOTHING.** Level 1 answered the
deform question three ways in three cycles: an UPPER bound (§18 — refuses every arc in existence),
a LOWER bound (§23 — "will a piece CERTAINLY unpack"), and now **nothing**. A bound that predicts
a refusal which no longer exists is not conservative, it is WRONG: it would refuse exactly the
1 237 ms this cycle removed. `_bend_bound`, its two `pc_cfg` operands, `bend_bound_ok`'s third
value and the whole-curve kink walk that fed them are gone; `pc_envelope` measures **0.0595–0.0602
µs/piece** where it measured 0.47–0.50, and its ceiling drops 1.28 → 0.21.

**What replaces it is three PER-PIECE refusals in `pc_deform_gate`, each explicit:**

| refusal | why it is not ported | reachable through the parm face? |
|---|---|---|
| a **SLICED** piece | 4.6's slice caps are N7 | no — level 1 refuses `fill = tile` first |
| the **order key** would overflow | a module with ≥ 65 536 prims or points | no — not a fixture |
| **D31's frame TRANSPORT** could flip on this span | `_transport` is a PREFIX SCAN over the piece's stations; a per-point wrangle cannot evaluate it in O(1) | **yes** — an overhanging crest |

`pc_frames_transportable` (pc_gate.h) asks whether the flip is REACHABLE over the piece's span:
do any two of the path's distinct directions oppose in plan, and is any of them without a
horizontal direction at all (`_frame`'s own `(0,0,1)` fallback). Where the piece is answerable
`_transport` provably returns its input untouched, so `pc_deform.vfl` reproduces it **by not
implementing it** — and stays one parallel wrangle.

⚠️ **THE SAMPLE SET IS THE PATH'S DIRECTIONS, NOT THE MODULE'S STATIONS, AND THAT IS WHY IT IS
SOUND.** A station's tangent is the direction of whatever segment its arclength lands in, so the
station tangents are a SUBSET of the segment directions over the span; the directions change only
at a KINK, so sampling the span's start, every interior kink and its end visits every one. The
span asked about is the STATION span, not the planned one — D20 lets a module overhang, so the
last station can sit past `sb`.

⚠️ **THE FIRST FIXTURE FOR THAT REFUSAL ASSERTED NOTHING, and the check now says so.** A FLAT
hairpin reverses by turning ~180°, which `pc_corners` reads as a CORNER and level 1 refuses
outright — so the build never reached level 2 and `guard_deform_refusals` passed while exercising
nothing below `pc_envelope`. The reversal has to be in the VERTICAL plane: an overhanging crest
reverses `across` (the horizontal normal both z-modes build from) while the 3D turn between the
two segments is 52°, under the 60 the fixture sets. `guard_deform_refusals` asserts **level 1
ADMITS** as a separate row from "the output took the reference", because the second is satisfied
just as well by the first being false.

`P_crest_bend` in the scene suite is that shape, and it is the one case of 92 that legitimately
pays the L1-admit / L2-refuse double cook. `no_case_pays_the_guard_fallback` compares the set
against a DECLARATION (`GUARD_FALLBACK_CASES`) rather than against zero, so the set growing is a
red check.

`bench_guard_fallback`'s lever moved with the class: it used to force LEVEL 1 open on a rippled
run — a run level 1 and level 2 both admit now, so the same mutation would have measured the
ADMITTED path and gone green while measuring nothing. It refuses ONE PIECE at the gate instead,
which is exactly what an unanswerable piece does: 1.50x at 2 km and 1.62x at 20 km, ceiling 1.8x.

### 26.6 Eleven source mutations, with the .hda rebuilt

Each row edits a real source file, **rebuilds the .hda** (so the asset and its declaration move
together) and runs `run_native_checks.py` whole. The tree is restored and the asset rebuilt after
every row.

| # | mutation | caught by |
|---|---|---|
| 1 | `pc_deform`'s span scale → 1.0 (the "nominal length IS the span" assumption) | `guard_deform_ladder`, `output_guard_parity` |
| 2 | **`_pkey` loses its within-piece rank** | **SURVIVED — see below** → `piece_order_key_is_total` |
| 3 | **`_pkeyp` loses its within-piece rank** | **SURVIVED — see below** → `piece_order_key_is_total` |
| 4 | `pc_order`'s `primsort` → `none` (ship in merge order) | `guard_deform_ladder`, `output_guard_parity` |
| 5 | D25's `pc_warn_bend_resolution` never appended | `output_guard_parity` |
| 6 | `pc_warn_collate` counts per PRIM again | `output_guard_parity` |
| 7 | `pc_finalize` stamps `pc_deformed = 0` as a constant | `guard_deform_ladder`, `output_guard_parity` |
| 8 | `pc_deform_prep` stops zeroing the target's `P` | `guard_deform_ladder`, `output_guard_parity` |
| 9 | level 2 counts `pc_packed_only` again (N5's own undo) | `deform_wrangle_rig_really_deforms`, `guard_deform_ladder`, `mutation_deform_refusals`, `mutation_every_wrangle_ceiling_bites_deformed`, `no_case_pays_the_guard_fallback`, `output_guard_cost` |
| 10 | D31's transport refusal removed | `guard_deform_refusals`, `mutation_deform_refusals`, `no_case_pays_the_guard_fallback`, `output_guard_parity`, `place_stamp_owed_is_live` |
| 11 | level 1 refuses everything (the shape of the deleted bound) | `bench_guard_fallback`, `config_cost_per_module`, `guard_deform_ladder`, `guard_deform_refusals`, `guard_padding_parity`, `guard_spline_attr_types` |

⚠️ **TWO SURVIVED, AND THEY ARE THE SAME HOLE.** Dropping the within-piece term from either key —
so every prim and every point of one deformed piece carries the SAME number — left the suite at
**136 `[PASS]` / 0**, `output_guard_parity` included. The reason is that Houdini's `sort` happens
to be **stable** here, so equal keys kept the order `copytopoints` emitted them in, which is
already the reference's. That is luck resting on an undocumented property: the whole point of the
second term is that the key is **TOTAL**, so the answer does not depend on how the sort breaks
ties. Nothing asserted totality, so nothing could see the term go.

`piece_order_key_is_total` asserts the property directly — on a build with 30 deformed pieces
whose largest contributes 34 prims, **0 duplicate prim keys and 0 duplicate point keys out of
1 052 / 1 112** — and it is independent of the sort, of stability and of the fixture's shape. Both
mutations were re-run against it and **both are caught**. **No survivors remain.**

⚠️ **AND THE RESTORED .hda's HASH MOVED, WHICH IS A THIRD FINDING.** `12ac2248 → 7f521a91` after
restoring every mutated file. It is not a failed restore: **two consecutive rebuilds of an
UNCHANGED tree produce different bytes** (138 512 vs 138 519, differing from byte 45 onward — a
container length or timestamp, and the archive is compressed so everything shifts). Structurally
the two are the same asset: **61 nodes, every node's type, every input wire and every parameter's
raw value identical**, compared through HOM rather than through a digest. §25 recorded the .hda as
"byte-identical before and after three rebuilds", and the committed file was indeed unchanged
across `935d2ef`…`b15a338` — so this is either new to this build or was previously masked. Either
way **a hash is not a valid A/B oracle for this asset**; the network comparison is. **D240.**

### 26.7 D25's warning, and the station table it needed

`_bend_deviation` is the one warning only a DEFORMED piece can raise, and without it the native
branch ships an element the reference stamps `pc_warn_bend_resolution` on and it does not — which
`output_guard_parity` reads as a missing PRIM ATTRIBUTE on the whole build. It is computed in
`pc_deform_gate`, which already holds the span and the verdict, from `pc_kit_meta`'s per-module
station table.

⚠️ **THE TABLE STORES THE RAW LOCAL x, NOT `x - ax`.** A point's `P` is float32 and the difference
of two float32s is not one, so writing the difference into an array attribute would quantise the
very numbers the deform is built on. The consumer subtracts `ax` itself, in 64 bits, exactly as
`place._stations` does — and the dedupe is unaffected, since `(x2 - ax) - (x1 - ax)` is `x2 - x1`.
The LAST run of a repeated `pc_name` wins, which is `Kit._by_name`'s rule and `copytopoints`'
`useidattrib`'s.

### 26.8 The ceilings: two tables, because the shape decides the cost

`WRANGLE_CEILING_US` is recorded on a STRAIGHT run where every piece stays packed. Five of the
asset's 23 wrangles never cook there, and four more see the module's whole polygon soup instead of
one prim per piece — so folding a deformed shape into it would move eighteen recorded rows by the
prim-to-piece ratio and guard neither shape. `WRANGLE_DEFORM_CEILING_US` is the deformed shape's
own table, same recipe, same two sizes, over a 0.6 m ripple at 4 466 and 17 852 pieces:

| node | deformed µs/piece | straight µs/piece | ceiling |
|---|---|---|---|
| `pc_deform` | 0.5755 – 0.5890 | — | 1.53 |
| `pc_deform_prep` | 0.1023 – 0.1059 | — | 0.32 |
| `pc_kit_meta` | 0.0451 – 0.0665 | — | 0.22 |
| `pc_kit_rank` | 0.0119 – 0.0129 | — | 0.09 |
| `pc_piece_key` | 0.2171 – 0.2325 | — | 0.64 |
| `pc_deform_gate` | 0.9103 – 1.0080 | 0.2355 – 0.2533 | 2.58 |
| `pc_warn_collate` | 1.5846 – 1.6232 | 0.2358 – 0.2490 | 4.12 |
| `pc_finalize` | 0.3030 – 0.3214 | 0.1119 – 0.1279 | 0.86 |

⚠️ **THE TWO MOST EXPENSIVE NODES ON A DEFORMED BUILD ARE NOT WRANGLES, so no ceiling row guards
them.** At 17 852 pieces / ~304 000 prims, `pc_order` (the sort) is **125–131 ms** and `pc_pieces`
(the merge) 24–28 ms. On the 20 km ripple that is 130 ms of a 1 007 ms cook — the second-largest
item after `pc_plan_solve`'s 568 ms. That is the price of putting `place.build`'s job order back
together and it is recorded here so nobody rediscovers it as a surprise. **D231**: if a cheaper
ordering exists it is worth a cycle, but the sort is correct and the correctness is not
negotiable.

⚠️ **`deform_wrangle_rig_really_deforms` ASSERTS HALF, NOT ALL, AND THE HALF IS D27.** The starter
kit alternates `post` (rigid) with `panel`, and `_needs_deform` returns False on a rigid module
before it measures anything — so a ripple no module can chord still leaves every post packed
(8 925 of 17 852). What the row asserts is that all five deform-branch nodes COOKED.

### 26.9 13.9 N6's deciding experiment, committed

Conform is the largest refused class left by 4.5x, so the next cycle's first question is whether
N6 is a PORT of the drape's numbers or a REWRITE of them. That is one measurement, and it is
`conform_drop_is_portable_to_vex`.

`conform.Surface.drop` is `hou.Geometry.intersect` twice (down-axis, then back no further than the
hit already found, nearest wins, `tolerance = 1e-6`, `min_hit = 0.0`); `drop_many` is the `ray`
VERB, already measured bit-identical to it. A native conform is a THIRD implementation — VEX's own
`intersect()` — over 200 queries on five surfaces: an exact-tie flat sheet, a rational slope, an
IRRATIONAL slope (the committed fixtures' `y = 0.25x` is exactly representable and hid a real
divergence for a cycle, D111), a REVERSED winding (D52), and the same irrational ramp at 20 km.

**0 hit-flag mismatches, worst 1.776e-15 m. The drape is a PORT.**

⚠️ **AND THE FIRST READING OF IT SAID NO.** Compared as DOUBLES the two disagree by **3.815e-07 m**
at fixture scale and **9.375e-04 m at 20 km**, which would fail `_first_difference`'s 1e-9
rounding outright. The whole divergence is in x and z — the two components a −Y drop never moves.
VEX's `intersect` in a 64-bit wrangle hands back the query's own DOUBLE coordinates and
`hou.Geometry.intersect` hands back float32 ones, so it is D111's finding **from the other side**:
the REFERENCE is the quantised one. It disappears in the float32 `P` storage the output is
actually compared in. The condition rides with the check: read the drop off the AXIS COMPONENT and
rebuild the position from the query, exactly as `drop_many` already does.

### 26.10 What still takes the reference, and why — stated rather than implied

| class | Python | why it is refused | where the refusal lives |
|---|---|---|---|
| **a SURFACE** (4.5 conform) | 3 645–4 225 ms on the citygen mix | nothing about the drape is ported | `[cfg:has_surface]`, and `pc_proto` declares every piece unanswerable |
| **a CORNER** (4.3) | 806 ms on the citygen mix | a corner reserves span off both ends of every leg, so the same element address names a different span in the two plans | `[vex:corners]` |
| a **fillet radius** | — | 4.3, N8 | `[cfg:fillet_radius]` |
| a **slope fix** (D26) | — | not ported | `[cfg:fix_slope]` |
| **D98's flatten-under** | — | not ported | `[cfg:flatten_stepped]` |
| **D99's band** | — | a per-point rule; the gate declares itself unanswerable | `[cfg:flat_band]` + the gate |
| `fill = tile` | 58 ms | 4.6's slice caps, N7 | `[cfg:fill]` + the gate's own assertion of it |
| `display = plan\|proxy` | 50 ms | D81/D82 | `[cfg:display]` |
| Colour Warnings | 54 ms | `colour_warnings`' `Cd` | `[cfg:show_warnings]` |
| a style or kit that WARNED, no rules, an unreadable `pc_cond`, a rule naming a module the kit has not | — | unchanged | `[cfg:*]` |
| a DUPLICATED curve id, a MARKER on a curve vertex, an artist attribute at a storage the chain cannot read, nothing to build | — | unchanged | `[vex:*]` |

**These are correct.** The guard fails safe, the output is the reference's bit for bit, and every
one of them is a row `guard_refusal_list_is_true` checks in both directions.

### 26.11 Numbers this cycle is standing on

* `output_guard_parity` **28 native / 64 reference, identical** (was 18 / 74).
* `output_guard_cost`, min over 3 interleaved repetitions: `straight_2km` 0.90x, `arc_2km` 0.98x,
  `curved_streets_300` 0.23x, **`arc_R50_2km` 0.78x, `bumpy_2km` 0.77x, `ramp_2km` 0.75x** — the
  three rows that flipped sides are not merely equal to the reference, they are **faster**;
  `corner` 1.02x refused.
* `guard_deform_ladder`: all six arcs of §23's ladder take the native chain and ship the
  reference's own fence — including `arc_R20_step0.05` and `arc_R50_step0.1`, which were PART B's
  two L2-refusal rows.
* **THE NUMBER, per class and per size**: straight **3.2 % Python at 2 km, 0.3 % at 20 km**; a
  curved R = 2 000 arc **3.2 % / 0.3 %**; **300 curved streets 2.1 %**; and the classes N5 opened —
  **rippled 2.0 % at 2 km and 0.2 % at 20 km, hilly streets 0.7 %** — against 96–99 % before. The
  classes the guard still refuses read **cornered 93–98 %, conformed 96.5–99.6 %**, which is
  correct.
* All the Python on an admitted build is still `config`, a FIXED 2.2–2.6 ms.

### 26.12 Where to start the audit

**Dev-loop rule 0 is NOT discharged for this cycle.** The honest words for everything above are
"implemented, verified by the suite, not independently audited". Start here:

1. **`pc_deform.vfl` against `place._deform_positions`, line for line.** The frame transport is
   the one thing deliberately NOT ported; check that `pc_frames_transportable`'s sample set really
   is a superset of the station tangents, and that `_transport` is the only prefix-scan in that
   function.
2. **The order key.** `_pkey0` is taken in `pc_proto`, which is upstream of `pc_place_valid`'s
   blast — check that nothing between the plan and the split can reorder points, and that a kit
   with two adjacent packed prims of the same `pc_name` cannot make two pieces' keys collide.
3. **`pc_built`'s switch.** Its selector is an `npoints()` expression on a sibling. Confirm the
   all-packed path is byte-identical to the pre-N5 build (`geometry_digest` on the 92 cases is the
   instrument) and that the expression cannot evaluate against a stale cook.
4. **The two ceiling tables.** `WRANGLE_DEFORM_CEILING_US` names eight rows and the asset has 23
   wrangles; four are in both tables. Check that no wrangle is guarded on a shape it never runs.
5. **`pc_order` at 125–131 ms.** Is the sort avoidable, and is D231 a cycle or a footnote?
6. **The conform experiment.** It compares in float32 because that is the output's storage —
   check that no consumer of a drop reads it at 64 bits before it lands there (`_bend_deviation`
   and `span_deviation` both do arithmetic on sampled positions).

### 26.13 Decisions taken this cycle (§10 continues here — D226–D230 are §24's and §25 reused
two of those numbers by accident, so this cycle starts at D231)

- **D231 — putting `place.build`'s JOB ORDER back together costs 125–131 ms, and it is the second
  largest item on a deformed cook.** The native branch builds packed prims and deformed polygons
  as two `copytopoints` streams; the reference interleaves them. `pc_order` (a `sort`) and
  `pc_pieces` (a `merge`) are 125–131 ms and 24–28 ms at 17 852 pieces / ~304 000 prims, against
  `pc_plan_solve`'s 568 ms on the same build. Recorded, not fixed. If a cheaper ordering exists it
  is worth a cycle; the sort is CORRECT and that is not negotiable.
- **D232 — a warning's COUNT is per element while its STAMP is per prim, and those were the same
  number until a piece could be more than one prim.** `place._collate_warnings` counts JOBS;
  `_stamp_bulk` writes the warning onto every prim the piece produced. `pc_warn_collate` counted
  prims, which agreed with the reference on every build the branch could previously make and read
  `pc_warn_bend_resolution:6` against `:1` the moment one could not. `_srcrank == 0` is "the first
  prim of this element" and reads 0 on a packed prim, which is the same sentence.
- **D233 — a heuristic that predicts a refusal which no longer exists is DELETED, not left
  evaluating into nothing.** Level 1's deform bound was exact, cheap and, after N5, actively
  wrong: it would have refused the 1 237 ms N5 removed. `_bend_bound`, its two `pc_cfg` operands,
  `bend_bound_ok` and the whole-curve kink walk went with it, and `pc_envelope` dropped from
  0.47–0.50 to 0.0595–0.0602 µs/piece. A dead heuristic reads exactly like a live one.
- **D234 — two measured `copytopoints` facts, both silent when wrong.** (a) With
  `transform = 0` a copy is STILL TRANSLATED to its target point (probed: local x 10 onto a target
  at 100 lands at 110), so a copy that must arrive in source-local space needs the target's `P`
  zeroed, not the toggle cleared. (b) A target attribute named in TWO `targetattribs` entries is
  applied by the LAST one only — `_pkey0` in both the prims and the points entry produced the
  point attribute, **no prim attribute at all**, no warning, and every prim key reading 0.
- **D235 — the output's ORDER is a contract, and it needs a key on BOTH classes.** `output_guard_parity`
  compares every prim in order and the entire `P` array element by element. The key is
  `_pkey0 * PC_PIECE_SPAN + <index within the piece>`, where the piece index is stamped in
  `pc_proto` (the last node upstream of the packed/deformed split that still sees plan order) and
  the within-piece index rides free off the KIT through `copytopoints`. The span is CHECKED
  (`_kit_rank_max`) and the build is refused rather than shipped permuted. Probed: `sort` by
  attribute is exact at 3.07e8 over 300 000 points; a float32 read would have quantised at 1.7e7.
- **D236 — a fixture for a LEVEL 2 refusal must be checked to REACH level 2.** The first
  `guard_deform_refusals` used a flat hairpin, which reverses by turning ~180° — a CORNER, which
  level 1 refuses outright, so the row passed while exercising nothing below `pc_envelope`. "The
  output took the reference" is satisfied just as well by level 1 refusing, so **level 1 admitting
  is a separate assertion** and the reversal has to be in the vertical plane (an overhanging
  crest: `across` reverses while the 3D turn stays under the corner threshold).
- **D237 — a cost-ceiling table is per SHAPE.** Five wrangles never cook on a straight run and four
  more see the module's whole polygon soup on a deformed one, so one table cannot guard both:
  folding a deformed shape into `WRANGLE_CEILING_US` would move eighteen recorded rows by the
  prim-to-piece ratio. `WRANGLE_DEFORM_CEILING_US` is the second table; a name in both is a node
  whose cost is shape-dependent, and a wrangle in neither is the fault.
- **D238 — 13.9 N6's drape is a PORT, and the reading is the condition.** VEX's `intersect()`
  reproduces `conform.Surface.drop` to 1.776e-15 m over 200 queries on five surfaces **in the
  float32 `P` storage the output is compared in** — but to 3.815e-07 m at fixture scale and
  9.375e-04 m at 20 km as DOUBLES, all of it in the two components a −Y drop never moves, because
  VEX returns the query's double coordinates and `hou.Geometry.intersect` returns float32 ones.
  Read the drop off the AXIS COMPONENT and rebuild from the query, as `drop_many` already does.
- **D239 — a conform benchmark whose runs are off the surface measures D53's miss path.** The
  first pass of §26.1 ran 300 streets 1 600 × 1 000 m across over `conform_bench.terrain`'s
  660 × 360 m default and reported 1 742 ms; a terrain that covers them reports 3 645 ms. Any
  conform fixture owes an assertion that the run is ON the surface.

- **D240 - the .hda is NOT byte-reproducible, so a hash is not a valid A/B oracle for it.**
  Two consecutive rebuilds of an unchanged tree differ from byte 45 onward and by seven
  bytes of total length; structurally they are the same asset (61 nodes, every type, every
  input wire and every parameter raw value identical, compared through HOM). Compare the
  NETWORK, or compare the OUTPUT - never the file.
## 27. Native network build log — cycle N-4F (the §26 audit, worked through)

**Read this first.** An independent audit of §26 returned **twenty findings**. Every one was
reproduced from a pristine `git archive HEAD` export before it was touched, and every one is closed
as a **standing check with a mutation that reddens**. Three of them were not the finding the auditor
wrote: fixing the instrument the auditor asked for immediately exposed **two further shipped
divergences on the DEFAULT path** that no check in the tree could see, and a third measurement
**contradicted the auditor's own proposed fix**. All three are recorded below as found, not as
hoped.

### 27.1 The four that shipped WRONG GEOMETRY on a build the guard admits

Each one reproduced on the pristine HEAD export, `Stage = output` against `Stage = reference`, level
1 and level 2 both admitting, no node warning on either side.

| # | what | measured on HEAD | now |
|---|---|---|---|
| 1 | **`attr:<name>` at an ARRAY or DICT storage** — `pc_sections.vfl`'s "present but unreadable" test was `t == 3`, and `primattribtype` returns 3 only for an **int** array | a FLOAT array `w = (7.5, 1.0)` under `attr:w eq 0.0` built **12 `gate` prims natively against the reference's 10 `panel`**; a STRING array and a DICT did the same; `ge`, `lt` and `ne` diverged identically. **12 divergences over 4 storages x 4 operators.** The int-array control was correctly refused | `t >= 3`, with the re-probed table in the comment. **0 divergences.** D241 |
| 2 | **`markerData:<k>` at a non-scalar slot** — the type probe asked only "is this a string" and everything else fell to `float v = md[k]` | `{"v": (1.0, 2.0, 3.0)}` under `markerData:v eq 0.0` built **1 gate + 10 panels natively against the reference's 10 panels**; a list and a nested dict the same, under `eq`, `lt` and `ge`. **9 divergences** | probes positively for `string`/`int`/`float` and publishes `!unreadable:` for everything else — the answer `pc_cond_subject` already models. **0.** D242 |
| 3 | **a marker-data KEY that `json_dumps` escapes** — the probe searched the whole marker's dump for the key's SOURCE spelling | `{'a"b': "x"}` fired a gate the artist never asked for; `{u"ké": "x"}` **dropped the gate the artist did ask for** — the failure runs in both directions. **6 divergences over 3 keys x 2 operators** | the dump is per KEY (a one-key dict, `removeindex`), so the key text is out of the question entirely. **0.** D242 |
| 4 | **`pc_row_warns` (D139's row channel)** | `place.build` **RAISED** `hou.OperationFailed: Could not add attribute` on a string-array storage while the native chain shipped 12 prims — parity was not different, it was **undefined**. With that fixed, the reference publishes `w1`/`w2` prim attributes and the native chain publishes **nothing**: a schema divergence the guard admitted | the reader drops tokens that are not attribute names (D258) and **level 1 REFUSES any build carrying a non-empty `pc_row_warns`** (D263), with an empty-value control row proving the refusal is not a blanket one |

### 27.2 The two the FIXED INSTRUMENT found, which no finding named

⚠️ **This is the part worth reading.** Finding 1 of the audit was that `_snapshot` — the oracle
`output_guard_parity` and eleven other rows compare through — recorded point attributes **by name
only** and had no storage dimension at all. Giving it point-attribute VALUES and a real storage
dimension (`hou.Attrib.numericDataType()`, which separates fpreal32 from fpreal64 and Int32 from
Int64 where `dataType()` cannot) turned **29 of 93 cases red on the first run**:

* **every int of 3.4's stamp shipped at int64 where `place.build` declares int32.** A 64-bit wrangle
  writes `i@` at int64; `geo.addAttrib(Prim, name, 0)` is int32. `pc_elem_key`, `pc_section`,
  `pc_generated`, `pc_deformed`, `pc_corner_cut`, `pc_replaced` and every `pc_warn_*` were **twice
  the reference's storage on every admitted build**, and `dataType()` reads `attribData.Int` for
  both — so `output_guard_parity` printed "identical" over all 92 cases for the whole of §26.
* and the fix has an ORDER: `pc_warn_collate` **creates** prim attributes at cook time
  (`setprimattrib` with a name that comes from data), so with `pc_out_cast` upstream of it
  `pc_warn_bend_resolution` stayed at int64 while every other int was correct. The cast is the
  **last** node of the stage now. **D262.**

Both are exactly D223's own rule — *the storage of an attribute is part of its contract* — applied
to the OUTPUT side, which is where nothing had ever applied it.

### 27.3 THE PYTHON SHARE, measured this cycle, per class

`hou.perfMon` per-node `Cook - ms` on the shipped asset at `Stage = output`, min over 3 dirtied
cooks; PYTHON is the sum over the nodes whose type is `python`.

| class | 2 km | 20 km | takes | the top native node |
|---|---|---|---|---|
| straight | **2.6 %** (2.2 of 84.5 ms) | **0.2 %** (2.4 of 1 019.7 ms) | NATIVE | `pc_plan_solve` 67–85 % |
| curved (R = 2 000 arc) | **2.5 %** (2.5 of 97.6 ms) | **0.2 %** (2.5 of 1 238.7 ms) | NATIVE | `pc_plan_solve` 68–86 % |
| hilly / elevated (deformed) | **1.9 %** (2.6 of 136.2 ms) | **0.2 %** (2.5 of 1 261.3 ms) | NATIVE | `pc_plan_solve` 48–64 %, `pc_order` 10–11 % |
| **cornered** | **95.2 %** (50.7 of 53.2 ms) | **97.5 %** (550.6 of 564.9 ms) | REFERENCE | `kernel` 91–97 % — 4.3 is **N8** |
| **conformed** | **97.1 %** (207.6 of 213.7 ms) | **96.9 %** (2 142.1 of 2 209.6 ms) | REFERENCE | `kernel` 96–97 % — 4.5 is **N6** |
| 300 hilly streets | **2.1 %** (2.8 of 138.2 ms) | — | NATIVE | `pc_order` 24.8 %, `pc_plan_emit` 11.9 % |
| 1 200 hilly streets | **0.5 %** (2.4 of 463.0 ms) | — | NATIVE | `pc_order` 30.4 %, `pc_plan_emit` 12.8 % |

**The classes that still take the reference are exactly two: CORNERED (4.3, N8) and CONFORMED
(4.5, N6).** Everything else the parm face can build — straight, curved, arc, hilly, ramped,
rippled, marked, padded, anchored, random, and 300–1 200 street networks — runs native.
`config` is the only Python SOP that cooks on an admitted build, at a fixed **2.2–2.8 ms**.

### 27.4 The two efficiency criticals, and the one the auditor's fix did NOT fix

**(a) `pc_fill` was QUADRATIC in the anchor count, and it shipped on two MAIN-page parms.**
`pc_fill`'s twelve plan arrays are `export`, so VEX copied the whole growing plan in and out once
per gap, and an `evenly` or `marker` rule calls it once per gap. Measured on the pristine HEAD
export, a 2 km straight with `Evenly Spaced Piece = post`, guard ADMITTING:
...
Doubling the anchors QUADRUPLED the node. Fixed by giving `pc_fill` fresh local arrays and
appending after it returns — semantically identical, because `pc_fill` reads its incoming arrays
exactly once as `int entry = len(o_slot)` (**D243**) — and by hoisting the pool and the slot context
out of the per-gap call (**D244**). Same fixtures now: **1.94x / 2.47x / 2.99x**, and
`pc_plan_solve` at 4 000 anchors went **47 168 ms → 719 ms**.

**(b) `Piece Order = Random` is 3.06x, and the auditor's proposed fix accounts for 6 % of it.**
The audit attributed ~230 us/piece to `pc_random01` re-seeding MT19937 across `export int mt[]`
function boundaries and proposed inlining, saying explicitly that the magnitude was inferred and not
measured. **It was measured this cycle and the inference is wrong.** Inlining the three functions
into one and truncating the first twist to the two words `genrand_res53` actually reads (provably
identical — mt[0]' and mt[1]' depend only on words the loop has not yet written) took the whole draw
from **110 us to 98 us**, and `pc_plan_solve` under `random` from 251 ms to 217 ms. Carrying
`mt[i-1]` in a register and replacing the two-element key array with scalars took it to **184 ms**.
End to end the 2 km random build went **3.96x → 3.06x**.

⚠️ **The floor is VEX itself, and it is measured.** `init_by_array` is ~1 870 strictly sequential
array steps; a bare 1 872-iteration scalar loop in a single-threaded VEX snippet costs **45 us**,
and the whole draw 98 us, against CPython's 1–2 us of C. `pc_plan_solve`'s parallel unit is the
SECTION (D222), so one long curve runs it on ONE thread. **There is no cheaper VEX.** Getting this
row under the guard's 1.6x contract needs the draws BATCHED into a parallel wrangle, which is a
two-pass solve. **D260, and it is the honest open item of this cycle.**

Both shapes now have standing rows: `output_guard_cost` gained `evenly_2km` (2.6x ceiling, reads
2.08x) and `random_2km` (4.0x ceiling, reads 3.11x), and `guard_anchor_cost_is_linear` bounds the
per-anchor GROWTH at 1.6x (reads 0.99–1.22x) with `mutation_anchor_fill_copies_the_plan` restoring
the shipped code and blowing it. **D259.**

### 27.5 The checks that could not fail, and what each one is now

| finding | what it really asserted | now |
|---|---|---|
| `_snapshot` compares no point VALUES and no storage | `v@pc_local = local * 1.5` and `= set(0,0,0)` (2.0 m out) both left `_first_difference` EMPTY | point columns read in bulk + a storage dimension, and `output_snapshot_sees_the_deformed_branch` mutates the shipped `pc_deform` twice and `pc_out_cast.numcasts` once and demands each be NAMED. **D246** |
| `conform_drop_is_portable_to_vex` rounds both sides to float32 first | the advertised 1e-12 m ceiling was a half-float32-ULP tolerance (~0.98 mm at 20 km); the row that decides N6 printed `0.000e+00` while the two implementations disagreed by **9.375e-04 m** | TWO ceilings: the f32 one unchanged, plus the RAW double difference against a **scale-relative** ceiling of two float32 ULP (`intersect()` IS a float32 ray test, so that is the honest floor). Reads **4.933e-08 relative**. **D247** |
| `solve_cost_is_flat_in_piece_count` reddens on an unmutated build | the ceiling was an ABSOLUTE us/piece figure from one machine: 25.97 idle, 33.46–34.56 loaded, against 34.0 | a fixed 3 000 000-iteration integer CALIBRATOR cooked interleaved in the same repetitions, and the ceiling is a RATIO against it. Two runs 33 % apart in absolute terms read **0.469 and 0.451** — stable to 4 %. **D248** |
| `mutation_deform_refusals` "still refused" x3 | deleting `if (i@pc_sliced) …` changed nothing because the TRANSPORT refusal already refused the fixture; a renamed `pc_sliced` would have printed the same words | each refusal FORCED on one deformed piece of an ADMITTED build, two-sided (with the line → refuses, without → admits), plus the four attributes they read asserted present on the gate's own inputs. **D256** |
| `guard_deform_refusals`' parity half | level 2 refuses both rows, so `Stage = output` IS `OUT_reference` and the diff is empty by construction | kept as "the switch really did select the reference" and **labelled as trivially satisfied** in the row's own text. **D255** |
| `place_deformed_covers_the_reference` on 3 elements | a SET comparison on ~1 element | `place_deformed_is_not_empty` (floor 40) beside it, and `DM_ripple_deformed` added to `cases.build_all()` — the only deformed case `_place_out_of_scope` does not skip. **148 elements. D253** |
| the D202 array fixtures were TUPLES | `addAttrib(Prim, name, [8,1,2])` is a size-3 int attribute: `isArrayType()` False. The array branch was dead code in every run, which is why three storages shipped wrong | `addArrayAttrib` + an `isArrayType()` assertion inside the fixture + a dict row + the `eq 0.0` / `lt 1.0` / `ge 0.0` operands the old set happened to agree on. **D257** |
| the N5 gate-image pair asserts prim COUNTS | `n5_polys` held the LAST loop iteration, so the "image has geometry" row measured the REFERENCE image; and two crops with equal prim counts and different vertices passed | per-tag counts, both asserted, and the two crops compared on **every rounded POINT POSITION**. **D254** |

### 27.6 The declarations that had gone stale

* **`GUARD_BEND_LADDER`'s two expectation columns contradicted their only consumer** on four of six
  rows — the table said REFUSED where `guard_deform_ladder` asserts ADMITTED. Deleted, with
  `GUARD_BEND_FALLBACK_CEILING` (referenced nowhere) and the ⚠️ block describing a check that no
  longer exists. **D251**
* **`bench_deform_20km` was cited by a comment and did not exist** — the third time this project has
  shipped a citation of an unwritten oracle. It is written: the shipped asset on a 20 km ripple,
...
* **`NATIVE_STAGES` / `OUTPUT_STAGE` named none of the ten nodes 13.9 N5 added** — the same sentence
  the tables' own comment says they exist to prevent. `OUTPUT_DEFORMED_STAGE` is the deformed
  branch's own row on its own fixture (a ripple; the straight run legitimately bypasses the branch),
  and `native_stage_mutation` gained `w3d`, the same undo judged on it. **D250**

### 27.7 Suites, from a pristine `git archive HEAD` export
...
| `scale_gate` | **0 failing rows** |
| `gate_images` | **0 failing** |
| `run_attrib_checks` | **0 failing / 0 unreviewed** |

**Baseline movement, key by key:** 90 case keys before, 91 after. **ADDED 1** (`DM_ripple_deformed`,
61 values, all new). **REMOVED 0. MOVED 0** — the only whole-record difference is a timing SENTENCE
inside `ZZ_port_tripwires/curve_sample_scaling`'s `detail` string (`warm 0.80 us` → `0.75 us`),
...
images whose drawn-segment counts are checked against the geometry (3 079 / 3 388 / 1 185 / 1 384).
PC-G2 **PASSES** — the hill in all three z-modes plus camber, `conform_contact_m` 0.0 (2 174 /
2 078 segments). PC-G3 **PASSES** — `scale_gate` 9 rows, 0 failing, 10 000 packed at 0.24–0.41 s and
the two shapes that must deform doing so at ~360 000 points. PC-G4 **PASSES** — `run_hda_checks`
31/0 and every gate image built through `style_from_parms(node)`. PC-G5 **STILL DOES NOT PASS**,
...
- **D241 — an "is it an array" test must be `>= 3`, and `primattribsize` cannot stand in for it.**
  Re-probed on 22.0.398: `primattribtype` is 0 int, 1 float, 2 string, **3 int-array, 4 float-array,
  5 string-array, 6 dict**, and every array reports `primattribsize == 1`. The comment that stood
  above the line said "3 ARRAY" and was wrong for three of the four.
- **D242 — an introspection probe may not contain the DATUM'S OWN NAME.** Searching a whole dict's
  `json_dumps` for `"<key>": {"type": "string"` fails on every key JSON escapes. Dump a ONE-KEY dict
  instead and read the FIRST `"type": "` token: the key text leaves the question. And a type probe
  with two outcomes on a three-outcome domain is a fail-open — anything that is not a scalar is
  `!unreadable:`, never a cross-type read.
- **D243 — `export` array parameters are COPIED, so an accumulator must never be passed to a
  function called in a loop.** 118x on a MAIN-page parm, invisible to 130 [PASS] / 0. The rule
  generalises: if a VEX function takes `export T[]` and the caller is in a loop, the caller passes
  FRESH arrays and appends.
- **D244 — a hoist has more than one level.** D175 hoisted the pool out of the PIECE loop; the same
  pool was still rebuilt once per GAP. Anything that is a pure function of (rule, yclass) belongs
  outside the gap loop too.
- **D245 — measure the optimisation before crediting it.** The inline the audit proposed for
  `pc_random01` is worth 6 %, not the ~100x its framing implied, and the remaining cost is VEX's own
  single-thread floor. An inferred magnitude is not a measurement, and this document says so where
  the inference was made.
- **D246 — an oracle that compares NAMES is not an oracle.** Point attributes had no values and no
  storage in `_snapshot`; two mutations 2.0 m apart moved nothing. `hou.Attrib.numericDataType()` is
  the accessor that separates fpreal32/fpreal64 and Int32/Int64 where `dataType()` cannot.
- **D247 — a tolerance applied to BOTH sides before the subtraction is not a ceiling.** `f32()` on
  both operands turns any ceiling below half a float32 ULP into a statement about storage, not about
  agreement. Assert the RAW difference against a scale-relative bound and report the quantised one
  separately.
- **D248 — an absolute timing ceiling is a property of the MACHINE.** Calibrate against a fixed
  workload cooked in the same interleaved repetitions and assert a ratio. Measured: a 33 % swing in
  the absolute figure is a 4 % swing in the ratio.
- **D249 — a multiparm is ENUMERATED, never listed.** A hand-written `class1/attribs1/...` list has
  to be edited every time an entry is added, and it was not.
- **D250 — a stage table needs a FIXTURE per branch.** The deformed branch cannot be watched on a
  straight run, where the `pc_built` switch legitimately bypasses it.
- **D251 — a declaration its own consumer contradicts is worse than no declaration.** Delete the
  columns rather than correcting them when the assertion is uniform.
- **D252 — the citation scan must read the TEST MODULES too.** That is where the runners document
  themselves, and it is where the third unwritten-oracle citation landed. Two shapes have to be
  modelled to keep it honest: keyword-argument names, and names a `check("prefix_%s" % ...)`
  composes at runtime.
- **D253 — a SET comparison needs a size floor, and the skip that feeds it must not exclude its own
  subject.** `place_packed_parity` skipped every case with no PACKED prim, which is exactly a case
  where every piece deforms.
- **D254 — a variable assigned inside a loop holds the LAST iteration.** The "this image contains
  geometry" row measured the reference image while claiming to measure the native one. And prim
  counts do not prove geometry: compare positions.
- **D255 — a comparison that is trivially satisfied says so in its own text.** A refused build
  routes `Stage = output` to the reference, so a parity diff there compares a node with itself.
- **D256 — a refusal is proved by FORCING ITS INPUT, not by deleting its line on a fixture something
  else already refuses.** And forcing cannot prove the attribute NAME is still the one upstream
  writes, so the inputs are asserted present separately.
- **D257 — a fixture must assert the property it is named for.** `addAttrib(Prim, name, [8,1,2])` is
  a TUPLE; five rows called `iarray`/`sarray` never executed the array branch, which is how three
  storages shipped wrong under five PASSing rows. This is the project's own recorded lesson
  happening inside the fix that lesson was written for.
- **D258 — artist-authored TEXT becomes attribute NAMES, so it is input and must be validated.**
  `pc_row_warns` at the wrong storage made `place.build` raise where the native chain built happily:
  warn-never-block broken on the Python side, and parity undefined rather than merely different.
- **D259 — a cost fixture built on the DEFAULT style measures the default style.** Every cost row in
  the suite left the slot parms alone, so the two MAIN-page parms that carried a 118x and a 4.0x
  regression were unreachable by any committed check.
- **D260 — OPEN: `Piece Order = Random` is 3.06x the reference and cannot be made 1.6x in VEX.**
  `random.Random(s).random()` is ~1 870 sequential array steps and VEX's single-thread floor is
  ~24 ns a step. The fix is a two-pass solve that computes the draws in a parallel wrangle. Until
  then the row ships with a 4.0x ceiling and this entry.
- **D261 — a performance headline names its SHAPE.** `pc_plan_solve` is 85 % of a 20 km single-curve
  cook and 6 % of a 300-street one, where `pc_order` is 25–30 %.
- **D262 — the OUTPUT's integer storage is a contract too, and the cast must be the LAST node.**
...
1. **N6, CONFORM — 97 % Python, 2 142 ms on a 20 km conformed run and 208 ms at 2 km.** The largest
   refused class, and its deciding experiment is committed and green (now with the raw double
   difference on the record as well as the f32 one, D247).
2. **N8, 4.3 CORNERS — 95–98 % Python**, 551 ms on a 20 km L. Still the hardest.
3. **D260 — the random draw.** A two-pass solve, or the row stays at 3x.
4. **D222 — `pc_plan_solve` parallelises per SECTION**, which is 85 % of a 20 km single-curve cook
   and 6 % of a citygen one (D261). Read those two numbers together before spending a cycle here.
5. **`pc_order` at 25–30 % of the citygen cook**, now guarded by `bench_deform_20km` but not made
   cheaper.
6. **PC-G5**, unchanged.

**Dev-loop rule 0 is NOT discharged for this cycle.** Nothing here has been audited by an agent that
did not write it. The honest words are **implemented, self-verified, unaudited**, and the specific
things an auditor should attack first are: the `int32` cast list in `native.py` (it is EXPLICIT, so
an attribute added to `ELEM_PRIM_ATTRS` without a line here ships at int64 again — and the check
that would catch it is `output_guard_parity`, which needs a case that carries it); the
`SOLVE_CEILING_PER_CALIB` ratio, which rests on two runs on one machine; and D263's refusal, which
is the first time level 1 refuses on a SPLINE ATTRIBUTE's VALUE rather than its storage or its
presence.



## 28. Cycle V1 — the double pillar at every corner (2026-08-24)

**Reported by Hannes, in the viewport, on the shipped defaults.** Gate PC-G1, miter mode, the
12 × 8 m rectangle, starter kit: at every corner an artist sees **two pillars of different
height** where they expect one. Measured on the shipped asset before the fix:

| what | X | Z | height | slot |
|---|---|---|---|---|
| corner post, two mated halves | 11.92 – 12.08 | −0.08 – 0.08 | 1.30 | `corner` |
| the run's FIRST piece | 11.94 – 12.06 | **0.08 – 0.19** | 1.20 | `default` |

The Z overlap is **exactly 0.0**. The two halves are correct — sliced on the bisector, mating
perfectly. `no_gaps_or_overlaps`, `corner_abut` and `corner_seam` all PASSED, and they were right
to: **the geometry IS closed.** Closure is not composition, and nothing in the suite asserted that
the result looks like a fence.

### 28.1 The layer diagnosis — it is (a) and (c), not (b)

**(a) THE SHIPPED STYLE. This is the proximate cause.** The parameter page shipped
`Repeating Pieces = "post panel"` with `Piece Order = In turn`, so the default run is
`post panel post panel …`. An alternating two-module unit placed in a run of even count **always**
ends with a panel and **always** opens with a post — so the leg *arriving* at a corner ends in a
panel (correct) and the leg *leaving* it opens with a post (a second pillar). The same defect fires
at a RUN END, where `Piece at the Start = "post"` caps a run whose first default piece is also a
post: measured 0.12 m, exactly one post, on `A_straight` and `DL_variant_kit`.

RailClone does not compose a fence this way. Its canonical fence makes **panels the Default and
posts Evenly-spaced** ([`railclone.md`](railclone.md) §1's slot list, §6.1's *"X Evenly = pillar
rhythm"*), precisely so that the rhythm element is anchored INSIDE the run and the run's ends
belong to whatever caps them — the Corner piece at a vertex, the Start/End piece at a spline end.

**(b) THE ENGINE — a real gap, but not this defect's cause.** The engine *cannot* express the
general case "phase a composed unit after a corner assembly", and that is worth writing down:

* `plan.choose` resolves a `sequence` as `cand[index % len(cand)]`, and `plan_section` starts
  every section's fill at `index = 0`. **There is no phase or offset control on a sequence**, on
  the page or in a payload.
* `plan.cond_subject`'s subject list has no term for *"the head of this run is already capped"*.
  `cornerAngle` is the closest and it is a different fact (the section's own turn, not what the
  neighbouring assembly supplied). A `conditional` rule picks between two modules; it cannot
  re-phase a period-2 pattern.

So a style that wants `post panel` AND one pillar at a corner is inexpressible today. **That is a
recorded gap, not a blocker** — the `evenly` slot is the general answer to "rhythm element", it is
the answer RailClone reaches for, and it is now the shipped default. Anyone who wants the phase
control anyway should add a `phase` field to `Rule` and a `section_capped` ctx subject; nothing in
this cycle depends on it.

**(c) THE FIXTURE SET. This is why it shipped.** `cases.fence_style` composes `post panel` and is
used by six straight/bend cases; `cases.corner_style` — which every one of the ~20 corner cases
uses — fills with **`panel` alone**. So *the one composition the parameter page actually shipped
was in no corner case in the suite*. 3 600 numeric checks were green on a fence with two pillars at
every corner because no fixture ever built one. This is the dev-loop's own question — *what can
this fixture NOT reach?* — answered the expensive way.

### 28.2 What changed

**D266 — the shipped default composition is RailClone's canonical fence.** On the parameter page
(`devScripts/create_pf_polychain_hda.py`):

| parm | was | is |
|---|---|---|
| `Repeating Pieces` (`slot_default`) | `post panel` | **`panel`** |
| `Evenly Spaced Piece` (`slot_evenly`) | *(empty)* | **`post`** |
| `Evenly Spacing (m)` (`evenly_spacing`) | `0.0` | **`2.0`** (the starter panel's own length) |

`Piece at the Start` / `at the End` / `at Corners` are unchanged. The page still reads as a
decision list — *"panels fill the run, posts every 2 m, a corner post at corners"* is a sentence an
art director says out loud — and both help strings now name the rule (`houdini-tool-design` §1:
the artist has to be able to steer it without reading the source).

Why the `evenly` slot rather than the fill: `plan.plan_section` places evenly anchors first and
fills the gaps between them, so the rhythm element is anchored INSIDE the run — which is the
property the composed fill could not have.

⚠️ **THE SENTENCE THAT USED TO BE HERE WAS FALSE, and it shipped inside the asset's own help
string.** It read: *"D15 keeps every anchor at least half a spacing clear of a capped end. So an
anchor can never land against a corner assembly or a start/end cap by construction."* D15's shed
keys on `start_cap`/`end_cap`, **D18 makes both false at a corner**, and what actually kept the
defaults clean was `justify = "center"`. `Evenly Justify = From the end` on a 12.161 m leg drove
the evenly post 0.061 m INTO the mitered corner post. **D269 makes the claim true** by guarding a
`trim` reservation the same way — with the one documented exception of `Adjust to End`, which
deliberately lands the last anchor on the end. See §28.7 F1.

Measured on the shipped asset, both corner modes, 12 × 8 m rectangle and the 10 × 6 m L:

* **miter** — corner: two mated `corner_post` halves and nothing else; its neighbours on both legs
  are panels. One pillar.
* **bend** — no corner assembly exists at all (D36 welds the ring), the panel wraps the vertex, and
  the evenly post rides it. One pillar, never two.
* **run ends** — `start`/`end` post, then a panel. The 0.12 m doubling is gone.

**D267 — `double_pillar_m`, the check that would have caught it** (`checks.single_pillar`). Two
ideas, and both are needed:

1. **UPRIGHT is geometry, not a name** — height ≥ 3 × the widest footprint side. `post` scores
   10.0, `corner_post` 8.1, `panel` 0.48, `gate` 0.66; the threshold is not tuned, it is a gulf.
2. **A RESERVED SLOT MAY NOT BE DOUBLED BY ANOTHER SLOT** — `corner`, `start`, `end`, `evenly` and
   `marker:<id>` each exist to put ONE piece at one place. A touching cluster of uprights drawn
   from more than one slot is the defect; the number is how much wider the cluster is than its own
   widest member, in metres.

Idea 2 is what keeps it honest on the runs that are *supposed* to be a row of pillars: 270 butted
posts on the hill cases are all slot `default` (one deliberate rhythm) and the two mitered halves
of a corner post are both slot `corner` (one assembly), while the shipped defect is `corner` +
`default` and measures **0.111698 m**.

**What it cannot see, stated because the checks it joins could not see the defect it was written
for:** a *separated* double pillar (two uprights 50 mm apart read as deliberate spacing; `TOUCH_M`
is 1 mm); a MISSING pillar, an empty corner, or a corner dressed in the wrong module — it never
asks what *should* be there; doubling WITHIN one slot (two corner assemblies squeezed onto a leg
shorter than both, `AP_narrow_rect` — that is `pc_warn_overflow`'s and `corner_face_mate_m`'s job);
duplication among non-upright members (two coincident panels are invisible); a kit whose default
piece is a narrow tall pier, which its aspect rule would classify as upright; and it compares
axis-aligned world bounding boxes, so a pillar standing on a bend reads slightly wide — hence a
2 mm tolerance rather than `TOL_M`. **It is a phase-1 check only** — it is not wired into
`run_2d_checks`, because a facade's mullions are uprights in different cells by design and the rule
would need a 2D reading before it means anything there.

**Three registered mutations, every one run through `run_mutation_registry.py` and reported
`[ok] RED` against a green control export.**

| id | runner | kills | measured |
|---|---|---|---|
| `default_slot_composed_again` | `hda` | `starter_fence_one_pillar_miter` | **0.111698 m**, 4 mixed-slot groups; reddens exactly 1 check name and nothing else |
| `evenly_doubles_the_fill` | `hda` | `starter_fence_one_pillar_bend` + `_miter` | **11.975 m** bend, **12.0 m** miter |
| `corner_style_composed_default` | `scene` | `double_pillar_m` | **19 cases red**, `V_rect_miter` at **0.111698 m** |

⚠️ **The second one exists because the `bend` row would otherwise be unfailable, and that is worth
saying out loud rather than leaving as a green.** `starter_fence_one_pillar_bend` cooks a CLOSED
rectangle: bend mode places no corner assembly (D36 welds the ring) and a closed run has no
start/end cap, so **no reserved slot exists for the fill to double** and `default_slot_composed_again`
leaves that row at 0.0 by construction. `evenly_doubles_the_fill` makes the FILL the rhythm element
as well as the `evenly` slot (`Repeating Pieces = post` with `Evenly Spaced Piece` still `post`) —
which is exactly what an artist does the first time they type `post` into Repeating Pieces without
clearing the evenly slot — and both rows go red.

The first is the exact source-level undo of D266 (with the asset rebuilt). The third composes
`cases.corner_style` the way the page used to — i.e. it applies the fixture gap of §28.1(c) — and
reddens `AA_reflex_miter`, `AB_fillet`, `AC_degenerate_corner`, `AH_sharp_turn`, `AI_triangle`,
`AJ_reflex_closed`, `AK_pentagon`, `AL_crest_corner`, `AM_graded_corner`, `AP_narrow_rect`,
`AR_offset_past`, `BI_conform_corner`, `CD_replace_bent`, `CJ_bend_butt_120`, `T_lshape_bend`,
`U_lshape_miter`, `V_rect_miter`, `W_corner_offset_pos`, `X_corner_offset_neg`.

⚠️ `default_slot_composed_again` reddens **only the `miter` row**, and that is correct rather than a
weak pairing: `bend` places no corner assembly, so there is no reserved pillar to double and
`starter_fence_one_pillar_bend` stays 0.0 on the mutated build (measured). It reddens **nothing**
in `run_scene_checks`, which is §28.1(c) restated as a number.

**D268 — a cost fixture states its composition instead of inheriting the parm defaults.** Three
cost tables read the shipped asset *at defaults*, so D266 moved them without a line of kernel code
changing:

| row | before | after D266, unpinned | cause |
|---|---|---|---|
| `output_guard_cost` straight_2km | 1.33x | **1.81x** (ceiling 1.60) | D259's per-anchor cost, now on the default path |
| `wrangle_cost_is_flat_in_piece_count` `pc_plan_solve` | 66.5 µs/piece ceiling | **87.2 µs/piece** | same |
| `bench_deform_20km` `pc_order` | 7.38 µs/piece | **14.10 µs/piece** (ceiling 12.0) | the fill lost `post`, so the 20 km ripple built **10 029** pieces instead of 20 057 while `pc_order`'s cook barely moved (147.9 → 141.4 ms) — it sorts PRIMS and the ceiling divides by PIECES |

A/B on ONE asset with only the two slot parms swapped, 2 km straight, min of 3 dirtied cooks:

```
NEW panel + evenly post      pieces=2001  output=0.1602 s  reference=0.0883 s  ratio=1.81x
OLD post panel, no evenly    pieces=1888  output=0.1105 s  reference=0.0832 s  ratio=1.33x
```

Neither ceiling was raised. A row's shape has to be the shape its ceiling was measured on — the
same rule that keeps `WRANGLE_CEILING_US` and `WRANGLE_DEFORM_CEILING_US` apart — so
`_wrangle_rig` now pins `slot_default = "post panel"` / `slot_evenly = ""`, and every
`output_guard_cost` row pins `slot_evenly = ""` before applying its own parms. **`evenly_2km`
sets it back, and `evenly_2km` IS the shipped default composition, so the default path stays
bounded — at the 2.6x ceiling D259 already justified.** It reads 1.83x.

⚠️ **AND THAT IS TRUE BUT TOO GENEROUS, in two ways (§28.7 F12/F13).** No ceiling constant was
raised — but the default composition MOVED from the row governed by 1.6x to the row governed by
2.6x, so the guard on the default path is **62 % looser** than it was. And `evenly_2km` is a
dead-flat straight line, so it bounds the default's STRAIGHT path only: the shipped default's
**deformed** path is bounded by nothing, because `_wrangle_rig` pins the old composition.

**The honest price of the fix, recorded rather than hidden:** the default cook is **+45 %** on a
2 km / 2 000-piece fence (0.1105 → 0.1602 s), and the shipped output path is 1.81x its own Python
oracle there where it was 1.33x. That is D259's per-anchor fixed cost and D260's named remedy
arriving on the default path; it is 50 ms on a 2 km fence and it buys a fence that reads correctly,
which is §6's own acceptance criterion. **It is a debt with an owner, not a surprise.**

### 28.3 The two pinned fixtures, and why they are not a fix

`DOUBLE_PILLAR` in `run_scene_checks.py` pins **`A_straight` and `DL_variant_kit` at 0.12 m** —
the run-END half of this very defect, live in the committed suite. `cases.fence_style` is the
suite's **only** coverage of the `sequence` selector, and rewriting it deletes that fixture; and a
live instance of the defect inside the suite is the standing proof this check can see one — the
value is exactly the post width, so a check that quietly stopped measuring would drop to 0.0 and go
red there. The shipped defaults no longer do this; `run_hda_checks` and `gate_images` drive the
parm face and assert 0.0. (⚠️ The `gate_images` half of that sentence was **false when it was
written** — that file ran nine checks over PC-G1 and `single_pillar` was not one of them. It is
true now; §28.7 F5.) A third entry, `ED_rect_evenly_adjust`, joined the table at D269 — see §28.7.

### 28.4 Suites and baseline movement

| suite | result |
|---|---|
| `tests/unit/test_polychain*.py`, `test_plan.py` | OK |
| `run_scene_checks.py` | **0 failing, 0 moved** |
| `run_2d_checks.py` | **0 failing, 0 moved** |
| `run_native_checks.py` | **0 failing** |
| `run_hda_checks.py` | **0 failing** (2 new rows) |
| `tests/hda/run_attrib_checks.py` | **0 failing, 0 unreviewed baseline change** |
| `scale_gate.py` | 0 failing rows |
| `gate_images.py` | **0 failing gate checks**, 19 images |

`baseline.json`'s diff is **purely additive**: one `double_pillar_m` row per case, plus one changed
*detail* text on `curve_sample_scaling` (a microsecond timing string; its value did not move). No
baselined VALUE moved anywhere in the tree — the parm-face change reaches no scene case, because
every scene case builds its style in Python. That is the fixture gap of §28.1(c), visible one last
time as an absence.

⚠️ **Two timing checks failed once each and passed on re-run, on a machine with another agent's
`hython` and Hannes' `houdinifx` both live**: `solve_cost_is_flat_in_piece_count` at 0.551 against
a 0.55 calibrated ceiling (0.2 % over — the check's own docstring records why that constant became
a calibrator), and the two D268 rows *before* they were pinned. The D268 rows had a real cause and
were fixed at the fixture; the solve row is contention and is recorded as such rather than
re-ceilinged.

### 28.5 Decisions

- **D266 — the shipped default composition is RailClone's canonical fence: panels fill the run,
  posts are Evenly-spaced at 2 m.** A composed fill cannot avoid doubling the piece at one end of
  every run, because an alternating unit of even period opens and closes with different members.
  The rhythm element belongs in a slot whose anchors are clear of the run's ends by construction.
- **D267 — composition is asserted as "a touching group of uprights drawn from more than one slot
  is a defect", and its size is how much wider that group is than its own widest member.** Names
  are not a vocabulary a check may depend on; geometry and slot are.
- **D268 — a cost fixture states the composition its ceiling was measured on.** A fixture that
  reads the parm defaults silently re-measures itself every time an artist-facing default changes,
  and the only two outcomes are a false failure or a loosened ceiling.
- **FINDING (no D, because nothing was built for it) — the engine cannot phase a composed unit
  against what a neighbouring assembly supplied.** No phase on `sequence`, no ctx subject for
  "this run is already capped". §28.1(b) has the shape of the fix if it is ever wanted.

### 28.6 What this cycle did NOT do

- ⚠️ **Dev-loop rule 0 was NOT discharged when this was written, and §28.7 is what discharged it.**
  The three things this bullet told an auditor to attack were the right three: the aspect-ratio
  classifier on a kit that is not the starter fence was **two separate defects** (F2's false
  negative on a blocky corner, F3's false positive on a dense picket spacing), the pinned
  `DOUBLE_PILLAR` entries survived the metric rewrite unchanged and were **not** over-reach, and
  `evenly_spacing = 2.0` remains open (§28.8 item 4). Fifteen findings in total; §28.7 has them.
- `run_2d_checks` does not run `double_pillar_m` (see §28.2).
- D259/D260 are not paid down; D266 merely moved their cost onto the default path.

### 28.7 The audit of §28, worked through (2026-08-24)

**Dev-loop rule 0, discharged for §28.** Fifteen findings from an agent that did not write it,
every one reproduced from a pristine export before it was touched. Three of them said the same
thing in three different places: *the check written to see a double pillar could not see one on
any kit but the starter fence.*

**F1 — the stated mechanism of D266 was FALSE, and the false sentence SHIPPED.** §28.2 claimed
D15's half-module shed keeps an evenly anchor clear of a corner "by construction", and
`slot_evenly`'s own help string on the built asset said "a corner or cap piece is never doubled by
one of these". Neither was true. `plan.plan_section` builds `head`/`tail` from
`section.start_cap`/`end_cap`, and **D18 makes both FALSE at a corner** — a corner reserves its
space through `trim`, not through a cap — so the shed never ran there at all. What actually kept
the shipped defaults clean was `justify = "center"`, whose lead is `(L-(n-1)d)/2` and therefore at
least half a spacing: an accident of one default, not a guarantee. **One Advanced parm reached the
defect this cycle exists to fix.** Reproduced on the shipped asset, miter, starter kit, nothing but
a curve wired:

| parm | leg | `double_pillar_m` before | after D269 |
|---|---|---|---|
| `Evenly Justify = From the end` | 12.161 m | **0.061** | **0.0** |
| " | 12.17 / 12.18 / 12.20 / 12.25 m | 0.07 / 0.08 / 0.10 / 0.15 | **0.0** |
| `Evenly Justify = From the start` | 12.161 m | 0.061, at the trailing end | **0.0** |
| `Adjust to End = 1.0` | 4.66 / 6.66 / 8.66 / 12.66 / 12.70 / 12.90 m | **0.06 at every corner** | 0.12, abutting |

**D269 sheds the clearance at any RESERVED boundary, not only at a capped one** — `trim[0] > 0` /
`trim[1] > 0` earn the same half-module + pad set-back that a cap gets. Two lines. The 0.061 m the
justify cases lost was **interpenetration**: the evenly post stood half inside the corner post.

⚠️ **`Adjust to End` is NOT taken to zero, and that is a decision rather than a residue.** The parm
means *land the last anchor exactly on the section end*, and at a corner that end carries the
corner post. After D269 the interpenetration is gone and what remains is an exact abutment — one
whole 0.12 m post, zero overlap. Engineering that away would be overriding an instruction the
artist typed. So it is **pinned** (`ED_rect_evenly_adjust` in the scene suite,
`evenly_clears_the_corner_adjust_to_end` in `run_hda_checks`) and both parm help strings now say
what is true instead of what was hoped: `slot_evenly` promises *half a module clear … except where
Adjust to End is on*, and `adjust_to_end` warns that the last piece will stand against a corner or
a cap.

⚠️ **THE LEG LENGTH IS HALF THE FIXTURE.** On the 12 × 8 m rectangle every justification measures
0.0 **both before and after** D269 — 12 m is an exact multiple of the 2 m spacing, so the justify
leftover never approaches zero and the defect is unreachable there. A fixture on the round number
is green on the broken build. `RECT_ODD` is 12.161 × 8.161 m for that reason, and the registered
mutation `evenly_ignores_the_corner_reserve` records it as its own second half.

**F2 / F3 / F9 / F8 — `single_pillar` was four different wrong checks, and D270 rewrote it.** Each
was measured, not argued.

| what was wrong | measured on the old check | after D270 |
|---|---|---|
| **F2, false NEGATIVE.** `_is_upright` gated *entry*, so a corner module that is not slender was never protected at all. The starter kit's own `corner_block` (1.20 × 1.30 × 0.16 m, aspect 1.08) is such a piece — and so is every plinth, pier cap and wall return, i.e. most real kits. | same build, only the corner module swapped: `corner_post` **0.111698, ok=False**; `corner_block` **0.0, ok=True** (its 8 pieces not counted at all) | `corner_block` **0.110491, ok=False**. A RESERVED piece is protected whatever its aspect; the aspect rule now only decides whether the *neighbour* is a doubler |
| **F3, false POSITIVE, non-monotonic.** The aspect was read off the PLACED world box, so the fitting solve decided the classification: adaptive fill shrinks the 2 m `panel` below 0.30 m long and its 0.90 m height then clears 3:1. | `evenly_spacing` sweep, shipped defaults otherwise, miter 12 × 8: 0.30 → **11.98**, 0.35 → 11.90, 0.40 → 11.88, 0.42 → 0.28, 0.45 → 0.28, 0.50 → 0.0, 0.60 → **0.28**, 0.80 → 0.0, 1.00 → 0.0 | **0.0 at every one of the nine.** Aspect is read off `pc_local`, the module's own space, so `fill`, `padding` and `evenly_spacing` cannot reclassify anything |
| **F9, the metric degenerated.** A transitive touching CLUSTER's span minus its widest member IS the run length once the fill is uprights and the ends are capped. | `Repeating Pieces = post`: **1999.880005 m** on a 2 km run, 11.975 / 12.0 m on PC-G1 — the registered mutation's own headline numbers were run lengths | **0.12 m**, one post, on both. The value is the worst single protected-vs-upright PAIR, per horizontal axis, bounded by one module's footprint by construction |
| **F8, cost.** The union-find swept every pair. | 3 333 uprights **0.686 s**; 16 667 uprights **33.938 s** | footprints bucketed on a grid of side `max_footprint + TOUCH_M`: 16 667 uprights in **0.108 s**, 314× |

The three pinned `DOUBLE_PILLAR` values are **unchanged** by the rewrite — `A_straight` and
...
defaults**. `grep -c evenly tests/polychain/cases.py` returned **0**. The ~35 corner and closure
checks were once again running a composition the asset does not ship, and the only assertion on
the new defaults was two rows of `run_hda_checks` on one rectangle: the same one-fixture shape
that let the original defect ship, one cycle later. **Nine cases close it** (EA–EI): the shipped
composition through the whole corner battery on a rectangle, an L, a reflex zigzag and a pentagon;
all three justifications and `adjust_to_end` on `RECT_ODD`; **EH**, the blocky corner module F2
could not see; and **EI**, a marker slot on a cornered curve, which no marker case had ever reached
(T2/T3/D/N all run straight curves with no corner rule).

**F5 — a comment that made one fixture look like two.** `run_scene_checks`'s `DOUBLE_PILLAR` block
said "`run_hda_checks.py` and `gate_images.py` drive the parm face and assert 0.0 there".
`gate_images` ran nine checks over PC-G1 and `single_pillar` was not one of them — all nine were
closure checks, which is precisely the family that cannot see this defect. It is there now, so the
parm face is asserted over the L and the **3 m close-up** as well as the rectangle.

**F6 / F10 / F11 — three blind spots the docstring got wrong or did not state.** `_touching`'s Y
clause was credited with stopping a finial stacked on a post reading as a second pillar; it is an
overlap test with 1 mm slack, so an exactly-stacked finial passes it — what actually makes a
concentric stack score zero is the `span − own` form of the metric (measured: 0.0). The rule flags
**any** upright from another slot touching a reserved piece, **including the lamp-on-a-bollard
composition `slot_evenly`'s own help advertises** (measured 0.08 m, red — `expected` is the escape
hatch). And the 2 mm `tol` is a dead zone in both directions: 0.0015 and 0.0019 pass, 0.0021 fails.
All three are in the check's "what it cannot see" block now.

**F7 — the mutation registry's control build could invalidate a sweep silently.** It reported
`hda 33 checks, 1 red` on a pristine HEAD export, aborted the whole sweep, **did not name the
row**, then ran green on re-run and green again when the export was reproduced by hand. The
control now prints its failing check names and re-runs once before declaring itself broken.

**F12 / F13 / F14 — three claims in this section that were too generous, corrected in place.**

- The `_wrangle_rig` pin's comment said the shipped default's cost is bounded by `evenly_2km` in
  `output_guard_cost`. `evenly_2km` is `guard_polyline_geo([(1.0*i, 0, 0) …])`, a dead-flat line
  that never enters the deform branch — so it bounds the shipped default's **straight** path and
  the shipped default's **deformed** path has no committed ceiling. Standing finding §28.8(3),
  not closed with a ceiling calibrated on a contended machine.
- No `*_CEILING_*` constant was raised (verified by diff), but the default composition **moved from
  the row governed by 1.6x to the row governed by 2.6x**. §28.2's "the default path stays bounded —
  at the 2.6x ceiling D259 already justified" reads cost-neutral and is not: the guard on the
  default path is **62 % looser** than it was, and paying down D259's per-anchor cost now buys back
  a *default-path* ceiling rather than an opt-in one.
- D266 is a **defaults change**, so `pc_elem_id` addresses on the default asset move (bend 38 → 39
  pieces, miter → 44). 3.4's "ids are structural and must survive" governs refactors, not
  deliberate composition changes — but 4.6's override cascade keys on `pc_elem_id`, so **an
...
1. **The engine cannot phase a composed unit against what a neighbouring assembly supplied.**
   Unchanged from §28.5, restated here because it is the answer to "why is the fix in the style and
   not in the engine". `plan.choose` resolves a `sequence` as `cand[index % len(cand)]` and
   `plan_section` starts every section's fill at `index = 0`; there is no phase field on `Rule`,
   and `plan.cond_subject` has no subject for *"the head of this run is already capped"*
   (`cornerAngle` is a different fact — the section's own turn, not what the neighbour supplied).
   So a style that wants `post panel` **and** one pillar at a corner is inexpressible today.
   The shape of the fix if it is ever wanted: a `phase` field on `Rule` and a `section_capped`
   ctx subject. Nothing in §28 depends on it — the `evenly` slot is the general answer to "rhythm
   element" and it is the one RailClone reaches for.
2. **A marker landing inside a corner assembly's reserve shares ground with it.** `trim` belongs to
   the corner alone; nothing reserves space for a `marker:<id>` anchor against it. Measured on the
   L with the marker 0.4 m short of the vertex: `pc_warn_overflow`, `corner_abut_m` **0.040**,
   `corner_face_mate_m` **0.113**, `double_pillar_m` **0.0767** (`marker:7:gate` +
   `corner:corner_post`). The engine warns rather than blocks, which is D13's policy working — but
   the seam does open. `EI_marker_at_corner` runs the marker mid-leg deliberately and records the
   collision numbers in a comment rather than pinning them, because closing it that way would mean
   giving `corner_abut` an `expected` for a fixture's convenience.
3. **The shipped default's DEFORMED path has no cost ceiling.** F12 above.
4. **`evenly_spacing = 2.0` is the starter panel's own length, not a general answer.** A kit whose
   panel is not 2 m gets a rhythm that does not land on its joints. Carried over from §28.6.

### 28.9 Decisions

- **D269 — a RESERVED boundary is guarded, not only a CAPPED one.** D15's half-module shed keyed on
  `start_cap`/`end_cap`, and D18 makes both false at a corner, so the guarantee the parm help
  advertised did not exist: `justify = "center"` was providing it by accident. Anything that
  reserves span — a cap through `head`/`tail`, a corner assembly through `trim` — earns the same
  set-back. `adjust_to_end` still lands on the guarded end, because that is what the parm means,
  and the help now says so.
- **D270 — a check may not let one geometric rule decide both what it protects and what counts as
  a doubler, and it may not read a quantity the pipeline downstream of it can change.** Protection
  follows the SLOT (structural, unchangeable by a fit); doubling follows the ASPECT of the module's
  OWN geometry (`pc_local`, before placement). Reading the placed box let `evenly_spacing = 0.35`
...
- **D271 — a fixture's own dimensions are part of the fixture.** Every justification measures 0.0
  ...
  fail. A fixture for a leftover-driven defect must be built on a length that leaves a leftover.

### 28.10 Verification of §28.7 — suites, mutations, images

**Every suite run from a `git archive HEAD` export of the audited tree, not from the working copy.**

| suite | result |
|---|---|
| `tests/unit/*` (5 files) | **287 tests, OK** |
| `run_scene_checks.py` | **0 failing, 0 moved** |
| `run_2d_checks.py` | **0 failing, 0 moved** |
| `run_native_checks.py` | **0 failing** |
| `run_hda_checks.py` | **0 failing** (37 rows: 4 new `evenly_clears_the_corner_*`, 2 D267 rows kept) |
| `tests/hda/run_attrib_checks.py` | **0 failing, 0 unreviewed baseline change** |
| `scale_gate.py` | **0 failing rows** |
| `gate_images.py` | **0 failing gate checks**, `double_pillar_m` 0.0 on all six PC-G1 builds |

**Baseline movement, re-derived key by key against the pre-audit tree** (a name-keyed diff, not a
line diff — the file re-serialises):

| | |
|---|---|
| cases | 91 → 100. **9 added** (EA–EI), **0 removed** |
| rows | 5 710 → 6 277. **0 rows added to an existing case, 0 removed** |
| **values moved** | **6**, all `double_pillar_m`, all `None → 0.0`: `AD_short_legs`, `AQ_asym_squeeze`, `CH_swap_tile_slice`, `CI_swap_zmode`, `N_marker_mixed`, `O_no_kit`. **Cause:** the old check needed two UPRIGHTS before it would measure anything and these six cases have fewer, so it reported SKIP; D270's candidate set is *reserved OR upright*, so all six are now measured. Six cases that could not be measured now are — the movement is coverage arriving, and the value it arrives at is the clean one |
| details moved | 91 — 90 `double_pillar_m` (the detail line is worded `N candidates, M doubled pairs` where it was `N upright members, M mixed-slot groups`) and one microsecond timing string on `curve_sample_scaling` whose value did not move |
| **A_straight / DL_variant_kit `double_pillar_m`** | **0.12, unchanged.** The metric was rewritten and the two calibration values did not move |

**Mutations, all against a pristine export of the audited tree.**

| runner | result |
|---|---|
| `scene` | **9 of 9 RED, 0 survivors** — including `corner_style_composed_default`, whose anchor had MOVED when `corner_style` grew its `evenly` argument. The registry hard-failed on the stale edit instead of reporting a silent green, which is how that was caught |
| `hda` | **5 of 5 RED, 0 survivors.** `default_slot_composed_again` — the source-level restore of the ORIGINAL double pillar — reddens 4 checks. `evenly_ignores_the_corner_reserve` (D269, new) reddens 3. `kit_input_unplugged` reddens 1 after D272, having reddened **nothing at all** before it |
| coverage | `hda` 37 checks: **15 proven, 0 unproven, 22 undeclared** — the four new rows are all proven, and the undeclared count went DOWN (23 → 22) |

⚠️ **AND THE CONTROL GATE PROVED F7's POINT ON ITS FIRST RUN, then proved the fix is not enough.**
The full sweep's control reported `native retrying the control: 1 red ['solve_cost_is_flat_in_piece_count']`
— named, which it could not do before — and the retry **went red on the same row again**, so the
sweep aborted honestly rather than silently. `solve_cost_is_flat_in_piece_count` is the row §28.4
already records as a contention artefact (0.551 against a 0.55 calibrated ceiling); three `hython`
processes were live on the machine. The per-runner sweeps above were run with `--runner`, which
narrows the control to the runners a sweep actually needs, and the same suite ran **0 failing** on
its own minutes later. **Naming the row is what made that diagnosable in one line instead of a
re-derivation.**

**The images, regenerated and LOOKED AT** (`tests/polychain/gate_images.py`, PC-G1, shipped
defaults, nothing but a curve wired):

* **`VG1_rect_miter_top.png` — the exact picture Hannes reported.** At each of the four corners:
  **ONE** square footprint with the bisector diagonal drawn across it — the two mated `corner_post`
  halves reading as a single pillar. **No second square touches any corner.** The nearest posts are
  the evenly rhythm, one clear span down each leg.
* **`VG1_rect_bend_top.png`** — no corner assembly exists (D36 welds the ring), the panel wraps each
  vertex, and the only squares on the ring are the 19 evenly posts. None of them is at a corner.
* **`VG1_closeup_miter_top.png` — the 3 m close-up, where a corner is not four pixels.** One corner
  square with its diagonal, panels running away from it on both legs, an evenly post mid-leg and a
  cap post at each open end. One pillar.
* **`VG1_closeup_bend_top.png`** — the panel turns the corner with nothing standing on it.
* `g1_*_parm_face` confirms each image is the parm face's own output (72 / 76 / 17 / 26 / 40 / 18
  elements against `place.build` on `style_from_parms`), and `drawn_covers_packed` confirms each
  raster contains its subject — D194's rule, which is why these pictures may be judged at all.

**Verdict: the corner reads as ONE pillar in both corner modes, on the rectangle and on the
close-up, with the shipped defaults and nothing but a curve wired.**
