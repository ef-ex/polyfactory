...
| Last completed | **§11 IS FINISHED, AND §11.8 P5 HAS BEEN REVIEWED AND CORRECTED — read §11.8 P5c before trusting any P5 number.** §11.8 P5 (the `ray` verb, batched) LANDED at `6c51bb2`, P5b at `17cbadd`, P6 **DECLINED** (D109); then two independent reviews returned **ten findings, ten reproduced**, and P5c is the cycle that closed them. **What changed:** (i) `drop_many` reads the verb’s **DISTANCE**, not its position (D111) — P5’s “bit-identical” parity was a property of the FIXTURES (`y = 0.25x` at 0.25 m stations is float32-exact); on an irrational-slope ramp the position route is **2.4e-07 m** and at 20 km **6.1e-05 m**, the distance route is **0.0** at both; (ii) a **tilted `conform_axis` is not batched at all** (1.9e-06 m at 20 m, 1.5e-05 m at 20 km — the divergence is ALONG the ray and no reconstruction removes it), with `BJ_tilted_axis` the case that builds in it; (iii) the batch is taken **ONCE PER BUILD, not per curve** (D112) — `ray`’s fixed cost scales with the SURFACE, so per-curve batching was **0.94–0.99x, slower than not batching**, on four of five conformed rows; after the hoist every row is **1.15–1.39x** and, against the P5-era commit, `streets_300c_smooth` **1.78 → 1.20 s (1.47×)**, `streets_300c_big` **2.23 → 1.50 s (1.48×)**, `fence_2km` **0.145 → 0.109 s (1.32×)** with **−188 MB of peak working set**; (iv) the gap midpoints — **47 % of every batch, 0–9 % consumed** — are no longer enumerated; (v) the hit test no longer builds one `hou.Point` wrapper per query. **Six mutations, six RED.** Baseline **89 → 90 cases, 5 488 → 5 555 values, 67 added, 0 removed, TWO moved** (both the redefined tripwires), `geometry_digest` **unmoved on 0 of 88**. New standing checks: `ray_executions_per_build`, `points_wrappers_built`(+`_streets`), `conform_prefetch_hit_rate_streets`, `conform_cache_per_element_streets`, `ray_verb_semantics`’ `dirty_ramp` / `dirty_ramp_20km` / tilted-gate trials, and the committed bench `tests/polychain/conform_bench.py`. |
