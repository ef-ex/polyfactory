...
**Status:** groundwork built 2026-09-17 on branch `pf-modeler`; lofted sheets, trunks and branch attachment added the same day (§2.4–2.6, all audited). Twelve checks, twenty-two mutations,
...
6. **Branches join the surface** (cage + `resolve` + bridge). A sphere that lies on a sheet
   ...
   nearest surface cell whose Houdini normal faces the branch (dot > 0.2) **and is local** —
   no farther than twice the nearest cell of any facing, or a branch aimed into a trunk takes
   the far wall and drives a limb through the mesh (audit round 4); a `resolve` pass
...
| c12 awkward trunk branches warn and stay closed | a branch aimed into the trunk is dropped, warns "no cell", 44 faces closed; two branches from the same station on the two lines of one wall, both heading out, want one cell: one keeps it, warning, 2 closed pieces; Loft Spans 1 on three lines (odd ring) rounds up, caps close, 18 faces | locality bound removed; resolve bypassed; odd ring allowed |
...
**Round 4 (2026-09-17, same agent, trunks and branch attachment).** Verdict "sound" on 40
cases — 3/5/8 lines, mixed point counts, any lines reversed, taper to a point, lean, a 90°
bend, Loft Spans 1 and 32, branches at the first and last station, up the axis (takes a cap
quad, still closed), adjacent stations, huge next sphere, branch of a branch, off a sheet's
top, bottom, wall and end wall — with one defect and three silences, all fixed: a **branch
aimed into the trunk took the far wall** and ran a limb through the mesh with no warning,
because the cell search had no locality (now bounded to twice the nearest cell, and the
"no cell" warning fires; c12); a **connection between two surface points** was dropped in
silence (warning now); a curve carrying **both `pf_trunk` and `pf_sheet`** lofted twice in
silence (warning now); **trunk lines given out of order** around the ring lofted a
self-intersecting star in silence (the ring is tested for crossing at every station, warning
now). Four c11 mutations had stayed green — first facing cell instead of nearest, resolve
bypassed, facing threshold, odd-ring rounding — c11 now asserts the limb leaves from the
branch point's own cell and c12 covers the clash and the odd ring. Stated, not fixed: a trunk
tapered to a point has a zero-area cap of coincident points.

**Status after round 4: trunks and attachment audited; the round-4 fixes are check-covered
but not themselves re-audited.**
