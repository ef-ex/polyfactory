...
| 35 | **A PRODUCTION change silently unmoored two of ANOTHER GATE's mutations.** Adding one term to `pf_collapse.vfl` made both of G1's registry rows unmatchable, because their anchors were the *whole* warning expression rather than the terms they were about. ⭐ **The anchor assert caught it loudly** (`MUTATION DID NOT APPLY`, 2 clauses unproven) — the alternative is a `.replace` that matches nothing, survives, and "proves" a check while editing no code. **Second occurrence in this file**; the general rule is *an anchor that spans more of a statement than the mutation needs is a tripwire on unrelated edits*. ⚠️ **And it is a cross-suite coupling: only running BOTH sweeps found it** | citygen buildings, G2 fix pass |
...
**⛔⛔ AND THE ONE THAT MATTERS MOST IN THIS FILE: INSTANCE 21 CAME BACK — FOURTH TIME, NEW STAGE
(B0 + B1 round-1 audit, 2026-08-27).** These are logged apart from 39–42 because those were defects
in a cycle's own *mutations*; these are defects in the *build*, found by an independent reader.

| # | Incident | Ref |
|---|---|---|
| 43 | ⛔⛔ **A BUILDING MASSED OUTSIDE ITS LOT, UNWARNED — INSTANCE 21, RE-OPENED IN B1.** `shapeL`/`shapeU` cut their notch out of the **axis-aligned** plan bbox, so on a 30 × 24 lot ROTATED about its own centre three footprint corners landed outside it at 15° (worst **4.084 m**), four at 30° (**9.392 m**), four at 45° (**11.465 m**); a trapezoid lot, two corners out at **3.953 m** — **all four `pf_warn_*` at 0 every time.** ⭐ **The guard existed and was called on no shaped site**: `pf_collapse.vfl` measures containment against `_p0`, which `pf_inset.vfl` writes **after** `pf_shape.vfl` has already discarded the lot, so it compared the footprint with the box-L; `C.masses_inside_lots` run by hand on the 30° build FAILS at **−9.39 m** and passes at 0°. ⚠️ **The recorded DEFENCE was also wrong** — *"that is CityEngine's behaviour too"*; CityEngine's scope is the **oriented** box and ours was the axis-aligned one, so on a rotated parcel its notch stays inside and ours did not. **Fixed** with both halves: the scope is now the lot's own minimum-area oriented box (exact for a rectangle at any angle), and a containment guard **inside `pf_shape.vfl`** refuses any notch that leaves the lot — measured there because it is the last node that still HAS the lot, and because `pf_mass.vfl`'s degraded fallback rebuilds on `_p0`, i.e. on the escaped footprint. Standing coverage: `shape_ops/inside_the_lot` plus a rotated and a trapezoid fixture | citygen buildings, B0+B1 round 1 `A1` |
| 44 | **A FIX LINE THAT WAS A NO-OP FOR A WHOLE BUILD, AND EVERY FIXTURE IN BOTH SUITES WAS BLIND TO IT.** `pf_shape.vfl` carried `if (sgn < 0.0) reverse(ring);` — and **`reverse()` is a pure VEX function, so the statement form does nothing**: `int a[] = array(1,2,3,4); reverse(a);` leaves `a[0] == 1`, while `b = reverse(b)` gives `b[0] == 4`. On a **clockwise** lot the ring therefore kept the box's positive winding while the role loop multiplied its normals by the input's `sgn` of −1, so **every `pf_face_role` — and therefore every setback — landed on the OPPOSITE edge**, producing a perfectly legal, perfectly wrong L. ⚠️ **Every fixture in both suites was CCW**, which is dev-loop rule "ask of every fixture what it cannot reach" costing a §12.4 contract break. **Fixed** (`ring = reverse(ring)`) with a clockwise fixture and its own mutation. ⚠️ **Whether S8 emits a clockwise lot is still NOT measured** — the code treats one as legal throughout | citygen buildings, `A2` |
| 45 | **`corner_closure_b1` COUNTED THE BUILD AGAINST ITSELF — shape 1 again, inside the clause advertised as B1's headline evidence.** With `pf_shape.vfl` neutered so B1 did **nothing**, the footprint stayed a plain four-corner rectangle and all three clauses PASSED: `[5112, 0, 0.000, 0, 0]`. Every clause in `corner_closure` walks the ring of the mass it was **handed**, so *"a corner module at every corner including the reflex one `shapeL` manufactured"* was carried by nothing at all. **Fixed** with a `footprint_asked_for` clause comparing the cap ring against the plan the fixture derives by hand from the notch numbers — re-measured after the fix: the other three clauses **still pass** under the neuter and only the new one fails. ⚠️ **Third check in this build to take shape 1 after it was named**, and the second inside a headline clause | citygen buildings, round 1 `C` |
| 46 | **THREE UNFAILABLE TERMS INSIDE ONE CLAUSE — the granularity below the one the sweep enforces.** `site_contract/published` had a mutation and reddened, so the per-CLAUSE sweep was satisfied; three *terms* inside it could still be deleted with the suite green — the `_*` **group** sweep and the **point-class** `_*` sweep (B0 makes no groups and writes no point scratch, so nothing could reach either) and any leak named neither `pf_*` nor `_*` (a bare `bogus` shipped green). **Fixed** by making the fixture's lot arrive carrying a `_scratch` group and a `_junk` point attribute, by widening the completeness rule past those two prefixes, and by three registry rows. ⚠️ **The rule this adds: a clause with a mutation is not a proved clause if it is an OR of terms — each term needs an input that reaches it** | citygen buildings, round 1 `D` |
| 47 | **THE FIXTURE WROTE GENERATION ORDER INTO ITS ORACLE AS THE RIGHT ANSWER.** `pf_site_id` fell back to the lot's own **primitive number** when nothing upstream supplied one, and the B0 fixture's `BARE_WANT` spelled that out as `{0: …, 1: …}` — so §12.7's own prohibition was asserted as correct. Measured: the same two unidentified lots cooked in the opposite order swap ids, and `elem_ids_structural` **cannot see it** because it compares the id **SET**, which is identical. ⛔ **And it was the NORMAL path, not an edge case:** streets publishes `block_id` / `lot_id` and nothing in the repo writes `pf_site_id`, so §12.7 was broken for every building built from a real S8 lot. **Fixed**: the minted id is now an order-independent hash of the lot's own plan position, the want is keyed `"*"`, and `site_ids_structural` compares the id → lot **mapping** across a reordered cook | citygen buildings, round 1 `B1-d` |

**The rule that falls out of 43 and 45 together, and it is not new — it is instance 21's own rule
arriving for the fourth time:** *a stage that REPLACES its input must be the thing that checks the
replacement against it, because downstream nobody has the input any more.* Both defects are the same
sentence read from two ends: `pf_collapse` could not see the escape because `pf_shape` had thrown the
lot away, and `corner_closure_b1` could not see the missing L because `pf_mass` had thrown the
...
| # | Incident | Ref |
|---|---|---|
| 48 | ⛔ **A GEOMETRIC FRAME DECIDED BY FLOAT32 NOISE, and the doc describes a tolerance the code never had.** The oriented-scope tie-break ships an **absolute `1e-6`** band; the measured float32 spread across geometrically identical candidate directions on a 720 m² lot is **6.1e-05 – 9.77e-04** — *three orders larger*, so **the tie-break is never consulted and rounding error picks the frame.** Swept 0–90° in 0.5° steps: **68 of 181 orientations cut the notch at the wrong corner** — a legal six-corner L, **entirely inside the lot** (+2.0 m clearance), **all four `pf_warn_*` at 0**, and every `pf_face_role` and setback landing on a different edge. `shapeU` identical at 69/181. Deterministic per build, not a fixture artefact. **Fixed** with three changes, because the obvious one is measured worse: the candidate areas are taken from the lot's own first point, the band is **relative** (`float SCOPE_REL = 1e-3;`, `float band = bestar * SCOPE_REL;`), and the tie-break is the **lot's** — longest edge, then first in the lot's own ring order, with the fold into the +x half-plane deleted. Swept after: **0 of 181** wrong at four world positions and **0 of 721** over the full circle, against 68/181 and 493/721 | B1 `P1` |
| 49 | ⭐ **§2a instance 13 INVERTED — the record claimed a tolerance the file does not contain.** §12.10d *and* commit `7b08c05` both state the band is *"relative (1e-4 of the area)"*. **It is absolute `1e-6`; the relative band is not in the file and never was.** Instance 13 was a check advertising exactness while comparing after rounding; this is its mirror — **a fix reported as implemented that was not written.** ⚠️ And the claimed value would not have fixed it either: a correct relative band takes the failure from **68 → 90 of 181**, because the tie-break *rule* ("nearest +x") flips at 45°, so **every lot rotated past 45° is wrong by construction**. **Fixed**, and the standing rule that came out of it is in this table's row 54: the fix pass quoted its band **out of the shipped file** and re-measured every number in its own header comment before committing — which caught one it had guessed | B1 `P1` |
| 50 | **A FIXTURE PROPERTY LOAD-BEARING WITHOUT SAYING SO — the third time this build.** The new rotated-lot fixture samples **one angle, 30°** — and 30° is in the passing set. The oracle was excellent (a hand-typed ring rigidly rotated, two genuinely independent derivations) and still proved nothing about the 68 failing orientations, because **one sample cannot discriminate a property that varies with the sample.** Compounding it, one half of the fix has **no isolating mutation**: the scope row reddens its clause *through the containment guard*, never through a mis-oriented frame that stays inside the lot. **Fixed**: standing coverage is a **37-angle sweep (0–180°, 5°), one prim per angle in one cook**, compared with a 1e-3 m tolerance rather than after rounding (exact 3-dp equality fails on 4 of 181 angles on a CORRECT build); and the isolating mutation restores the world tie-break, which is correct at 0° and 30° and so reddens **`shape_frame` and nothing else**, with the footprint legally inside the lot | B1 `P1` |
| 51 | **A GUARD AND THE CHECK THAT VERIFIES IT SHARED THE SAME HOLE.** Both test corners only, at both ends of the pipeline. On an ordinary slotted lot a footprint **edge** runs **5.800 m outside the lot** between two corners that are both inside it — warnings 0, and `masses_inside_lots` reports **PASS** on 2 of 3 notch sizes. The limitation was stated for the guard and **not** for the check, so nothing recorded that the verifier could not see what the guard missed. **Fixed at both ends**: the guard splits each footprint edge at every parameter where it meets the lot and tests the pieces at their midpoints (exact, no sample step), `C._escapes` walks the edges at 0.25 m for both standing checks, and **one mutation proves both** — restore corners-only and the row is GREEN unless the check sees edges too. ⚠️ The fixture that reaches it had to be built (site 41): every other lot in both suites is convex, where corners imply edges | B1 `P2` |
| 52 | **A STATED LIMITATION THAT NAMED THE CASE WHICH CANNOT HAPPEN, while the one that can went unstated — row 51's shape, in a different file.** `pf_site_in.vfl` recorded its minted-id residual as *"two lots sharing a centroid to the centimetre"*, which **cannot occur for non-overlapping lots**. The residual that can occur is a **31-bit hash collision between lots with different centroids**, arriving in structured pairs because `h(x₁)^h(z₁) == h(x₂)^h(z₂)` forces `h(x₁)^h(z₂) == h(x₂)^h(z₁)` and city lots are on a lattice. Re-measured independently of the audit on a Python model of the same arithmetic: **0** collisions at 9 600 lots (20 × 30 m), **0** at 46 225, **18** at 160 000 on a 5 m grid. **Recorded rather than fixed, and the reason is in the file**: `pf_site_id` is an **int**, so 31 bits is one bit short of everything there is, and the pairing survives any bijective post-mix — escaping it needs a wider id (a schema decision, Hannes') or reading the streets `lot_id` by name | citygen buildings, B1 `P3` |
| 53 | **A MUTATION'S STATED REACH WAS NEVER MEASURED, AND THE FAILING MESSAGE TRUNCATED ITS OWN EVIDENCE.** §12.10d claimed *"`degrades` is proven through both ops … the clause's mutation reddens on all three degraded sites"*. It reddened on the sites whose warning had **no other source** — and `shape_ops`' message printed `bad_degrade[:2]`, so a reader could not tell "two sites" from "the first two of however many", while the `value` array carried the true count all along. An independent audit read **2 of 3** off that message. Substantively the audit was right: `shapeU`'s refusal was **not** proven, because the one degraded `shapeU` site was a 6 × 6 lot that `pf_collapse` flags by its **own** terms. **Fixed** by deleting the slice and by adding a site whose warning has exactly one possible source (an 8 × 20 lot refusing a 10 × 8 notch and then surviving its setbacks by 13 m × 3.5 m); the mutation now reddens `[32, 38, 42]`. ⭐ **The rule: a failing message may not slice an evidence list shorter than the count it reports** | citygen buildings, B1 `P4` |
| 54 | ⭐ **ROW 49 CAUGHT IN FLIGHT, IN THE PASS THAT WAS FIXING ROW 49.** The fix pass wrote into `pf_shape.vfl`'s header that normalising the candidate areas to the lot's own origin makes the float32 spread *"4.24e-07 at every one of those origins"* — **before measuring it**. Measured, it is 3.39e-07 at x = 0 and **2.89e-05 at x = 20 km**: normalising removes the cancellation (worth ~5×) and cannot remove the input's own resolution, because `P` is float32 and a lot 20 km out is already quantised to ~2 mm. The claim was corrected to the measured table before the commit, and the real ceiling — float32 `P`, not the band — is now named in the file. ⚠️ **Knowing the shape did not prevent writing it**, exactly as row 40 says of `R4-1`; what prevented shipping it was re-running the probe against the sentence | citygen buildings, B1 fix pass |
| 55 | **THE OBVIOUS FIX WAS WRITTEN, MEASURED, AND FOUND WRONG — before it shipped, and only because a probe was run on the case it was written for.** `P2`'s guard was first fixed with a **proper segment-crossing test** (both segments straddling the other's line), which is the textbook answer and reports the failing case as CONTAINED: the footprint edge runs along `z = 24` and the slot's two sides meet it exactly **at their endpoints**, so it crosses nothing — it passes through two lot **vertices**. That is not a degenerate corner case; it is what a slot bitten out of an edge the footprint runs along looks like every time. Replaced with a midpoint scan over the pieces the edge is cut into, which is exact. ⭐ **The rule: run the new guard against the measured failing case before writing its comment, not after** | citygen buildings, B1 `P2` |
...
| 58 | ⛔ **A NEW STAGE SILENTLY REPAIRED THE DEFECT AN OLDER STAGE'S MUTATION INJECTS, AND THE ROW WENT GREEN.** B3 was wired into the main chain and `no_scratch`'s registry row — *"the prim-class `_*` sweep is removed"*, RED for two cycles — turned **GREEN**: B3's own cleanup node sweeps `_*` on all four classes, so deleting B2's sweep left the SHIPPED output clean while B2's own output leaked. The row had stopped being about the node it was written for. ⚠️ **Nothing in the sweep's design can see this**, because the row still names a real check and that check still measures a real property; only the *subject* moved downstream. **Fixed** at both ends: the row is re-paired with a new `no_scratch_b2` check on **B2's own output** — a stream `plan_follows_data_b0` and every shape-op check still consume — and `no_scratch` gets a mutation on **B3's DETAIL sweep**, the one class B2 had nothing to remove from and which therefore could not fail until B3 wrote to it. ⭐ **The rule: adding a stage downstream of a guarded property re-opens every mutation that guards it. Re-run the sweep after wiring a new stage into a chain, and read the GREENs, not the count** | citygen buildings, B3 |
| 59 | ⚠️ **A SNAPSHOT ROW THAT COULD NEVER COMPARE EQUAL — 20 phantom movements, on every run, for ever.** `record()` stored B3's two per-storey arrays as **tuples**; the baseline is JSON, JSON round-trips a tuple as a **list**, and `diff()` compares with `!=`. The FILE was correct at every moment — `json.dumps` cannot tell the two apart — so the defect was invisible in the artefact and loud in the runner, which reported `20 moved value(s)` and exited non-zero immediately after a `--update-baseline` that had absorbed nothing. **Fixed** by storing lists. ⭐ **The rule: a value that goes into a serialised snapshot must be of the type the serialiser hands back, and the cheap proof is to run `--update-baseline` and then run again — a snapshot that does not report zero on the second run is broken, whatever the first run said** | citygen buildings, B3 |
| 60 | ⛔⛔ **"NOT STATED" WAS A CLAIM ABOUT THE SEARCH, WRITTEN INTO THE DATA AS A CLAIM ABOUT THE WORLD — and the source was already in the project, extracted, unread.** `at_ziegel_gruenderzeit.bayMaxM` shipped as *"NOT STATED. No source for a Gruenderzeit Fensterachse spacing was found"*. The spacing is in *Anhang I — Historisches Mauerwerk der Wiener Gründerzeit*, **downloaded and `pdftotext`-extracted into this build's own scratchpad the evening before B3 ran** (2026-08-26 19:34) and cited nowhere: Friedel 1900, *"In gewöhnlichen Wohnhäusern beträgt die fensterachsen-Distanz 2.50 bis 3.00 m"*, plus the author's own table from *"mehr als 20 Einreichprojekten"* giving Achsen-Distanz 2,50 / 3,00. ⚠️ **THE RESTRAINT THAT PRODUCED IT WAS CORRECT AND ONLY ITS CONCLUSION WAS WRONG** — declining to cite what could not be verified is this project's doctrine and it stopped a bad citation the same night (the 37–74 cm band, which lives in a FIGURE that did not survive extraction and genuinely could not be verified). The error is the step after: *"I could not verify it"* became *"it is not stated"* without re-opening the documents already in hand. It is a **frame error** in this table's sense — a true statement about the search reported as a statement about the world — and it cost the build its headline conclusion: §12.10e reported *"the culture-side arm of §9c's chain is unexercised by every sourced style"* upward as a finding about §9's hypothesis, and with the sourced `bayMaxM 3.0` the cap binds **below** the 6.0 m span, so it binds on a sourced system. ⭐ **THE RULE: "I could not verify it" must be re-tested against the documents already in hand before it becomes a NOT STATED.** The restraint is about the CITATION, never about the SEARCH. **Fixed** in the data and in the doc, and the re-test was then run against every held document for every remaining `0.0` — which found two more false "nothing was found" lines on the Vierkanthof (a vault span in Rücklinger, an axis-count band in Huber; both named in the `sources` line, neither adopted, because a vault span is not a floor span and an axis count is a division away from an axis distance) and one admitted placeholder replaced by a sourced number | citygen buildings, B3 round-1 audit |
| 61 | ⚠️ **A TOLERANCE WHOSE STATED JUSTIFICATION WAS EXACTLY INVERTED — the third appearance of rows 48 / 49 / 54's shape, and the first where the number itself was written before it was computed twice over.** `pf_structure.vfl` said its bay epsilon *"sits just above"* float32 noise: *"6 µm at a 6 m cap … absolute float32 noise on a 100 m face is ~7.6e-6 m."* **Measured: 6e-6 < 7.63e-6, and one ulp at this fixture's own ~500 m domain is 3.05e-5 m — the band sat an ORDER OF MAGNITUDE BELOW the noise, not above it.** The two numbers in the sentence were both right and the comparison between them was backwards. ⚠️ **And the audit's "consequence is nil today" did not survive the fix that came with it:** the sourced 3.0 m bay cap makes exact multiples ordinary, and **20 of the 73 capped wall faces in the G1 fixture now sit EXACTLY on an integer multiple of their cap** — where one ulp on the wrong side is 20 bays against 21. **Fixed** by making the band a LENGTH rather than a ratio — `float BAY_EPS_M = 1e-3;`, the same millimetre `pf_warn_span_exceeded` already uses, ~16× the coordinate noise at 500 m and three orders under a brick — and by replacing the comment with the measurement. **It moved no fixture value**, which is the check that it was a justification fix and not a behaviour change. ⚠️ **What no check can see, and it is stated in the file:** no fixture face lands inside the band on the wrong side, so the band has no isolating mutation; the two `bay_respects_the_span` rows anchor the COUNT expression, not this number | citygen buildings, B3 fix pass |
...
⭐ **WHY 60 IS THE FIRST ENTRY IN THIS TABLE THAT IS NOT ABOUT A CHECK.** Every incident above it is a
verification failure — a check that could not fail, a mutation whose reach was assumed, a tolerance
measured against the wrong magnitude. **60 is a RESEARCH failure with the identical shape**: a true
statement about what the searcher managed to confirm, written down as a statement about what exists.
The correction discipline this log has built for CODE — *state what you measured against, and
re-measure before you write it down* — turns out to apply verbatim to `sources`. **The library of
downloaded documents is the "current build" for a provenance claim, and it must be re-cooked before
a NOT STATED is committed**, exactly as an audit re-cooks the fixture before it believes a green.
