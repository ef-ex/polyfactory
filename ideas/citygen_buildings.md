...
| ⛔⛔ **STOP — B4 NEEDS A DECISION THAT IS HANNES', AND THE OVERNIGHT RUN ENDED HERE (2026-08-27 ~10:00)** | **Everything below this row is history; read this row first.** Closed by independent audit: **G1, G2, G3, B0, B1, B3**; B2 arrived from G1; B4/B5/B6 exist as G2 prototypes. **B4 is next and it is not a wiring task.** Measured across two cycles and one audit, with negative and positive controls: **both arms of §9c's chain now compute correctly on SOURCED systems** — the culture arm (`bayMaxM 3.0`, Friedel 1900 + a 20-project survey) **binds** on the Gründerzeit block and turns a 60 m Blockrand front into **20 axes of 3.000 m** instead of 10 bays of 6.000 m — **and the building is BIT-FOR-BIT IDENTICAL** (0 of 133 faces move; positive control moves 56). ⛔ **Connecting it takes TWO changes, not one:** **(i)** B3 is not in B4's chain at all — `structure()` is a leaf off B2, `build_shell()` takes B2's OUT, and **no line of production reads any of B3's five published names**; **(ii)** only then the facade input contract — `plan.fit`'s `mode="count"` is fed by a parm or row alignment, never a per-face attribute, so **wiring `pf_bay_u` through changes polyChain's INPUT CONTRACT — another tool, owned by another session.** **Three options and an agent may not pick one:** ① **connect it** (cross-tool change; makes §9e's layer 1 load-bearing in geometry); ② **leave it computed-and-unread** (cheap and honest, but §9's hypothesis stays untested in geometry — **this is what ships today and it is not neutral**); ③ **delete the arm and stop claiming it.** ⚠️ **Also unresolved and B4's to inherit:** `wallThicknessesM` is **ground-indexed** while Bauordnung 1883 §37's ladder is **top-anchored** — exact at 5 storeys, too thick on the 3-storey tract; a top-anchored index is a new field semantic and therefore Hannes'. |
...
2. ⚠️ **`bayRangeM` ships as `bayMaxM` — the upper bound only.** The lower bound has no consumer
   ...
   ⛔ **Adding a warning name is Hannes'** (§0.0g row 6's precedent). ⭐ **And the field is EXERCISED
   ON SOURCED DATA since 2026-08-27**: `at_ziegel_gruenderzeit` states `bayMaxM 3.0` (Friedel's
   Fensterachsen-Distanz), which sits below its 6.0 m span, so the culture-side arm of §9c's chain
   binds on a real system — it shipped as *"NOT STATED"* on a source the project already held, and
   the correction is §12.10e's fix pass.
...
5. ⛔ **THE SPAN→BAY CHAIN IS DERIVED, CONSISTENT AND DOES NOT MOVE A VERTEX.** Measured: changing
   ...
   geometry. ⭐ **And it holds for the CULTURE arm too, on sourced data** (fix pass, 2026-08-27):
   giving `at_ziegel_gruenderzeit` its sourced `bayMaxM 3.0` moves **45 of 45** Gründerzeit wall
   faces — a 60 m Blockrand front goes 10 × 6.000 m → 20 × 3.000 m — with **0 of 133 faces differing
   in raw float32 `P` bits**, against a positive control that moves 56. ⛔ **Closing it is TWO
   changes and §12.12 named only one**: B3 is not in B4's chain at all (`structure()` is a leaf off
   B2 and nothing in production reads its five names), so first wire B3 between B2 and B4, then
   extend polyChain's facade input contract (`plan.fit(length, nominal, …)`). Both are B4's.
...
| Warning | Fires when | What is built instead |
|---|---|---|
| `pf_warn_span_exceeded` | the volume's clear span (its floor face's `area / longest plan edge`) exceeds the construction system's `maxSpanM` by more than 1e-3 m | the building, at the span asked for. ⚠️ **It fires on both Gründerzeit styles and that is CORRECT, not noise**: B2 builds no Mittelmauer, so the tract really is a **9.0–16.0 m** clear span against a 6 m timber floor (⚠️ **this row said 9.6–14.0 m — wrong in the safe direction, re-measured per volume 2026-08-27, §12.10e's fix pass**). ⭐ **And the ruling is SOURCED, not merely argued**: Fierro states the Gründerzeit load path outright — *"Die Decken spannen sich jeweils als Einfeldträger von der straßenseitigen Außenmauer zur Mittelmauer und von der Mittelmauer zur hofseitigen Außenmauer"* — so the missing Mittelmauer is a missing support, not a modelling opinion. The fix is an intermediate support in B2/B3, never a bigger number |
...
⚠️ **AND THAT RULE HAD ONE EXCEPTION UNTIL 2026-08-27, WHICH THIS DOC STATED ABSOLUTELY AND THE DATA
DID NOT:** `at_mauerwerk_land.wallThicknessM` shipped as `0.45` labelled *"UNSOURCED (…) a placeholder
the cascade overrides"* — honest in the data, overstated here. It is sourced now, so the rule holds
without exception again; if one is ever re-added, this sentence has to move with it.

⛔⛔ **THE TABLE BELOW IS THE CORRECTED ONE (fix pass, 2026-08-27). Three of its cells moved and the
reason is in the round-1 audit + fix pass at the end of this section — read that before quoting any
row of it.** The superseded values are named there, not hidden.

| Field | `at_lehm_massiv` | `at_ziegel_gruenderzeit` | `at_mauerwerk_land` | `fic_coruscant_mega` |
|---|---|---|---|---|
| `maxSpanM` | **5.0 DERIVED** | **6.0 DERIVED on a SOURCED premise** | **0.0 NOT STATED** | 400.0 AUTHORED FICTION |
| `bayMaxM` | **0.0 NOT STATED** | ⭐ **3.0 SOURCED** (was `0.0 NOT STATED`, falsely) | **0.0 NOT STATED** | 60.0 AUTHORED FICTION |
| `maxStoreys` | **2 SOURCED** (modern) | **5 SOURCED** | **2 SOURCED** | 500 AUTHORED FICTION |
| `wallThicknessM` | **0.365 SOURCED** (modern) | **0.45 SOURCED** | **0.70 SOURCED** (was `0.45 UNSOURCED`) | 2.5 AUTHORED FICTION |
| `wallThicknessesM` | — | **1→0.75, 2→0.60, 3→0.60 SOURCED** (was `0.60 / 0.60 DERIVED`) | — | 1→4.0 |
...
- ⭐ **SOURCED, and flagged MODERN:** the **Lehmbau-Regeln (1990s)** limited load-bearing earth
  ...
  the data, not hidden.
- **SOURCED:** the 1883 Bauordnung brick format **14/29/6.5 cm**, cellar walls **up to 1 m**
  ...
  Vierkanthof. ⛔ **The same blog's "60 cm Stiegenhaus minimum" was CITED FOR THE WRONG WALL and is
  gone from the ladder's provenance** — Friedel gives *Stiegenmauern* as **0.30 / 0.45 m**; the 60 cm
  is the **Mittelmauer** (*"im obersten Geschoße 2 Ziegel (0.60 m)"*, 0.75 m in the ground floor of a
  five-Geschoß house). The blog's other two facts stand.
- ⭐ **SOURCED, and it is the stepping rule B3 declined to cite:** **Bauordnung für Wien 1883 §37**,
  quoted verbatim in **Braun** — a document this template already cited three times —
  *"So müssen die Hauptmauern bei Tramdecken im obersten Stockwerk 45 cm dick sein und nach unten
  alle zwei Stockwerke um 15 cm dicker werden. Daraus folgt, dass vierstöckige Gebäude im Erdgeschoß
  Wandstärken von 75 cm haben."* Friedel says the same from the other side in
  [Anhang I](https://nachhaltigwirtschaften.at/resources/sdz_pdf/FFG873475-Anhang-II-historischesMWK-Statik.pdf).
  So a five-Geschoß house is **0.75 / 0.60 / 0.60 / 0.45 / 0.45** from the ground up — the shipped
  table over a 0.45 default. ⚠️ **One limitation of the field shape, stated rather than left to read
  as exact:** the Bauordnung's ladder is anchored at the **top** storey and `wallThicknessesM` is
  indexed from the **ground**, so the table is exact for a five-Geschoß volume and too thick for a
  shorter one — the three-storey rear tract gets 0.75 / 0.60 / 0.60 where the same rule read from the
  top gives 0.60 / 0.45 / 0.45. A top-anchored index is a new field semantic and therefore Hannes'.
- ⭐ **SOURCED, the Vierkanthof's wall, replacing an admitted placeholder:** *"Die massiven Trakte
  alter Vierkanthöfe weisen meist eine sehr dicke Mauerstärke auf (…) Als Richtwerte wird zuerst eine
  70 Zentimeter starke Sandsteinwand herangenommen"* (Huber 2021). ⚠️ **It is a Richtwert taken for a
  U-value calculation, not a structural table** — flagged in the data the way the Lehmbau-Regeln's
  modernity is. The 0.45 it replaces was 1.5 Stein off the **Viennese brick** ladder, which is the
  wrong material as well as an unsourced number.
- **DERIVED, with the derivation stated:** the Einhof's **5.0 m span** is the *sourced house WIDTH*
  ...
  Gründerzeit **6.0 m** is the sourced ~12 m Wiener Dachstuhl halved over a load-bearing Mittelmauer;
  ⭐ **the Mittelmauer half is no longer a reading** — see §12.8 and the fix pass below.
- ⛔ **NOT sourced, and NOT cited because of it — one of the two, and the other one was in a document
  already read:** a 37–74 cm ground/cellar band came back from search and belongs where it was left,
  because it lives in a **figure** (Kolbitsch 1989's comparison table) that did not survive text
  extraction. The *"reduced by half a stone every one to two storeys"* stepping rule did **not**
  belong there: it is plain text in Braun, and it is cited now. **That is `build_retrospective.md`
  §2a row 60 in one line — the restraint was right and the conclusion was wrong.**
...
Two statements rather than one ternary **because each arm needs its own mutation anchor** — and the
reason given for it has been overtaken: the span arm binds every real system and the bay arm was said
to bind "only the invented one", which stopped being true when `bayMaxM 3.0` shipped. **Both arms now
bind on `at_ziegel_gruenderzeit`, the smaller one wins, and the two anchors are still two anchors.**
⛔ **THE `1e-6` BAND'S JUSTIFICATION WAS EXACTLY INVERTED AND IT IS FIXED** (`build_retrospective.md`
§2a row 61). It said the band *"sits just above"* float32 noise. **Measured: 1e-6 of a 6 m cap is
6e-6 m; one float32 ulp is 3.81e-6 m at 48 m, 7.63e-6 m at 100 m and 3.05e-5 m at 500 m — so at this
fixture's own ~500 m domain the band sat an ORDER OF MAGNITUDE BELOW the noise.** The two numbers in
the sentence were right and the comparison between them was backwards. ⚠️ **And "consequence nil" did
not survive the sourced cap:** 20 of the 73 capped wall faces now sit **exactly** on an integer
multiple of their cap, where one ulp on the wrong side is 20 bays against 21. The band is now a
LENGTH — `float BAY_EPS_M = 1e-3;`, the same millimetre the span warning uses, ~16× the coordinate
noise at 500 m and three orders under a brick — and **it moved no fixture value**, which is what
makes it a justification fix rather than a behaviour change.
...
be discovered.
⛔⛔ **THE PARAGRAPH THAT STOOD HERE IS FALSE AND IS QUOTED IN FULL RATHER THAN DELETED, BECAUSE IT
WAS REPORTED UPWARD AS A FINDING ABOUT §9's HYPOTHESIS:** *"`bayMaxM` is NOT STATED on every real
system in the library, because no source for a Gründerzeit Fensterachse spacing or a Streckhof
opening rhythm was found. So even as data, the culture-side arm of §9c's chain is unexercised by
every sourced style here — only the invented Coruscant block binds it (…) this build has no sourced
number at all for the second."*
⭐ **CORRECTED 2026-08-27. The Gründerzeit Fensterachse spacing WAS sourced — in a document this
project had downloaded and extracted the evening before B3 ran, and cited nowhere** (Friedel 1900 via
*Anhang I*: *"In gewöhnlichen Wohnhäusern beträgt die fensterachsen-Distanz 2.50 bis 3.00 m"*, plus
the author's own table from *"mehr als 20 Einreichprojekten"*). `at_ziegel_gruenderzeit` now states
**`bayMaxM 3.0`**, which sits **below** its 6.0 m span, so **the culture-side arm of §9c's chain
BINDS ON A SOURCED SYSTEM** — measured below. **The Streckhof half of the sentence stands**: no
Streckhof document is held here, so there is nothing to re-read and no opening rhythm was found.
§9c's claim is *"material + jointing set a maximum practical span; span sets the bay"*; this build
now shows **both** halves driving numbers on real systems, and still **neither** of them driving a
vertex. **How the false half came to be written is `build_retrospective.md` §2a row 60.**
...
⛔ **IT WAS THE ONLY BLOCK IN THE LIBRARY WHOSE BAY CAP SAT BELOW ITS SPAN, AND IT IS NOT ANY MORE**
(fix pass, 2026-08-27): `at_ziegel_gruenderzeit` states a sourced 3.0 m cap under a 6.0 m span, so
the culture-side arm can be seen to bind on a **real** system too. What is still only here is a bay
cap on an **invented** material. ⚠️ **Its mutation's blast radius grew with that, and the old claim
is superseded:** `float hi = bcap;` → `float hi = span;` was recorded as reddening `bay_cap_binds`
*"and nothing else in either suite, precisely because every real system states no bay cap"*. It now
reddens **`bay_cap_binds` and `structure/bay_respects_the_span`** — re-measured, every clause diffed
— because the Gründerzeit walls go back to 6 m bays against a 3 m cap. **That second clause is the
culture arm binding on a sourced system, asserted rather than described**, and both clauses keep
their own isolating rows. ⚠️ **The other half of the old claim was true and vacuous and stays worth
saying: G2 never wires `structure()`, so `pf_structure.vfl` is not cooked there at all.** Coruscant's
two other mutations move the invented system itself — `maxStoreys` → 4, and `storeyHeightsM` → `[]` —
and each reddens one clause and nothing adjacent.
...
- ⛔ ~~**The `bayMaxM` arm on any sourced style** — no real system states one (see above).~~
  ✅ **CLOSED BY THE FIX PASS 2026-08-27**: `at_ziegel_gruenderzeit` states a sourced 3.0 m cap and
  `structure/bay_respects_the_span` asserts it on 73 wall faces, with `min(span, bayMax)` as the
  cap — so the arm is exercised, on sourced data, with a mutation that reddens it. ⚠️ Still not
  verified: that 3.0 m is TRUE of Gründerzeit Vienna. That is the `sources` list's job and a human's.
- ⚠️ **`pf_warn_span_exceeded` fires on `at_vienna_perimeter` and `at_zinshaus_row`, and it is
  CORRECT rather than noise.** B2 builds no Mittelmauer, so the tract really is a **9.0–16.0 m**
  clear span against a 6 m timber floor. ⚠️ **This doc said "9.6–14.0 m" and that was wrong in the
  SAFE direction** (§2a row 57(i)'s shape) — re-measured per volume: site 2 runs 9.0 / 9.0 / 9.6 /
  9.6, site 4 12.012 / 13.988, site 8 **16.0 / 16.0**, sites 6/7/9 10.0. The claim was understating
  its own case; the warning SET was right either way. **The fix is an intermediate support in
  B2/B3, not a bigger number**, and tuning the number to silence it would have been exactly the
  frame error §2a warns about. Eight of the ten G1 fixture sites warn; the expectation is
  hand-derived in `SPAN_EXCEEDED`, edge by edge, and `span_warning_is_true` compares the SET of
  warning sites against it in both directions.
...
#### ⭐ Round-1 FIX PASS (2026-08-27, on `HEAD` `090e2b2`) — the provenance blocker is closed, and the correction is a RESULT

⭐ **B3 IS DONE AS A STAGE — the audit granted that and nothing here re-opens it.** ⛔ **What was
withheld is now released only for the four claims named below: the record's other paragraphs stand
as the audit left them, and this pass rendered no image, so §0.0g row 3 is untouched.**

##### ⭐⭐ (a) `bayMaxM` — the blocker, the shipped line, and the measurement

**The shipped line, quoted out of `polyfactory/library/citygen/systems/at_ziegel_gruenderzeit.geo`**
(`bayMaxM` is `"type":"float", "value":3`):

> `bayMaxM 3.0: SOURCED.` ⛔ `THIS FIELD SHIPPED AS 'NOT STATED, no source found' AND THAT WAS FALSE:
> the source was downloaded and text-extracted into this project's scratchpad the evening before B3
> ran, and nobody re-read it. Friedel 1900, quoted in §2.2 'Fassade: Oeffnungen und Pfeiler': 'In
> gewoehnlichen Wohnhaeusern betraegt die fensterachsen-Distanz 2.50 bis 3.00 m' (lower-case 'f' is
> the extraction's, not a second spelling) - and on the next page the author's OWN table 'Recherchen
> aus mehr als 20 Einreichprojekten des Verfassers' gives Achsen-Distanz a [m] 2,50 / 3,00 against
> Fensterbreite 1,20-1,50 m and Pfeilerbreite 1,00-1,80 m. 3.0 is the top of the band, which is what
> a MAX is.` …⭐ `CONSEQUENCE, AND IT IS A RESULT ABOUT §9c RATHER THAN A DATA TIDY-UP: 3.0 sits
> BELOW this system's 6.0 m span, so the culture-side arm of the chain BINDS on a sourced system - it
> is no longer exercised only by the invented Coruscant block.` ⚠️ `Palais and Monumentalbauten are
> given 4.00-5.00 m by the same sentence and are a different building type; this block is
> 'gewoehnliche Wohnhaeuser'.`

⚠️ **The document's own title page says "Anhang I" and the URL slug says "Anhang-II"** — both are in
the citation, because the slug's *Anhang-I* is a different file (Prieler's Bauordnung table, already
cited elsewhere here). Retrieved 2026-08-26 19:34, which is where "the evening before" comes from.

**⭐⭐ WHAT CHANGED WHEN THE CAP BEGAN TO BIND — and this is the interesting measurement, because it
is a result about §9c and not about a data file.** Whole ten-site fixture, `bayMaxM` patched by
`systemId` so every style reading the block moves; positions hashed as **raw float32 bits**, faces
keyed on `pf_elem_id`:

| | result |
|---|---|
| **negative control** — 3.0 vs 3.0 | 0 of 133 faces differ (the hash is not constant-by-accident) |
| **positive control** — `storeyHeightsM` → `[]` | **56 of 133 faces differ** — so *"I failed to observe a change"* is excluded |
| **bay grid**, `bayMaxM` 0.0 → 3.0 | ⭐ **45 of 45 Gründerzeit wall faces move. Every one.** |
| **geometry**, `bayMaxM` 0.0 → 3.0 | ⛔ **BIT-IDENTICAL — 0 of 133 faces differ** |
| the longest Viennese wall (`2:B2:v0:outer`) | **10 bays × 6.000 m → 20 axes × 3.000 m** |
| widest bay, per style | `at_vienna_perimeter` 6.000 → **3.000**; `at_zinshaus_row` 5.714 → **2.857**; `at_einhof` 5.000 → 5.000 (span-bound); `at_vierkanthof` 54.000 → 54.000 (states nothing) |

⭐ **SO THE RESULT ABOUT §9 IS SHARPER THAN THE ONE B3 REPORTED, AND IT CUTS BOTH WAYS.** §9c's chain
is *"material + jointing set a maximum practical span; span sets the bay."* B3 reported that the
first half drove numbers and the second had **no sourced number at all** — a claim about the
hypothesis that turned out to be a claim about the search. Corrected: **both halves now drive numbers
on sourced systems, the culture half is the BINDING one on the Gründerzeit block, a 60 m Blockrand
front resolves to 20 axes of 3 m — which is what such a facade actually is — and the building is
still bit-for-bit the same building.** The decorative finding survives the correction intact and gets
stronger: the arm that was supposed to be the weak one is now sourced, binding, and equally
weightless. **§9c's chain reaches DATA on both arms and GEOMETRY on neither.**

##### The other three provenance corrections

1. ⭐ **The stepping rule was in a document already cited three times.** `wallThicknessesM` goes
   `1→0.60, 2→0.60 DERIVED` → **`1→0.75, 2→0.60, 3→0.60` SOURCED**, over the unchanged 0.45 default,
   so a five-Geschoß house is **0.75 / 0.60 / 0.60 / 0.45 / 0.45** from the ground up. Braun quotes
   **Bauordnung 1883 §37** verbatim. ⛔ **And the shipped 0.60 had been attributed to a property
   blog's "60 cm Stiegenhaus" — the wrong wall**: Friedel's Stiegenmauern are 0.30 / 0.45; 60 cm is
   the Mittelmauer, 0.75 in the ground floor of a five-Geschoß house, which is the same number Braun
   derives from §37 by a different route. ⚠️ **Limitation stated in the data**: the ladder is
   top-anchored and the field is ground-indexed (above).
2. ⭐ **The 6.0 m span is better supported than its own citation said.** Fierro states the load path
   outright — *"Die Decken spannen sich jeweils als Einfeldträger von der straßenseitigen Außenmauer
   zur Mittelmauer und von der Mittelmauer zur hofseitigen Außenmauer. Alle Geschosslasten werden
   somit über diese drei Mauern abgetragen"* — in the same document already cited here for the
   Dachstuhl, where it was cited **for the Dachstuhl span and pitch only**. ⚠️ **Stated precisely:
   it is said OF a worked example (a documented 1890 Gründerzeit house), not as a typology-wide law**
   — so the premise is upgraded to SOURCED and corroborated (Friedel's Hauptmauern step at a 6.50 m
   Zimmertiefe, which is where the Bauordnung itself puts the ordinary floor span), while **the
   number stays DERIVED**: halving 12 m is arithmetic on a truss span, not a floor-span table.
   ⚠️ **§12.8's ruling that the span warning is CORRECT rather than noise is therefore SOURCED, not
   merely argued** — corrected there too.
3. ⭐ **The one admitted placeholder is gone.** `at_mauerwerk_land.wallThicknessM` **0.45 UNSOURCED →
   0.70 SOURCED** (Huber 2021's Richtwert for the massive trakte of old Vierkanthöfe, flagged as a
   thermal reference value rather than a structural table). The 0.45 was 1.5 Stein off the *Viennese
   brick* ladder — the wrong material as well as an unsourced number — and §12.10e's rule about
   preferring `0.0` now holds without exception.

⚠️ **AND THE RULE THAT CAME OUT OF THIS WAS THEN APPLIED TO EVERY REMAINING `0.0`, which is the only
way it means anything.** Re-reading the held documents found **two more false "nothing was found"
lines**, both on the Vierkanthof: Rücklinger states the stall **vaults'** span (*"sechs- bis acht
Meter"*, Gurtengewölbe *"bis zu sechs Meter"*) and Huber an axis **count** band per front length
(Kleinvierkanter under 30 m with six to ten Fensterachsen; Großvierkanter 40–60 m with fifteen to
twenty). ⛔ **Neither is the field, and neither was adopted:** a vault span is not the timber
Deckenbalken span that sets a bay in the dwelling trakt, and an axis count over a front length is a
division away from an axis distance — deriving 2.0–5.0 m from it would end the Vierkanthof's one-bay
output and is an authoring decision, not a fix-pass one. **Both are now named in the `sources` line
so the next reader does not re-run the search.** The Streckhof half stands: no Streckhof document is
held here at all.

##### ⛔ The corrected account of why the chain is decorative — it is TWO changes, not one

§12.10e blamed polyChain's kit-sized bays, and the audit confirmed that half against the shipped
asset. **But B3 is not in B4's chain at all**: `structure()` hangs off B2's OUT as a **leaf**,
`build_shell()` takes B2's OUT as its own input, and **not one line of production reads
`pf_bay_u`, `pf_bay_v`, `pf_bay_width`, `pf_storey_split` or `pf_wall_thickness`** — they are
write-only names. So closing it is **(i) put B3 between B2 and B4, and only then (ii) extend the
facade's input contract**, and §12.12's row named only (ii). ⚠️ **B4 must be planned around both**,
and (i) is the cheap one that has to happen first. **§12.12's row is corrected in place.**
⚠️ **Two method flaws in §12.10e's own probe, recorded so the record does not imply a cleaner method
than was used**: `b3_chain.py` hashed positions **rounded to 1e-6** (a comparison after rounding,
CLAUDE.md rule 5) and keyed faces by `(site, volume, wall_role)`, **not unique per face**, with
`setdefault` silently keeping the first. Neither changed its answer — the audit and this pass both
re-measured on raw float32 bits keyed on `pf_elem_id` and got the same verdict. ⚠️ **And §12.10e's
own table was measured on an uncommitted three-lot scratch script that is not in the repo**; the
audit's and this pass's numbers are from the **shipped ten-site fixture**, which is why the face
counts differ (18 of 26 there, 45 of 45 Gründerzeit wall faces here).

##### The queued items — what was fixed, what is recorded

- ⭐ **The bay epsilon's inverted justification: FIXED**, value and comment (§2a row 61, and the
  measurement is above). It moved no fixture value.
- ⭐ **Three undeclared blast radii: DECLARED, and re-measuring found two more.** Every clause diffed
  under each mutation. `others_silent` → also `fiction_is_silent`; `splits_match_the_wall` → also
  `heights_follow_data` **and** `fiction_is_coherent`; `thickness_follows_the_table` → also
  `fiction_is_coherent`; **new:** B2's prim `CLEAN` row → also `attribute_storage`, and **both**
  `bay_respects_the_span` rows → also `bay_cap_binds` (one shared count line). No pairing is
  invalidated: every clause still has its own isolating row, and all 68 mutations are RED.
- ⛔ **The unreachable sweep terms: MEASURED AND RECORDED, NOT DELETED.** One term at a time:
  removing **any** of B2's four `CLEAN` classes reddens `no_scratch_b2` (row 58's fix generalises,
  confirmed); removing B2's `groupdelete`, B3's `groupdelete`, or B3's **point, vertex or prim**
  clean class reddens **nothing** — only B3's detail class can fail. `conventions.md` §2 asks for all
  four classes and a term that cannot fail today catches the day B3 writes point scratch, so they
  stay; **the coverage claim is four terms wide and one term deep and now says so**, in
  `buildings.py` and in the registry row.
- ⭐ **Tract span 9.6–14.0 → 9.0–16.0**, corrected here, in §12.8 and in `SPAN_EXCEEDED`'s
  hand-derivation comment, where **site 8 was written as "7.39 / 8.61" and measures 16.0 / 16.0**.
- ⭐ **§12.12's CLOSED row now cross-links §0.0g row 7**, which was **not** ratified.

##### The sweeps, the baseline and the budget

**Both sweeps after the last production edit: G1 25 checks / 54 clauses / 68 mutations ALL RED, G2
6 / 14 / 15 ALL RED, 0 failing on either.** ⭐ **`baseline_g2.json` untouched and G2's baseline 0
moved** — G2 never wires `structure()` and its `g2_lshape` style names no system, so nothing there
could move, and it did not. **G1's baseline moved exactly 19 values and was re-blessed**: `bayU` /
`bayWidth` on the six Gründerzeit sites, `wallThickness` on those six **and** on the Vierkanthof.
⛔ **Nothing else moved — no `topY`, no `planBox`, no `planAreas`, no warning, on any site** — which
is the baseline agreeing, at 3 dp, with the bit-identity measured above. Row 59's proof was run:
`--update-baseline`, then a clean run reporting **0 moved**.
⚖️ **Budget 2 590 test / 1 178 production = 2.20×**, from 2 560 / 1 176 = 2.1769×. **Marginal +30
test / +2 production**, and the shape of it is worth stating plainly: the 2 production lines are
`BAY_EPS_M` and the wrapped ternary; **the 30 test lines are entirely registry prose — declared blast
radii and corrected mutation descriptions. No new check, no new clause, no new fixture, no new
mutation.** §0.0g row 4 is unchanged and still Hannes'.

##### What this fix pass could NOT verify

- ⛔ **That 3.0 m, 0.75 m or 0.70 m is TRUE of the material or the city it names.** Every one of them
  is now quotable to a document and a page; none of them is verified against a building. That is the
  `sources` list's job and a human's, and it is the same limit B3 stated.
- ⚠️ **Fierro's load-path sentence is stated of a worked example**, and this pass did not find a
  typology-wide statement of it. The corroboration (Friedel's 6.50 m Zimmertiefe threshold) is
  independent but indirect.
- ⛔ **The bay band has no isolating mutation** and cannot get one on this fixture — no face lands
  inside it on the wrong side. Stated in `pf_structure.vfl`.
- ⛔ **No image was rendered or opened.** The geometry is bit-identical, so no gate image could have
  moved and none did — but **B3's own output still has no committed image**, its bay grid is still an
  attribute nothing draws, and **Hannes' viewport pass is owed on G1, G2, a `shapeU`, a rotated L and
  six taller Gründerzeit masses. No agent may record it as satisfied.**
...
| ~~**NEW — per-storey heights**~~ | ✅ **CLOSED 2026-08-27 by B3, §12.10e.** ⛔ **"CLOSED" MEANS THE FIELD IS AUTHORABLE AND REACHES THE MASS. IT DOES NOT MEAN THE PLACEMENT IS RATIFIED: [§0.0g row 7](#00g-owed-to-hannes--decisions-no-agent-may-take-as-of-2026-08-26) IS STILL OPEN AND STILL HANNES'.** The implementer put the table on the **construction system**; the two alternatives row 7 names — on the style root, or per VOLUME inside `volumes[]` — were not built, and **building on a placement does not ratify it** any more than building on the B0 schema ratified that (row 1). Read row 7 before treating anything below as settled. A per-storey table IS authorable — `constructionSystem.storeyHeightsM: [{"n": 1, "hM": 4.2}]`, a list of DICTS, which G3 measured as round-tripping intact (only a list-in-list is fatal, and `assert_storable()` raises on that at authoring). ⭐ **And it reaches the MASS**: `stamp()` sums it into `_volh`, `pf_mass` builds the wall to the sum, so both Gründerzeit styles now stand at 18.200 m with splits `[4.2, 7.7, 11.2, 14.7, 18.2]` — publishing the table while the wall stayed at `storeys × storeyHeightM` would have made the template say a thing the building does not do. A system with NO table keeps the original product expression bit-for-bit, and exactly the six sites on `at_ziegel_gruenderzeit` moved. ⚠️ The number 4.2 is **DERIVED**, not sourced: OEAW gives Raumhöhe 3.2–4.0 m and *"in der Erdgeschosszone oft auch darüber"*. ⚠️ And the 3.5 m it is measured against is a **Raumhöhe used as a storey height** — an inherited imprecision recorded in §12.10e, not corrected |
| **NEW — the bay grid has no geometric consumer** | **B4, AND IT IS TWO CHANGES, NOT ONE — this row named only the second** (corrected 2026-08-27 by the round-1 audit + fix pass). ⛔ **(i) B3 IS NOT IN B4's CHAIN AT ALL**: `structure()` hangs off B2's OUT as a **leaf**, `build_shell()` takes B2's OUT as its own input, and **not one line of production reads `pf_bay_u`, `pf_bay_v`, `pf_bay_width`, `pf_storey_split` or `pf_wall_thickness`** — they are write-only names. B3 has to be wired **between** B2 and B4 first, and that is the cheap half. **(ii) THEN the facade's input contract**: polyChain's facade sizes bays from the **kit** (`plan.fit(length, nominal, …)`); its `mode="count"` (D122) takes `n` from a **parm** or from an aligned row's measurement, never from a per-face attribute, so a per-face bay count is not expressible on that port today. ⚠️ **B4 must be planned around both.** Measured: changing `maxSpanM` **or** the sourced `bayMaxM` moves every published bay number **without moving a vertex** — 45 of 45 Gründerzeit wall faces, 0 of 133 faces differing in raw float32 `P` bits, against a positive control that moves 56 (§12.10e) |
