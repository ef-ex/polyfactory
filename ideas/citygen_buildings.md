...
`pf_shape.vfl` tests every ring corner against the lot — **the last node that still has it** — and a
...
position, and `site_ids_structural` compares the id → lot **mapping** across a reordered cook. Override kinds,
...
**Both sweeps, after the last production edit.** G1 **21 checks / 40 clauses / 51 mutations all RED,
