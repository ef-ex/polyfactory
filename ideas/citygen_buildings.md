...
INDEXING** is not, and `at`, the four axes and every inherited role come off the indexing. Which of
the rectangle's two edge directions becomes the frame's `u` is settled by a tie-break whose band is
**absolute `1e-6`** while the float32 spread over those geometrically identical directions is
**6.1e-05 – 9.77e-04** — so at a rotated lot the tie-break is never reached and the frame is chosen
by rounding error. **Measured: 68 of 181 half-degree orientations in 0–90° get the notch at the wrong
corner of the parcel** — a legal L, inside the lot, all four `pf_warn_*` at 0, every role and setback
on a different edge; `shapeU` 69 of 181. ⛔ **`shapeL`/`shapeU` ON A ROTATED LOT IS NOT DELIVERED**,
and the fixture cannot see it because it samples one angle, 30°, which happens to work.
